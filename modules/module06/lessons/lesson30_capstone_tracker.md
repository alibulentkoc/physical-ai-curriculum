...
unit: 08
lesson: 8.2
title: "Capstone II — The Resolved-Rate Tracker"
core_idea: "The tracker is the velocity layer made concrete: a loop that takes a commanded tool twist, resolves it into joint rates with the (damped) inverse, integrates open-loop, and recomputes — the exact joint-velocity stream Module 7 will drive with trajectories."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M6 L7.4 — Open-loop resolved-rate motion"
  - "M6 L8.1 — The capability analyzer"
  - "M6 L7.1–7.3 — Inverse velocity kinematics, pseudoinverse, damping"
learning_objectives:
  - "Implement the resolved-rate tracker as a clean velocity-layer loop."
  - "Confirm it follows a commanded tool velocity open-loop."
  - "Keep the interface twist-in / joint-rate-out, with no feedback or trajectory logic."
  - "Prepare the tracker to consume the analyzer's diagnostics (next lesson)."
tags:
  - capstone
  - resolved-rate
  - velocity-layer
...
`[Visual: the tracker as a black box — commanded twist ξ_d in on the left, joint-rate stream q̇ out on the right, with the internal loop (resolve → integrate → recompute J) shown inside, and the tool tracing the commanded velocity]`
