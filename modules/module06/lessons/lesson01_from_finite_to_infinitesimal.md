...
unit: 01
lesson: 1.1
title: "From Finite to Infinitesimal: Differential Translation and Rotation"
core_idea: "A small motion of a frame splits into a differential translation and a differential rotation; to first order, differential rotations commute and add like vectors — and that linearization is exactly what makes the Jacobian possible."
estimated_time: "35 min"
difficulty: "Intermediate"
prerequisites:
  - "M2 — Forward kinematics and rigid-body transforms"
  - "M5 — Numerical IK and the local linear map Δp ≈ J Δθ (D-054)"
  - "Linear algebra: matrices, cross product"
learning_objectives:
  - "Decompose a small rigid-body motion into a differential translation dp and a differential rotation δθ."
  - "Write a differential rotation as R ≈ I + S(δθ) using the skew-symmetric operator."
  - "Explain why differential rotations commute and add like vectors, while finite rotations do not."
  - "Recognize M5's Δp ≈ J Δθ as a statement about differential motion."
tags:
  - differential-motion
  - twists
  - skew-symmetric
  - rotations
  - jacobian-foundations
---
...
`[Visual: a coordinate frame nudged by a small translation dp and a small rotation δθ, with the displaced frame drawn dashed alongside the original]`
**Diagram Specification**

- Draw an original right-handed frame $\{O\}$ with solid axes $x, y, z$ at the
  left-center of the canvas.
- Draw a displaced frame $\{O'\}$ with **dashed** axes, offset by a short
  translation arrow labeled $d\mathbf{p}$ and rotated by a small angle.
- Show a curved arrow about an axis $\hat{\mathbf{k}}$ labeled
  $\delta\boldsymbol{\theta} = \delta\phi\,\hat{\mathbf{k}}$.
- Annotate: "small motion = $d\mathbf{p}$ (straight) + $\delta\boldsymbol{\theta}$
  (twist)".
- Caption: "A differential motion is a small translation plus a small rotation.
  Because the rotation is tiny, $R \approx I + S(\delta\boldsymbol{\theta})$ and
  order no longer matters."
- Keep it schematic and uncluttered; no numeric tick marks.
