...
unit: 06
lesson: 6.4
title: "Pseudoinverse and Damped Least Squares via the SVD"
core_idea: "Inverting the velocity map means dividing by the singular values, which is why it explodes near a singularity (1/σ_min → ∞); damped least squares replaces each 1/σ with the bounded gain σ/(σ²+λ²), and the SVD shows exactly why this tames the blow-up — re-deriving the M5 damped inverse as a geometric fix."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M6 L6.1–6.3 — SVD, condition number, four subspaces"
  - "M6 L5.2 — Joint-rate blow-up ~ 1/σ_min"
  - "M5 — Damped least squares (used numerically; explained here)"
learning_objectives:
  - "Build the pseudoinverse from the SVD as inversion of the nonzero singular values."
  - "Explain the 1/σ_min blow-up geometrically and why pure inversion is unsafe near singularities."
  - "Derive damped least squares via the SVD: gain σ/(σ²+λ²)."
  - "Reconnect to Module 5's damped inverse, now explained rather than asserted."
tags:
  - pseudoinverse
...
`[Visual: two gain curves vs σ — the pseudoinverse gain 1/σ shooting to infinity as σ→0, and the damped gain σ/(σ²+λ²) rising then peaking at 1/(2λ) and falling back to zero — with the small-σ region shaded "danger / damped"]`
