...
unit: 01
lesson: 1.3
title: "The Twist: Linear and Angular Velocity Together"
core_idea: "Stacking linear velocity v and angular velocity ω into a single 6-vector ξ = [v; ω] captures the complete instantaneous motion of a rigid body; every point's velocity follows from v_P = v + ω × r."
estimated_time: "35 min"
difficulty: "Intermediate"
prerequisites:
  - "M6 L1.1 — Differential motion"
  - "M6 L1.2 — Angular velocity and Ṙ = S(ω)R"
  - "Vector cross product"
learning_objectives:
  - "Assemble the twist ξ = [v; ω] in the locked D-057 ordering (linear on top)."
  - "Compute the velocity of any point of a rigid body via v_P = v + ω × r."
  - "Explain why six numbers fully specify instantaneous rigid-body motion."
  - "Distinguish the velocity of a chosen reference point from the body's angular velocity."
tags:
  - twist
  - rigid-body-velocity
  - velocity-kinematics
...
`[Visual: a rigid body with reference point O moving at v and rotating at ω, showing a second point P whose velocity is v + ω × r]`
**Diagram Specification**

- Draw a rigid body (an L-shape or bracket) with a marked reference point $O$.
- Arrow $\mathbf{v}$ from $O$ (linear velocity); curved arrow $\boldsymbol{\omega}$
  about $O$ (angular velocity).
- Mark a second point $P$, draw $\mathbf{r}$ from $O$ to $P$, and draw $\mathbf{v}_P$
  as the sum of a copy of $\mathbf{v}$ and the term $\boldsymbol{\omega}\times\mathbf{r}$.
- Annotate the twist stack $\boldsymbol{\xi} = [\mathbf{v};\,\boldsymbol{\omega}]$.
- Caption: "One twist (6 numbers) sets the velocity of every point:
