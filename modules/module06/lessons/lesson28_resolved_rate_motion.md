...
unit: 07
lesson: 7.4
title: "Resolved-Rate Motion: The Open-Loop Velocity Layer"
core_idea: "Resolved-rate motion is the kinematic velocity layer: at each instant take a commanded tool twist, resolve it into joint rates with the (damped) inverse, and integrate those rates forward — an open-loop scheme that produces exactly the joint-velocity stream Module 7's trajectories and Module 8's controllers will build upon."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M6 L7.1–7.3 — Inverse velocity kinematics, pseudoinverse, damped inverse"
  - "M6 L2.1 — ξ = J q̇"
  - "Basic numerical integration (Euler step)"
learning_objectives:
  - "Describe resolved-rate motion as the open-loop kinematic velocity layer."
  - "Map a commanded tool twist to joint rates and integrate forward."
  - "Distinguish this velocity layer from trajectory generation (M7) and feedback control (M8)."
  - "Recognize open-loop integration drift and where it is (and isn't) addressed."
tags:
  - resolved-rate
  - velocity-layer
  - open-loop
...
`[Visual: a loop — commanded tool twist ξ_d → resolve via (damped) inverse to q̇ → integrate q ← q + q̇ Δt → recompute J at new pose → back to resolve; with the tool tracing the commanded direction]`
