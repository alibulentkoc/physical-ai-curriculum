...
unit: 02
lesson: 2.4
title: "Numerical Validation: Geometric J vs Finite Differences"
core_idea: "Trust no Jacobian you haven't checked: nudge each joint a little, watch how the tool actually moves, divide by the nudge, and confirm it matches the analytic column — the verification habit that anchors the whole module."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M6 L2.2–2.3 — The geometric Jacobian (revolute, prismatic, mixed)"
  - "M6 L1.2 — Recovering ω from a rotation difference (vee)"
  - "M6 L1.1 — Central-difference / second-order error scaling"
learning_objectives:
  - "Build a finite-difference Jacobian by perturbing each joint and measuring tool motion."
  - "Handle the angular part correctly using a rotation difference and the vee operator."
  - "Confirm the analytic geometric Jacobian against finite differences within tolerance."
  - "Verify the expected second-order convergence and adopt validation as standard practice."
tags:
...
`[Visual: left — an arm at q−ε, q, q+ε with the measured tool displacement Δpose that forms a Jacobian column; right — a log–log plot of error vs step size with slope ≈ 2]`
