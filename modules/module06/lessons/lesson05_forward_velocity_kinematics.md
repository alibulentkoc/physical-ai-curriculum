...
unit: 02
lesson: 2.1
title: "Forward Velocity Kinematics: Defining the Jacobian"
core_idea: "Move the joints and the tool moves; the Jacobian J(q) is the bookkeeping that says how much each joint's rate contributes to the tool's twist, so that ξ = J(q) q̇ — the same matrix M5 used inside its solver, now seen as the velocity map of a moving arm."
estimated_time: "35 min"
difficulty: "Intermediate"
prerequisites:
  - "M6 L1.3 — The twist ξ = [v; ω]"
  - "M2 — Forward kinematics: tool pose from joint values"
  - "M5 — The local linear map Δp ≈ J Δθ (D-054)"
learning_objectives:
  - "See forward velocity kinematics as 'joint rates in, tool twist out' on a moving arm."
  - "Read ξ = J(q) q̇ as the sum of per-joint contributions."
  - "Explain why J changes with the arm's pose but acts linearly on the joint rates."
  - "Recognize this J as M5's solver Jacobian, now the velocity map."
tags:
  - jacobian
...
`[Visual: a three-panel sequence of a planar arm — joint 1 alone produces a tool velocity, joint 2 alone produces another, and both together produce the summed tool twist]`
