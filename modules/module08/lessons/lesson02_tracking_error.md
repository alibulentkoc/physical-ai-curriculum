...
unit: 01
lesson: 1.2
title: "Tracking Error: The Quantity Control Fights"
core_idea: "Tracking error e = q_d − q is the single number that captures the reference-vs-reality gap. It is the input every feedback controller acts on: measure where you are, subtract from where you should be, and correct in proportion to the difference. Everything in control is a strategy for driving error to zero and keeping it there."
estimated_time: "35 min"
difficulty: "Beginner"
prerequisites:
  - "M8 L1.1 — Why open-loop isn't enough"
learning_objectives:
  - "Define tracking error e = q_d − q and explain its sign and meaning."
  - "Explain why error, not position, is the controller's input."
  - "Distinguish transient error, steady-state error, and the goal of driving error to zero."
---
...
`[Visual: top panel shows q_d(t) and q(t) converging; bottom panel shows the error signal e(t) = q_d − q starting large, crossing zero (overshoot flips its sign), and settling — with transient region and steady-state region labeled]`

**Diagram Specification**

- **Objective:** show error as its own signal derived from the gap, with transient vs steady-state regions and the sign-flip at overshoot.
- **Scene:** **Top:** $q_d(t)$ (teal) and $q(t)$ (sky) starting apart and converging. **Bottom:** the error $e(t) = q_d - q$ (violet) starting at a large positive value, decaying, dipping below zero briefly (overshoot → sign flip), then settling near zero. Shade/label the "transient" region (early) and "steady-state" region (late). Mark "e = 0" as the goal line; if a constant offset remains, show $e_{ss}$.
- **Labels:** "$e = q_d - q$", "sign flips at overshoot", "transient", "steady-state", "goal: e → 0".
- **Form:** SVG, stacked plots. $q_d$ teal `#0d9488`, $q$ sky `#0ea5e9`, error violet `#8b5cf6`, zero-line muted.
