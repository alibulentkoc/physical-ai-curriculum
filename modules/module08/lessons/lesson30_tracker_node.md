...
unit: 08
lesson: 8.2
title: "A Lightweight ROS 2 Tracker Node"
core_idea: "Zoom in on the one node that does the control: the tracker node. It subscribes to the reference and the measured state, runs the feedforward + PID law through the actuator once per cycle, and publishes one actuator command. That's the whole node — a thin wrapper around the control law you already built, with two subscriptions and one publication. Seen this way, 'writing a ROS 2 controller' is mostly plumbing: the control intelligence is the law from Units 1–5; the node just connects it to topics and runs it at a fixed rate. Lightweight and conceptual — no middleware internals."
estimated_time: "50 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L8.1 — The closed-loop control stack in ROS 2"
  - "M8 L6.2 — Publish and subscribe"
learning_objectives:
  - "Describe the tracker node: two subscriptions, one publication, one control law per cycle."
  - "Separate the control law (the intelligence) from the node (the plumbing)."
  - "Show the node publishes one command per state and closes the loop."
  - "Keep ROS 2 lightweight: the node as a thin wrapper around the control law."
---
...
`[Visual: a single 'tracker node' box with two inbound topic arrows on the left — 'reference (q_d, q̇_d, q̈_d)' and 'joint/state (q, q̇)' — and one outbound topic arrow on the right — 'joint/command (u)'. Inside the box, a compact pipeline: 'feedforward + PID → actuator', tagged 'the control law (Units 1–5) = the intelligence'. Around the box, a thin dashed frame tagged 'the node = plumbing: subscribe · compute once per cycle · publish'. A small counter shows 'N states in → N commands out']`

**Diagram Specification**

- **Objective:** show the tracker node as a thin wrapper — two subscriptions, one publication — around the control law.
- **Scene:** a central rounded box "tracker node". Two inbound arrows on the left, each a topic pill: "reference $q_d,\dot q_d,\ddot q_d$" and "joint/state $(q,\dot q)$". One outbound arrow on the right: topic pill "joint/command $(u)$". Inside the box, a compact horizontal pipeline "feedforward + PID → actuator" with a tag beneath "control law (Units 1–5) = intelligence". A thin dashed frame around the whole box tagged "node = plumbing: subscribe · compute once/cycle · publish". Small badge near the output: "N states in → N commands out".
- **Labels:** "tracker node", "reference", "joint/state", "joint/command", "feedforward + PID → actuator", "intelligence vs plumbing", "one command per state".
- **Form:** SVG, one node box with 2-in/1-out topic pills + an inner control-law pipeline. Node ink-outlined on `#f8fafc`; inbound pills sky `#0ea5e9`, outbound pill teal `#0d9488`; inner pipeline emerald `#10b981`; the dashed plumbing frame muted `#64748b`.
