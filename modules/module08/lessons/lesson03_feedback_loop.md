...
unit: 01
lesson: 1.3
title: "The Feedback Loop: Sense → Compare → Correct → Actuate"
core_idea: "Feedback control is a loop that runs continuously: sense the actual state, compare it to the reference to get the error, compute a correction from that error, and actuate. The loop closes the open-loop gap by measuring and reacting every cycle. Closing the loop is the structural fix open-loop lacked."
estimated_time: "40 min"
difficulty: "Beginner"
prerequisites:
  - "M8 L1.2 — Tracking error"
learning_objectives:
  - "State the four stages of the feedback loop: sense, compare, correct, actuate."
  - "Explain how closing the loop rejects disturbances open-loop cannot."
  - "Describe why the loop runs continuously at a fixed rate."
---
...
`[Visual: the feedback loop as a block diagram — reference q_d into a summing junction (compare) that subtracts measured q to form error e, error into the controller (correct) producing command u, u into the plant (actuate) producing q, and q fed back to the summing junction; the four stages labeled sense/compare/correct/actuate]`

**Diagram Specification**

- **Objective:** show the closed loop as a block diagram with the four stages and the feedback path that makes it "closed."
- **Scene:** left-to-right blocks: reference $q_d$ → **summing junction** (⊕ with a − on the feedback input) producing **error $e$** → **controller $C$** ("correct") producing **command $u$** → **plant** ("actuate") producing **$q$**. A **feedback path** carries $q$ from the plant output back down to the summing junction's negative input (labeled "sense" with a sensor icon). Stage labels: sense (feedback path), compare (summing junction), correct (controller), actuate (plant). A disturbance arrow enters the plant.
- **Labels:** "$q_d$ reference (from M7)", "$e = q_d - q$", "controller $u = C(e)$", "plant", "measured $q$", "disturbance", the four stage names; "closed loop: $q$ feeds back".
- **Form:** SVG, standard control block diagram. Reference teal `#0d9488`, error/controller violet `#8b5cf6`, plant slate, feedback path emerald `#10b981`, disturbance rose.
