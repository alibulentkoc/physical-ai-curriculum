...
unit: 06
lesson: 6.4
title: "A Data-Flow Architecture Layered by Timing"
core_idea: "Assemble Unit 6 into the robot's data-flow architecture and organise it by timing. Different subsystems run at different rates and tolerate different delays: a fast inner control loop that must be tight and low-latency — the layer most sensitive to the instability of 6.3 — versus slower outer layers (planning, coordination, supervision) that can tolerate more latency. This inner-loop / outer-layer split is the architecture's backbone: it pinpoints where real-time guarantees matter (the inner loop → Unit 7) and frames the full ROS 2 control stack (Unit 8). It closes Installment C and bridges to the rest of the module."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L6.2 — Publish–subscribe (the topic graph)"
  - "M8 L6.3 — Latency and control rate (loop delay and instability)"
learning_objectives:
  - "Describe the robot as a layered data-flow architecture."
  - "Explain the inner-loop / outer-layer split by timing requirements (rate and latency tolerance)."
  - "Place each subsystem (sensing, control, planning, supervision) in the layered picture."
  - "Explain how the timing split motivates real-time execution (Unit 7) and the ROS 2 stack (Unit 8)."
---
...
`[Visual: a layered data-flow diagram — an INNER LOOP at the bottom (sensor → controller → actuator → joint) tagged 'fast, low-latency, latency-CRITICAL (Unit 7 real-time)', and OUTER LAYERS stacked above (planning/trajectory from Module 7, coordination, supervision) tagged 'slow, latency-TOLERANT'; a thin arrow from the outer layers hands a setpoint down into the inner loop; two callouts show 'latency here → unstable' on the inner loop and 'latency here → tolerable' on the outer handoff; the whole graph is captioned 'one ROS 2 stack (Unit 8) hosts all layers']`

**Diagram Specification**

- **Objective:** show the robot's data flow layered by timing, with the inner loop latency-critical and outer layers latency-tolerant.
- **Scene:** a vertical stack. **Bottom band — Inner loop:** Sensor → Controller → Actuator → Joint drawn as a tight cycle, tagged "fast · low-latency · latency-CRITICAL"; a red callout "delay here → unstable (Lesson 6.3)". **Upper bands — Outer layers:** "Planning / Trajectory (Module 7)", "Coordination", "Supervision", each tagged with a slower rate; a thin downward arrow labelled "setpoint / reference (updated slowly)" feeds the inner loop, with a green callout "delay here → tolerable". A side note: "→ inner loop needs real-time (Unit 7)"; a base caption: "one ROS 2 control stack (Unit 8) hosts every layer".
- **Labels:** "inner loop (fast, latency-critical)", "outer layers (slow, latency-tolerant)", "setpoint handed down", "delay here → unstable", "delay here → tolerable", "→ Unit 7 real-time", "→ Unit 8 ROS 2 stack".
- **Form:** SVG, layered bands with a downward setpoint arrow. Inner-loop band teal `#0d9488` with a red `#e11d48` "unstable" callout; outer bands soft `#f1f5f9` with ink `#0f172a` labels and an emerald `#10b981` "tolerable" callout; Unit-7/Unit-8 tags sky `#0ea5e9`; rates in muted `#64748b`.
