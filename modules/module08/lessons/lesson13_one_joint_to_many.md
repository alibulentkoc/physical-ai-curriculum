...
unit: 04
lesson: 4.1
title: "From One Joint to Many: Tracking a Trajectory with Feedback"
core_idea: "A real arm has several joints, and Module 7 hands each one a whole moving reference, not a single setpoint. The simplest extension is one independent PID per joint, each chasing its joint's q_d(t). It works — but watch closely during fast motion and you'll see every joint lag slightly behind its reference, because feedback only reacts after an error has already opened up. That visible lag is the problem the rest of Unit 4 solves."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L3.4 — Tuning a controller"
  - "M7 — the reference trajectory layer (q_d, q̇_d, q̈_d)"
learning_objectives:
  - "Extend single-joint PID to per-joint control of a multi-joint arm tracking a trajectory."
  - "Distinguish tracking a moving reference from regulating to a fixed setpoint."
  - "Observe and explain feedback lag during fast motion."
---
...
`[Visual: a two-joint arm tracking a fast reference with feedback only — for each joint, the reference q_d(t) and the actual q(t) overlaid, the actual trailing slightly behind during the fast middle of the move (a shaded following-error gap that swells where the reference moves fastest), plus a small inset of the tool tip tracing just inside the planned path]`

**Diagram Specification**

- **Objective:** show feedback-only tracking working but lagging, with the lag largest where the reference moves fastest.
- **Scene:** two stacked position-vs-time plots (joint 1, joint 2). In each, the **reference $q_d(t)$** (smooth quintic-style S-curve) and the **actual $q(t)$** trailing slightly behind, with the gap shaded and clearly widest through the steep middle section (peak speed) and closing at the ends. A small inset (top-right): the planned tool path (dashed) and the actual path tracing just *inside* it.
- **Labels:** "joint 1 / joint 2", "reference $q_d(t)$", "actual $q(t)$ — feedback only", "following error (largest at peak speed)", inset "planned vs actual tool path".
- **Form:** SVG, two real engine tracking plots + tip-path inset. Reference teal `#0d9488`, actual sky `#0ea5e9`, gap shaded rose `#e11d48` faint.
