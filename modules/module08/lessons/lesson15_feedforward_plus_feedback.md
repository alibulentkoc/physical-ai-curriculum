...
unit: 04
lesson: 4.3
title: "Feedforward + Feedback Together"
core_idea: "Feedforward anticipates the known motion; feedback corrects the unknown residual. Run them together and you get the best of both — the feedforward command does the bulk of the work from Module 7's plan, and a light feedback loop mops up the model mismatch and disturbance it can't predict. On the identical trajectory, feedback-only leaves a visible following error while feedforward+feedback tracks several times tighter. This combination is the controller Module 8 was built to produce."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L4.2 — Feedback reacts, feedforward anticipates"
learning_objectives:
  - "Combine feedforward and feedback into one command and explain each part's role."
  - "Compare feedback-only versus feedforward+feedback on an identical trajectory and quantify the improvement."
  - "Explain why the combination is more robust than either part alone."
---
...
`[Visual: the identical fast reference tracked two ways, overlaid — FEEDBACK ONLY (actual trailing with a visible following-error gap) vs FEEDFORWARD+FEEDBACK (actual sitting on the reference, gap nearly gone) — with a side bar chart of tracking RMS showing the combined controller several times lower, and a small command breakdown showing u = u_ff (large, from the plan) + u_fb (small, the residual correction)]`

**Diagram Specification**

- **Objective:** make the measurable improvement undeniable and show the labour split (large feedforward, small feedback).
- **Scene:** a position-vs-time plot of one reference $q_d(t)$ with two actual curves: **feedback only** (trailing, shaded following-error gap) and **feedforward+feedback** (on top of the reference, negligible gap). Beside it, a small **bar chart** "tracking RMS": tall bar "feedback only ≈ 0.075", short bar "ff + fb ≈ 0.017", annotated "~4.5× tighter". A small stacked-command inset: a large block "$u_{ff}$ (from the plan)" plus a thin block "$u_{fb}$ (residual)".
- **Labels:** "reference $q_d$", "feedback only (lags)", "feedforward + feedback (tracks)", "tracking RMS", "~4.5× tighter", "$u = u_{ff} + u_{fb}$".
- **Form:** SVG, one tracking plot + RMS bar chart + command-split inset. Feedback-only rose `#e11d48`, combined emerald `#10b981`, reference teal `#0d9488`.
