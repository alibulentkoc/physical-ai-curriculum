...
unit: 08
lesson: 8.3
title: "The Control Layer: the Module 9 Handoff"
core_idea: "Package the whole module as one clean interface: the control layer. It is tracking_controller(reference, measured_state) → actuator_command — feed it a Module 7 reference sample and the measured joint state, and it returns the actuator command that drives the joint toward the reference, with feedforward anticipating, feedback correcting, and the actuator delivering. This is the Module 9 handoff, the third link in the chain velocity_layer (M6) → reference layer (M7) → control layer (M8) → integrated system (M9). Module 9 integrates this layer into a full autonomous system; everything beyond this interface is its job, not ours."
estimated_time: "50 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L8.2 — A lightweight ROS 2 tracker node"
  - "M8 L4.4 — The complete joint tracker"
  - "M7 — The reference trajectory layer"
learning_objectives:
  - "State the control-layer interface tracking_controller(reference, measured_state) → actuator_command."
  - "Identify it as the Module 9 handoff in the M6→M7→M8→M9 chain."
  - "Explain what is inside the layer and what is deliberately outside it."
  - "Verify the layer's interface and that it tracks closed-loop."
---
...
`[Visual: the M6→M7→M8→M9 chain as four linked layer-blocks left to right: 'velocity layer (M6)', 'reference layer (M7)', 'control layer (M8)' (highlighted), 'integrated system (M9)' (drawn faint/dashed as 'next'). The highlighted control-layer block is opened to show its interface — two inputs 'reference (q_d, q̇_d, q̈_d)' and 'measured_state (q, q̇)', one output 'actuator_command' — and inside it 'feedforward + feedback + actuator'. A fence line around the block reads 'inside: M8 — feedforward, feedback, actuator'; outside the block, faint items 'plan the world · coordinate · supervise' are labelled 'Module 9 (not here)']`

**Diagram Specification**

- **Objective:** show the control layer as the M9 handoff, opened to its interface, with the chain and the boundary.
- **Scene:** four layer-blocks left→right linked by arrows: "velocity layer (M6)" → "reference layer (M7)" → **"control layer (M8)"** (highlighted, larger) → "integrated system (M9)" (faint/dashed, tagged "next"). The highlighted M8 block is opened: two input ports on its left "reference $(q_d,\dot q_d,\ddot q_d)$" and "measured_state $(q,\dot q)$", one output port on its right "actuator_command"; inside, a compact pipeline "feedforward + feedback → actuator". A subtle fence around the M8 block: inside-tag "M8: feedforward · feedback · actuator"; just outside, faint items "plan the world · coordinate subsystems · supervise" with a tag "Module 9 — beyond the control layer (not here)".
- **Labels:** "velocity layer (M6)", "reference layer (M7)", "control layer (M8)", "integrated system (M9)", "reference", "measured_state", "actuator_command", "inside: feedforward + feedback + actuator", "Module 9 (not here)".
- **Form:** SVG, a four-block chain with the M8 block opened to ports + an inside/outside fence. Chain blocks ink-outlined; M8 highlighted teal `#0d9488`; M9 faint dashed muted `#64748b`; input ports sky `#0ea5e9`, output port emerald `#10b981`; the "Module 9 / not here" items faint with an amber `#d97706` boundary line.
