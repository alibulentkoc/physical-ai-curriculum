...
unit: 03
lesson: 3.2
title: "The Shape of a Response: Rise, Overshoot, and Settling"
core_idea: "Now that you can recognise good and bad responses by eye, we give the shape names and numbers: rise time (how fast it gets there), overshoot (how far it sails past), settling time (when it stops wiggling), and steady-state error (how far off it ends). These four numbers turn 'looks about right' into a measurable specification you can tune toward."
estimated_time: "40 min"
difficulty: "Beginner"
prerequisites:
  - "M8 L3.1 — When correction goes wrong"
learning_objectives:
  - "Define and read rise time, overshoot, settling time, and steady-state error off a response."
  - "Relate each metric to the underlying behaviour (speed, aggressiveness, damping, offset)."
  - "Use the four metrics as a tuning specification rather than a subjective impression."
---
...
`[Visual: a single step response annotated with all four metrics — the 10–90% rise time bracket on the climb, the overshoot peak above the target with a labelled height, the ±2% settling band with the settling-time mark where the curve last enters it, and the steady-state error gap at the right edge]`

**Diagram Specification**

- **Objective:** put all four response metrics on one annotated curve so each name attaches to a feature of the shape.
- **Scene:** one well-formed step response (rises, small overshoot, settles) with a dashed **target line**. Annotations: a **10%→90% bracket** on the rising edge labelled "rise time $t_r$"; the **peak** above target with a vertical callout "overshoot %"; a shaded **±2% band** around the target with a vertical mark where the curve last enters it labelled "settling time $t_s$"; at the far right, a small gap between the curve and target labelled "steady-state error $e_{ss}$" (drawn near zero for a good response, with a note "→0 with integral").
- **Labels:** "target $q_d$", "rise time $t_r$ (10–90%)", "overshoot %", "settling time $t_s$ (±2% band)", "steady-state error $e_{ss}$".
- **Form:** SVG, one response curve + target + the four annotations. Curve teal `#0d9488`, overshoot callout amber `#d97706`, settling band light sky, error gap rose `#e11d48`.
