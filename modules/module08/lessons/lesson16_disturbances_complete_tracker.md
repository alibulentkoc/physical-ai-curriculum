...
unit: 04
lesson: 4.4
title: "Disturbances, Load, and the Complete Tracker"
core_idea: "Feedforward anticipates the plan, but the plan never mentions the unexpected — a bumped arm, a heavier-than-modelled payload, friction that changed overnight. Those are disturbances, and feedforward is blind to them because they aren't in the reference. Feedback is exactly what catches them. The complete joint tracker is feedforward for the known plus feedback for the unknown — and this lesson proves feedback is non-negotiable by showing feedforward alone cannot reject a disturbance while the combination does."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L4.3 — Feedforward + feedback together"
learning_objectives:
  - "Explain why feedforward cannot reject disturbances and feedback can."
  - "Assemble the complete joint tracking controller (feedforward + feedback) and state its interface."
  - "Summarise Unit 4 and the Module 7→8→9 continuity."
---
...
`[Visual: the same trajectory hit by a disturbance kick partway through — FEEDFORWARD ONLY: at the kick the actual jumps off the reference and never recovers (a persistent error to the end); FEEDFORWARD+FEEDBACK: at the kick the actual dips then is pulled right back onto the reference — with the feedback command spiking exactly at the disturbance to reject it]`

**Diagram Specification**

- **Objective:** prove feedback is essential — show feedforward-only failing to reject a disturbance and feedforward+feedback rejecting it.
- **Scene:** a position-vs-time plot of one reference $q_d(t)$ with a marked **disturbance kick** at some time (a small arrow/label "disturbance"). Two actual curves: **feedforward only** — tracks until the kick, then steps off the reference and stays off (persistent gap to the end, shaded); **feedforward + feedback** — dips at the kick then returns onto the reference (gap closes). Below, the **feedback command** trace flat until the kick then spiking to reject it, labelled "feedback reacts to the disturbance".
- **Labels:** "reference $q_d$", "disturbance kick", "feedforward only — can't recover", "ff + feedback — rejects it", "feedback command spikes at disturbance".
- **Form:** SVG, one tracking plot with kick + the two actual curves + feedback-command trace. Feedforward-only rose `#e11d48`, combined emerald `#10b981`, reference teal `#0d9488`, disturbance marker amber `#d97706`.
