...
unit: 05
lesson: 5.1
title: "The Actuator: From Requested Command to Delivered Effort"
core_idea: "Every lesson so far assumed the controller's command u reached the joint exactly. It doesn't. Between the controller and the joint sits an actuator — a device that converts a *requested* command into a *delivered* physical effort, and the two are not equal. The actuator can't deliver unlimited effort (saturation), can't deliver tiny efforts (deadband), and can't change instantly (rate limit). This lesson opens that box and replaces the 'u reaches the joint' fiction with the real request-to-delivery picture — at the plant level, no motor internals."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L4.4 — The complete joint tracker (Units 1–4)"
  - "M8 L2.2 — Integral control (for the windup link in 5.2)"
learning_objectives:
  - "Describe the actuator as a request→delivery converter sitting between controller and plant."
  - "Identify saturation, deadband, and rate limit by their effect on delivered effort."
  - "Read a requested-vs-delivered transfer characteristic."
  - "Explain why the 'command reaches the joint' assumption of Units 1–4 breaks on real actuators."
---
...
`[Visual: the actuator transfer characteristic — a requested→delivered curve that is flat at zero across the central deadband, then rises linearly, then flattens at ±u_max; an example request is shown mapping down to three different delivered values for ideal / deadband / saturated actuators; a small inset shows a rate-limited step ramping rather than jumping]`

**Diagram Specification**

- **Objective:** show that the actuator converts a requested command into a different delivered effort, with deadband, saturation, and rate limit visible at a glance.
- **Scene:** a central plot with requested command on the x-axis and delivered effort on the y-axis. The transfer curve is flat at $0$ across the deadband $[-d, d]$, rises with unit slope outside it, and saturates flat at $\pm u_{\max}$. Mark one requested value with a vertical dashed line and show it mapping to three deliveries (ideal on the diagonal, deadband at $0$, saturated at the ceiling). A small inset: a requested step (square) vs the delivered effort ramping toward it under a rate limit.
- **Labels:** "requested command", "delivered effort", "deadband (delivers nothing)", "saturation ceiling $\pm u_{\max}$", "rate limit: effort ramps, doesn't jump".
- **Form:** SVG, one transfer plot + rate-limit inset. Diagonal/ideal teal `#0d9488`, delivered curve ink `#0f172a`, deadband region soft `#f1f5f9`, ceiling lines amber `#d97706`, rate-limit ramp violet `#8b5cf6`.
