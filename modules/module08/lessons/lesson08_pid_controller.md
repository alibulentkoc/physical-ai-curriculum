...
unit: 02
lesson: 2.4
title: "The PID Controller: The Complete Single-Joint Tracker"
core_idea: "PID combines all three actions into one law: u = Kp·e + Ki·∫e + Kd·ė. Proportional pushes toward the target, integral erases steady-state offset, derivative damps overshoot. Together they form the workhorse controller that tracks a reference precisely and cleanly on a single joint — the complete building block the rest of the module deploys and scales."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L2.1–2.3 — Proportional, integral, and derivative control"
learning_objectives:
  - "Assemble the full PID law and state each term's role."
  - "Explain how the three terms interact and trade off in a complete controller."
  - "Apply PID to track a single-joint reference; recap Unit 2."
---
...
`[Visual: the PID controller as three parallel blocks (P, I, D) each fed the error and summed into one command, beside a step response showing the progression P-only (offset) → PI (offset gone, overshoot) → PID (clean, precise) converging onto the target]`

**Diagram Specification**

- **Objective:** show PID as the sum of three parallel actions and the response progression P → PI → PID.
- **Scene:** **Left:** error $e$ fanning into three parallel blocks — **$K_p e$** (proportional), **$K_i \int e$** (integral), **$K_d \dot e$** (derivative) — summed (⊕) into one command $u$ to the plant. **Right:** overlaid step responses on a target line: **P-only** (settles below — offset), **PI** (reaches target but overshoots), **PID** (reaches target, clean, minimal overshoot). Each curve labeled; the PID curve highlighted as the goal.
- **Labels:** "$u = K_p e + K_i\int e + K_d\dot e$", the three term roles ("push / erase offset / damp"), "P → PI → PID".
- **Form:** SVG, parallel-block controller + step-response comparison. P muted, I sky `#0ea5e9`, D violet `#8b5cf6`, PID result teal `#0d9488`, target dashed.
