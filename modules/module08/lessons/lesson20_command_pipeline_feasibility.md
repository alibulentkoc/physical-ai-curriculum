...
unit: 05
lesson: 5.4
title: "The Command Pipeline and the Feasibility Envelope"
core_idea: "Assemble Unit 5 into one picture: reference → controller → requested command → actuator (deadband, saturation, rate limit) → delivered effort → motion. The actuator's limits define a feasibility envelope — the effort and rate the joint can actually deliver. A trajectory whose demanded effort or rate exceeds the envelope cannot be tracked, no matter how good the controller. This closes the loop back to Module 7: the envelope is the hardware constraint behind a trajectory's feasibility, complementing M7's geometric and velocity limits."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L5.2 — Saturation and windup"
  - "M8 L5.3 — Deadband, stiction, and integral action"
  - "M7 — Trajectory feasibility (velocity/acceleration limits)"
learning_objectives:
  - "Trace the full command pipeline from reference to delivered motion."
  - "Define the feasibility envelope as the actuator's effort and rate limits."
  - "Connect the envelope to Module 7's trajectory feasibility."
  - "Demonstrate that a within-envelope trajectory tracks cleanly while an over-aggressive one violates the bounds."
---
...
`[Visual: a left-to-right pipeline — reference → controller → requested command → actuator (deadband/saturation/rate limit) → delivered effort → joint motion — with the actuator box highlighted; to the right, a feasibility-envelope box (effort vs rate) containing a gentle trajectory's demand curve safely inside, and an aggressive trajectory's demand curve spilling outside the box where it is clipped, with the resulting tracking gap shown on a small position plot]`

**Diagram Specification**

- **Objective:** show the full command pipeline and the feasibility envelope deciding trackability.
- **Scene:** top — a horizontal pipeline of labelled boxes: **reference** → **controller** → **requested command** → **actuator (deadband · saturation · rate limit)** → **delivered effort** → **joint motion**, with the actuator box visually emphasised (it is where the baton can only shrink). Bottom-left — a "feasibility envelope" rectangle on effort (y) vs rate-of-change (x) axes; a **gentle** demand curve sits comfortably inside (green), an **aggressive** demand curve pushes outside the top/side (red), with the out-of-bounds part marked "clipped/slewed — unavailable". Bottom-right — a small position-vs-time inset: gentle tracks the reference; aggressive drifts off it (shaded gap).
- **Labels:** "command pipeline", "actuator re-shapes the request", "feasibility envelope ($u_{\max}$, $r_{\max}$)", "within envelope → tracked", "beyond envelope → clipped, missed", "ties back to Module 7 feasibility".
- **Form:** SVG, a pipeline row + envelope plot + tracking inset. Pipeline boxes ink `#0f172a` on soft `#f8fafc`, actuator box accent teal `#0d9488`, envelope box outline ink with amber `#d97706` bounds, gentle curve emerald `#10b981`, aggressive curve red `#e11d48`.
