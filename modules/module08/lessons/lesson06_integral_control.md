...
unit: 02
lesson: 2.2
title: "Integral Control: Erasing Steady-State Error"
core_idea: "Integral control accumulates error over time and adds a command proportional to that accumulation: u_I = Ki·∫e. Because the accumulation keeps growing as long as any error remains, it drives the steady-state offset to zero — the gap proportional control couldn't close. Its cost is sluggishness and windup, which must be managed."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L2.1 — Proportional control and its steady-state offset"
learning_objectives:
  - "State the integral control law u_I = Ki·∫e and explain why it erases steady-state error."
  - "Explain the cost of integral action: added lag and overshoot."
  - "Describe integral windup and how anti-windup limiting addresses it."
---
...
`[Visual: two stacked panels. Top: a P-only step response settling short of target, vs a PI response that climbs and reaches the target (offset erased), the PI slightly overshooting. Bottom: the integral accumulator ∫e growing while error persists and leveling off once error hits zero; an inset shows windup — the accumulator ballooning during saturation and the resulting big overshoot, vs a clamped accumulator behaving]`

**Diagram Specification**

- **Objective:** show integral action erasing the offset (and its overshoot cost), plus the windup mechanism and the clamp fix.
- **Scene:** **Top:** step response — P-only curve settling below the target line (offset), PI curve reaching the target (offset gone) with slight overshoot. **Bottom-left:** the accumulator $\int e$ rising while error is positive, flattening when error → 0. **Bottom-right (inset):** "windup" — accumulator ballooning during an actuator-saturation episode and the response overshooting badly, beside a "clamped" accumulator that stays bounded and the response behaving.
- **Labels:** "P: offset", "PI: reaches target", "$\int e$ accumulates → holds", "windup (unbounded ∫e → big overshoot)", "clamp / anti-windup".
- **Form:** SVG, stacked panels. P muted, PI teal `#0d9488`, accumulator violet `#8b5cf6`, windup rose `#e11d48`, clamped emerald `#10b981`.
