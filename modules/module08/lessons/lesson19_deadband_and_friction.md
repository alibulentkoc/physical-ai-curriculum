...
unit: 05
lesson: 5.3
title: "Deadband, Stiction, and Why Integral Wins the Last Millimetre"
core_idea: "At the opposite end from saturation, small commands deliver no motion: a deadband swallows tiny efforts and static friction (stiction) holds the joint until the effort exceeds a breakaway threshold. Proportional and derivative action vanish exactly when the error is small, so they leave the joint stuck short of target. Integral action is the cure — it accumulates the small persistent error until the command finally crosses the threshold and nudges the joint home. This is why the last millimetre belongs to the integrator."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L5.1 — The actuator: deadband and delivered effort"
  - "M8 L2.2 — Integral control (erasing steady-state error)"
learning_objectives:
  - "Describe deadband and stiction as the small-effort failure mode of an actuator/joint."
  - "Explain why proportional and derivative action leave a residual error inside the deadband."
  - "Explain how integral accumulation overcomes deadband and stiction to reach the target."
  - "Demonstrate the integral cure on a joint that P/PD leaves stuck short."
---
...
`[Visual: a setpoint move where the actual under PD rises then flattens out short of the dashed target — frozen by deadband/stiction with a clearly labelled residual gap — while the PID actual rises, briefly stalls, then breaks through and lands on the target as its integrator term climbs past the breakaway threshold; an inset shows the small command of PD sitting inside the deadband while PID's accumulated command crosses it]`

**Diagram Specification**

- **Objective:** show P/PD stalling short inside the deadband/stiction and integral action breaking through to the target.
- **Scene:** position vs time tracking a setpoint (dashed target). **PD** curve rises and flattens *below* the target with a labelled residual gap ("stuck short"). **PID** curve rises, shows a brief plateau, then steps up onto the target. An inset bar/track of "delivered command vs deadband threshold": PD's command sits *inside* the deadband (delivers 0); PID's accumulated command rises *past* the threshold (breakaway), with an arrow "integral accumulates → crosses threshold".
- **Labels:** "target", "PD — stuck short (residual ≈ (load+deadband)/Kp)", "PID — integral breaks through", "deadband / breakaway threshold", "integral accumulates".
- **Form:** SVG, one tracking plot + a deadband-threshold inset. PD rose `#e11d48`, PID emerald `#10b981`, target teal `#0d9488`, deadband band soft `#f1f5f9` with amber `#d97706` threshold line, integral arrow violet `#8b5cf6`.
