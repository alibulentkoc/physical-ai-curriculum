...
unit: 01
lesson: 1.4
title: "Anatomy of a Control System: Reference, Plant, Controller, Feedback"
core_idea: "Every feedback control system is built from four named parts: the reference (what we want, from Module 7), the plant (the thing being controlled — the joint and its physics), the controller (the law that turns error into command), and the feedback path (the sensor closing the loop). Naming the parts gives a shared vocabulary and the block diagram every control design starts from."
estimated_time: "35 min"
difficulty: "Beginner"
prerequisites:
  - "M8 L1.3 — The feedback loop"
learning_objectives:
  - "Name and define the four parts of a control system: reference, plant, controller, feedback."
  - "Read and draw the standard control block diagram."
  - "Map each part to the greenhouse arm and to Module 7's handoff; recap Unit 1."
---
...
`[Visual: the canonical control block diagram fully labeled — reference q_d (from Module 7) → summing junction → error e → controller C → command u → plant (joint) → output q, with the feedback sensor path returning q, a disturbance entering the plant, and each block annotated with its greenhouse-arm meaning]`

**Diagram Specification**

- **Objective:** the canonical four-part block diagram, every block named and mapped to the arm, marking the M7/M8 boundary.
- **Scene:** the standard loop. **Reference** block (labeled "$q_d, \dot q_d, \ddot q_d$ — Module 7 reference layer") → **summing junction $\bigoplus$** (− on feedback) → **error $e$** → **controller $C$** ("the part we design — Unit 2") → **command $u$** → **plant** ("the joint: inertia, friction, load") → **output $q$**. **Feedback path** (sensor/encoder) returns $q$. **Disturbance** arrow into the plant. A dashed vertical line separates the "Module 7 (reference)" block from the "Module 8 (controller + loop)" region.
- **Labels:** the four part names; the M7/M8 boundary line; "design freedom = controller"; "sensor closes the loop"; disturbance.
- **Form:** SVG, labeled block diagram. Reference teal `#0d9488`, controller violet `#8b5cf6`, plant slate, feedback emerald `#10b981`, disturbance rose, M7/M8 divider dashed muted.
