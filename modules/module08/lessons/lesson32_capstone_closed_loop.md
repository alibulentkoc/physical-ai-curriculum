...
unit: 08
lesson: 8.4
title: "Capstone: The Greenhouse Arm Tracks Its Trajectory"
core_idea: "The whole module in one run: the planar 2-link greenhouse arm tracks a Module 7 reference through the closed-loop control layer — feedforward anticipating, feedback correcting, the actuator delivering, the loop running as a periodic real-time task in a ROS 2 stack — and feedback rejects a disturbance that feedforward alone cannot. This capstone integrates Units 1–8 into the control layer that Module 9 will consume, and recaps the module's arc from open-loop drift to a real, distributed, actuator-limited, real-time control system. Module 8 is complete; the next step is Module 9, the integrated autonomous system."
estimated_time: "60 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L8.3 — The control layer: the Module 9 handoff"
  - "M8 L8.1 — The closed-loop control stack in ROS 2"
  - "M8 L4.4 — Disturbances, load, and the complete tracker"
learning_objectives:
  - "Run the full closed-loop control stack on the 2-link arm tracking a Module 7 reference."
  - "Show feedback rejects a disturbance feedforward alone cannot."
  - "Recap the module's arc from open-loop drift to a real-time control stack."
  - "Hand the control layer to Module 9 and bridge to the integrated system."
---
...
`[Visual: the 2-link greenhouse arm (L1, L2) reaching toward a fruit, with the closed-loop control stack drawn around it: reference (M7) → control layer (per joint: feedforward + feedback → actuator) → arm → measured state → back to the control layer. Two small trajectory plots: (left) 'no disturbance' — measured curves sit exactly on the reference curves; (right) 'disturbance on joint 0' — both a 'feedforward + feedback' trace (kicked, then returns to the reference, labelled 'rejected') and a 'feedforward-only' trace (kicked, stays off, labelled 'not rejected'). A banner reads 'Module 8 complete → control layer → Module 9']`

**Diagram Specification**

- **Objective:** show the full closed-loop stack on the 2-link arm, tracking and rejecting a disturbance, and mark module completion.
- **Scene:** center, the planar 2-link arm (segments $L_1$, $L_2$) reaching toward a small fruit; around it, the loop: "reference (M7)" → "control layer (feedforward + feedback → actuator)" → "arm" → "measured state" → back to the control layer (closing). Two small plots beneath: **left "no disturbance"** — measured joint curves lie exactly on the dashed reference curves (label "tracks, RMS ≈ 0"); **right "disturbance on joint 0"** — a dashed reference, a "ff + fb" trace that dips at the kick then returns to the reference (emerald, label "rejected, |e|→0"), and a "ff-only" trace that dips and stays off (red, label "not rejected, large |e|"). A bottom banner: "Module 8 complete → control layer → Module 9 (integrated system)".
- **Labels:** "$L_1$, $L_2$", "reference (M7)", "control layer", "feedforward + feedback → actuator", "measured state", "no disturbance: tracks", "disturbance: ff+fb rejects, ff-only can't", "Module 8 → Module 9".
- **Form:** SVG, the arm + closing loop + two comparison plots + completion banner. Arm ink on `#f8fafc`; reference dashed muted `#64748b`; ff+fb trace emerald `#10b981`; ff-only trace red `#e11d48`; control-layer block teal `#0d9488`; the completion banner amber `#d97706`.
