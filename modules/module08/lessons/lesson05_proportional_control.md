...
unit: 02
lesson: 2.1
title: "Proportional Control: Correction Proportional to Error"
core_idea: "The simplest controller commands an effort proportional to the error: u = Kp·e. Push harder when far, gently when close, stop at zero error. It is intuitive and effective, but under a constant load it settles short of the target — leaving a steady-state offset Kp can shrink but never erase. That residual gap is exactly what integral control will fix."
estimated_time: "40 min"
difficulty: "Beginner"
prerequisites:
  - "M8 L1.2 — Tracking error"
  - "M8 L1.4 — Anatomy of a control system"
learning_objectives:
  - "State the proportional control law u = Kp·e and explain its behaviour."
  - "Explain why higher Kp tracks tighter but risks overshoot and oscillation."
  - "Explain why proportional control leaves a steady-state offset under constant load."
---
...
`[Visual: step responses for three proportional gains — low Kp (slow, large offset), medium Kp (good, small offset), high Kp (fast but overshoot/oscillation) — all settling short of the target line by ess = load/Kp, with the offset shrinking as Kp grows]`

**Diagram Specification**

- **Objective:** show the $K_p$ trade-off (speed vs overshoot) and the persistent steady-state offset shrinking but never vanishing with gain.
- **Scene:** a position-vs-time step-response plot with a dashed **target line** at $q_d$. Three response curves: **low $K_p$** (slow rise, settles well below target — large offset), **medium $K_p$** (good rise, small offset), **high $K_p$** (fast, overshoots and rings before settling, smallest offset). Each curve's final value sits *below* the target by a bracket labeled "$e_{ss} = \ell/K_p$". A small caption: "offset shrinks with $K_p$ but never reaches zero".
- **Labels:** "target $q_d$", "low/med/high $K_p$", "overshoot (high $K_p$)", "$e_{ss} = \ell/K_p$ (never 0)".
- **Form:** SVG, three step-response curves + target line + offset brackets. Low gain muted, medium teal `#0d9488`, high gain rose `#e11d48`.
