...
unit: 08
lesson: 8.1
title: "The Closed-Loop Control Stack in ROS 2"
core_idea: "Unit 8 assembles everything into one closed-loop control stack, expressed as a ROS 2 node/topic graph. The reference (Module 7) is published; a controller node — the tracker, carrying the feedforward + PID law and the actuator pipeline — subscribes to the reference and the measured state, computes the command, and publishes it; an actuator node drives the plant; a sensor node publishes the state, closing the loop. This is the publish/subscribe pattern of Lesson 6.2 made into the actual control stack, with the inner loop running on the real-time target of Unit 7. ROS 2 here is conceptual and lightweight: the pub/sub bus stands in for the middleware."
estimated_time: "55 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 L6.2 — Publish and subscribe: nodes, topics, messages"
  - "M8 L7.4 — Running the inner loop on a real-time target"
  - "M8 L4.4 — The complete joint tracker"
learning_objectives:
  - "Express the closed loop as a ROS 2 node/topic graph (sensor, controller, actuator)."
  - "Identify the topics that carry the loop (joint/state, joint/command)."
  - "Show the stack tracking a Module 7 reference end to end."
  - "Place the inner loop on the real-time target within the stack."
---
...
`[Visual: a ROS 2 node/topic graph. Three node boxes — 'sensor node', 'controller node (tracker)', 'actuator node' — connected through two topic pills: sensor → [joint/state] → controller, and controller → [joint/command] → actuator; an arrow from the actuator back to the plant/joint and from the joint back to the sensor closes the loop. The controller node is tagged 'real-time target (Unit 7)' and shows inside it 'feedforward + PID → actuator'. A faint extra subscriber ('logger') hangs off joint/state to show decoupled extensibility. A small label notes 'ROS 2 (conceptual): nodes + topics + messages']`

**Diagram Specification**

- **Objective:** show the closed loop as a ROS 2 node/topic graph with the two loop-carrying topics and the real-time controller node.
- **Scene:** three node boxes left-to-right: **sensor node**, **controller node (tracker)**, **actuator node**. Topic **pills** between them: sensor →`joint/state`→ controller; controller →`joint/command`→ actuator. A return path: actuator → "plant / joint" box → back up to sensor (closing the loop). Inside the controller node, small text "feedforward + PID → actuator"; a tag on it "real-time target (Unit 7)". A faint dashed subscriber "logger" tapping `joint/state` with a tag "decoupled: add consumers freely". Footer label "ROS 2 (conceptual): nodes + topics + messages — bus stands in for the middleware".
- **Labels:** "sensor node", "controller node (tracker)", "actuator node", "joint/state", "joint/command", "plant / joint", "real-time target", "logger (extra subscriber)".
- **Form:** SVG, three node boxes + two topic pills + a closing return path. Nodes ink-outlined on soft `#f8fafc`; topic pills teal `#0d9488`; the controller node's real-time tag amber `#d97706`; the closing loop path emerald `#10b981`; the logger faint muted `#64748b` dashed.
