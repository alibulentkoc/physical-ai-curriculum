...
learning_objectives:
  - Compute the dot product by components and by the angle formula.
  - Use the dot product to find the angle between vectors and test perpendicularity.
  - Interpret the dot product as alignment/projection in a robotic context.
tags:
  - physical-ai
  - vectors
  - dot-product
...
- **Animation Notes:** rotate $\mathbf{a}$ from aligned → perpendicular → opposed; show the dot-product value sweeping positive → 0 → negative as the projection shrinks, vanishes, then reverses.
