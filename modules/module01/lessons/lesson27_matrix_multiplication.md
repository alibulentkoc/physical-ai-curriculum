...
core_idea: "Matrix multiplication combines transformations: do this action, then that one. The row-by-column arithmetic is just how that combination is computed."
estimated_time: 50
difficulty: Introductory
prerequisites: [4.1, 4.5]
learning_objectives:
  - Understand matrix multiplication as composing two transformations (do A, then B).
  - Apply the product of two matrices to a point and match it to applying them in sequence.
  - Recognize that multiplication is not commutative (order matters), setting up 4.8.
tags:
  - physical-ai
  - transformations
---
...
`[Visual: "apply A then B" shown as two sequential transformations of a shape, equal to applying the single product matrix BA]`

**Diagram Specification**
- **Objective:** show that composing two transformations equals applying their product.
- **Scene:** top row: shape → (A) → intermediate → (B) → result; bottom: shape → (BA) → same result; a note "B(Ap) = (BA)p"; one row·column entry highlighted to show the mechanic supports the geometry.
- **Labels:** "do A, then B," "= apply product BA," "first action on the right."
- **Form:** SVG.
