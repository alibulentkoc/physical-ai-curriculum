...
unit: 03
lesson: 3.7
title: Robot and Camera Frames
core_idea: "A real pick is a chain of frames: the camera sees the fruit, the robot must act on it, and the world remembers it — perception to action is a walk across frames."
estimated_time: 50
difficulty: Introductory
prerequisites: [3.1, 3.5]
learning_objectives:
  - Identify the world, robot, and camera frames in a real harvesting setup.
  - Trace a tomato's coordinates from the camera frame, to the robot frame, to the world frame.
  - Connect the perception → reasoning → action loop (Lesson 1.1) to a chain of frame descriptions.
tags:
  - physical-ai
  - coordinate-frames
  - flagship
...
`[Visual: one pick shown with world, robot, and camera frames simultaneously, and the tomato's coordinate readout in each]`

**Diagram Specification**
- **Objective:** show a single pick described in three frames at once, with the camera→robot→world chain made explicit.
- **Scene:** greenhouse with a tomato; camera frame (offset/tilted on the robot), robot frame (at the base), world frame (room corner); three readouts for the same tomato; arrows showing camera→robot→world.
- **Labels:** "camera frame (perceive)," "robot frame (act)," "world frame (remember)," "same tomato."
- **Form:** SVG.
