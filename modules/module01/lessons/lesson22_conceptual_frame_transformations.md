...
unit: 03
lesson: 3.6
title: Conceptual Frame Transformations
core_idea: "Transforming a point between frames is re-describing it from a shifted, rotated viewpoint — an offset plus a rotation, understood before any matrix appears."
estimated_time: 50
difficulty: Introductory
prerequisites: [3.4, 3.5]
learning_objectives:
  - Describe a frame transformation as an offset (translation) plus a rotation.
  - Convert a point's coordinates between two frames conceptually, without matrices.
  - Explain why the same point gets new coordinates while not moving.
tags:
  - physical-ai
  - coordinate-frames
  - transformations
...
`[Visual: two frames A and B with B offset and rotated from A; one point shown once, with both frames' coordinate readings and the "shift then turn" steps]`

**Diagram Specification**
- **Objective:** show a transformation as offset + rotation, re-describing one fixed point.
- **Scene:** frame A axes; frame B axes drawn offset and rotated by θ; a single point P; readouts "P in A" and "P in B"; small annotations for the offset arrow and the rotation angle.
- **Labels:** "frame A," "frame B," "offset (dx, dy)," "rotation θ," "same point P."
- **Form:** SVG.
