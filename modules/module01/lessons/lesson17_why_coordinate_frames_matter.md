...
unit: 03
lesson: 3.1
title: Why Coordinate Frames Matter
core_idea: "A position has no meaning without a reference frame: the same point gets different coordinates depending on who is measuring."
estimated_time: 40
difficulty: Introductory
prerequisites: [2.1, 2.10]
learning_objectives:
  - Explain why a coordinate is meaningless without a reference frame.
  - Describe the same point from two different origins and get two different (both correct) coordinates.
  - Recognize world, robot, and camera as different frames in a greenhouse robot.
tags:
  - physical-ai
  - coordinate-frames
---
...
`[Visual: one tomato, two observers (a person and a robot) reporting different coordinates for it]`

**Diagram Specification**
- **Objective:** make unmistakable that one fixed point yields different coordinates from different origins.
- **Scene:** a single tomato in a greenhouse; two origins drawn — one at a person/world corner, one at the robot — each with its own axes and its own coordinate readout pointing at the same tomato.
- **Labels:** "same tomato," "observer A coordinates," "observer B coordinates," "the point hasn't moved."
- **Form:** SVG.
