...
unit: 01
lesson: 1.3
title: Configuration vs. Pose
core_idea: "Configuration is the list of joint variables (joint space); pose is the gripper's position and orientation (task space). Forward kinematics maps configuration to pose."
estimated_time: 40
difficulty: Core
prerequisites: [1.2]
learning_objectives:
  - Distinguish joint space (configuration) from task space (pose).
  - State forward kinematics as the map between them.
  - Recognize that the map is many-to-one in general.
tags:
  - physical-ai
  - kinematics
  - configuration
...
`[Visual: joint space (a list of angles) on one side, task space (the gripper pose in the workspace) on the other, with the forward-kinematics arrow between them and a hint that two configurations can map to one pose]`

**Diagram Specification**
- **Objective:** contrast joint space and task space and show FK between them.
- **Scene:** left panel "joint space": dials/sliders for θ1,θ2,θ3; right panel "task space": an arm with the gripper pose (position + small orientation frame) in the workspace; arrow "forward kinematics" left→right; a faint second arm pose (elbow-up vs elbow-down) reaching the same point, labeled "many configs → one pose."
- **Labels:** "configuration q (joint space, n-D)," "pose T_0^n (task space, 6-D)," "FK: q → pose," "many-to-one."
- **Form:** SVG.
