...
unit: 04
lesson: 4.2
title: Position and Orientation of the Gripper
core_idea: "The end-effector pose T_0^n packs position (translation column) and orientation (rotation block). Both come from the same product; orientation matters as much as position for grasping."
estimated_time: 45
difficulty: Core
prerequisites: [4.1]
learning_objectives:
  - Extract position and orientation from T_0^n in 3D.
  - Interpret the rotation block as the gripper's axes.
  - Explain why grasping needs the full pose.
tags:
  - physical-ai
  - kinematics
  - pose
...
`[Visual: the end-effector pose matrix with the translation column labeled "position p" and the rotation block's three columns labeled as the gripper's x, y, z axes, shown on a gripper in the base frame]`

**Diagram Specification**
- **Objective:** show reading position and a full 3D orientation out of T_0^n.
- **Scene:** the 4×4 matrix with translation column boxed "position p", rotation block boxed "orientation R"; arrows from R's three columns to a gripper drawn with its x (red), y (green), z (blue) axes in the base frame; the z-axis labeled "approach direction."
- **Labels:** "p = translation column," "R columns = gripper axes in base frame," "z_g = approach direction," "FK gives both at once."
- **Form:** SVG.
