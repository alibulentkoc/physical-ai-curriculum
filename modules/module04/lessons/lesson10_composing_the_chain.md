...
unit: 03
lesson: 3.2
title: Composing the Chain
core_idea: "Forward kinematics is the matrix product of the per-joint transforms, multiplied base-to-tip: T_0^2 = T_0^1 T_1^2. The trig formulas of the previous lesson fall straight out of it."
estimated_time: 45
difficulty: Core
prerequisites: [3.1]
learning_objectives:
  - Express two-joint forward kinematics as T_0^1 T_1^2.
  - Connect the matrix product to the trig position formula.
  - Apply Module 2 composition (right-to-left frame chaining).
tags:
  - physical-ai
  - kinematics
  - composition
...
`[Visual: T_0^1 times T_1^2 equals T_0^2, with the resulting rotation = θ1+θ2 and translation = the 2-link position formula, shown as frames chained base→elbow→gripper]`

**Diagram Specification**
- **Objective:** show FK as the matrix product and its equivalence to the trig.
- **Scene:** three frames base→elbow→gripper with transforms T_0^1, T_1^2 labeled on the links; an equation row "T_0^2 = T_0^1 · T_1^2"; a callout that the product's rotation is θ1+θ2 and translation is the 2-link formula.
- **Labels:** "compose base→tip," "angles add: θ1+θ2," "translation = 2-link position formula," "extends to T_0^3 = T_0^1 T_1^2 T_2^3."
- **Form:** SVG.
