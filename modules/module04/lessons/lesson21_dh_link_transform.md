...
unit: 06
lesson: 6.1
title: The DH Link Transform
core_idea: "The four DH parameters assemble into one fixed SE(3) formula: T = Rot_z(θ) · Trans_z(d) · Trans_x(a) · Rot_x(α). Plug in four numbers, get the link transform."
estimated_time: 50
difficulty: Core
prerequisites: [5.4]
learning_objectives:
  - Write the DH link transform as a product of four elementary motions.
  - Compute T_{i-1}^i from a row of DH parameters.
  - Verify it against the hand-built planar transform.
tags:
  - physical-ai
  - kinematics
  - denavit-hartenberg
...
`[Visual: the four elementary motions Rot_z(θ), Trans_z(d), Trans_x(a), Rot_x(α) composed left to right into the DH link transform T_{i-1}^i]`

**Diagram Specification**
- **Objective:** show the DH transform as four composed motions.
- **Scene:** four boxes in a row — "Rot_z(θ)", "Trans_z(d)", "Trans_x(a)", "Rot_x(α)" — with × between them, arrow to a single box "T_{i-1}^i ∈ SE(3)"; small icons (curved arrow for rotations, straight for translations).
- **Labels:** "spin about z," "slide along z," "walk along x," "tilt about x," "standard (distal) order."
- **Form:** SVG.
