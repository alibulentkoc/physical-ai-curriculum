...
unit: 04
lesson: 4.4
title: "Unit 4 Recap and Midpoint Checkpoint: The Forward Path, Assembled"
core_idea: "Units 1–4 built the forward path: Perceive → Understand → Plan → Execute. This midpoint checkpoint assembles all four stages and runs them once, end to end, on the real layers — perception to physical joint motion — with no failure handling yet. Proving the layers connect and the gripper reaches the fruit is the gate that de-risks the back half, where the system learns to detect failures and recover."
estimated_time: "35 min"
difficulty: "Intermediate"
prerequisites:
  - "4.1 — The motion stack"
  - "4.2 — Handoff contracts"
  - "4.3 — A goal pose's journey to joint motion"
learning_objectives:
  - "Assemble and run the forward path Perceive → Understand → Plan → Execute end to end."
  - "Confirm the forward path reaches the target with no failure handling."
  - "State what the back half (Units 5–8) adds: detection and recovery."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: the four forward stages Perceive → Understand → Plan → Execute as a left-to-right pipeline with the value on each arrow (detections, target+pose, reference, motion), ending in a gripper at the fruit marked "arrived ✓"; the Track/Recover stages shown greyed-out and labelled "back half: Units 5–8".]`

**Diagram Specification**
- **Objective:** show the assembled forward path reaching the target, with the back half greyed as next.
- **Scene:** four active stages with batons, arrival at the fruit, two greyed stages (Track, Recover) labelled as the second half.
- **Labels:** "detections", "target + pose", "reference(t)", "joint motion", "arrived ✓", "Units 5–8: detect & recover".
- **Form:** SVG.
