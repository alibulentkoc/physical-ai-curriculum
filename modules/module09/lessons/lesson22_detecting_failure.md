...
unit: 06
lesson: 6.2
title: "Detecting Failure: Health Signals and Guards"
core_idea: "A taxonomy is a list; a guard is the check that fires an entry from it. A guard sits at a seam, reads the existing output crossing it, and raises the matching event the moment a condition is met. Place a guard at every stage of the forward path and the pipeline detects failure automatically — halting at the first hard failure and flagging warnings — all from signals the layers already emit. This is detection as thresholded reading, not diagnosis."
estimated_time: "50 min"
difficulty: "Intermediate"
prerequisites:
  - "6.1 — The failure taxonomy"
  - "5.2 — System monitoring (the signals guards read)"
learning_objectives:
  - "Define a guard as a condition on an existing output that fires a taxonomy event."
  - "Place guards across the forward path and run the guarded pipeline."
  - "Explain halting on hard failure vs. flagging warnings."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: the forward path Perceive → Understand → Plan → Execute → Track with a guard (tripwire) at each seam; each guard wired to its signal and its taxonomy event; hard-failure guards drawn halting the flow (red stop), the health screen drawn annotating (amber flag) without halting.]`

**Diagram Specification**
- **Objective:** show guards placed across the forward path, halting on hard failure and flagging warnings.
- **Scene:** the five guard points with their signals and events; red halts vs amber flags.
- **Labels:** "no target → halt", "None → halt", "validated=False → halt", "verdict false", "health screen (warn)".
- **Form:** SVG.
