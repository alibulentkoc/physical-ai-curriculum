...
unit: 05
lesson: 5.1
title: "Closing the Loop: Tracking Error and Success Criteria"
core_idea: "Execute produces motion; it does not pronounce on whether the motion succeeded. The Track stage does that — it reads the executed run against explicit success criteria (final error, tracking RMS, and the tool actually reaching the target pose) and returns a verdict. 'The arm moved' and 'the pick succeeded' are different claims, owned by different stages. Track owns the judgement; the controller only produced the motion."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "4.4 — Midpoint checkpoint (the forward path reaches the target)"
  - "M8 — the control layer (which produces the tracking error Track reads)"
learning_objectives:
  - "Distinguish Execute (produces motion) from Track (judges success)."
  - "State explicit success criteria: final error, tracking RMS, pose reached."
  - "Return a Track verdict and explain who owns it."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: Execute's output (an error-vs-time curve and a final tool position) flowing into a "Track" gate that checks three criteria boxes — final error ≤ τ, RMS ≤ τ, pose ≤ τ — and emits a verdict {success, reason}; an annotation "Execute produces motion · Track judges it".]`

**Diagram Specification**
- **Objective:** show Track as a judgement gate reading Execute's output against criteria.
- **Scene:** execution record → three criteria checks → verdict {success, reason}.
- **Labels:** "final error ≤ τ_final", "RMS ≤ τ_rms", "pose ≤ τ_pose", "verdict + reason", "Execute moves · Track judges".
- **Form:** SVG.
