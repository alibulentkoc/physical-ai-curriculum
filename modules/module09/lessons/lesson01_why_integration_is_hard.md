...
unit: 01
lesson: 1.1
title: "Why Integration Is Hard: The Whole vs. The Parts"
core_idea: "Eight modules each built a layer that works in isolation. A working robot is not the sum of working layers — it is the layers cooperating across interfaces, under failure, in real time. Integration is its own discipline: the bugs live in the seams, not the parts. Module 9 teaches the seams."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M8 — the control layer (tracking_controller), the last layer built"
  - "M3–M7 — perception, kinematics, velocity, trajectory layers"
learning_objectives:
  - "Explain why a system of working parts can still fail as a whole."
  - "Distinguish component correctness from system correctness."
  - "State what Module 9 adds that no earlier module did: the seams between layers."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: two panels — eight green "verified" layer boxes on the left labelled "each correct"; on the right the same boxes wired in a ring with red seam-markers on every handoff, labelled "the bugs live here".]`

**Diagram Specification**
- **Objective:** show that correctness moves from the boxes (parts) to the arrows (seams) when you assemble a system.
- **Scene:** left — a stack of independently checked layers; right — the same layers composed into the Perceive→…→Recover loop with the handoffs highlighted as the risk surface.
- **Labels:** "each layer verified", "interface", "timing", "shared world state", "the seams".
- **Form:** SVG.
