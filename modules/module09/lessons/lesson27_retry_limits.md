...
unit: 07
lesson: 7.3
title: "Retry Limits and State Across Attempts"
core_idea: "A retry that can repeat forever is not recovery — it is a loop. The orchestrator must remember state across attempts (how many tries, against which fault) and enforce a budget, so a transient fault clears within a few attempts while a persistent one escalates instead of spinning. Deterministic faults escalate immediately; retryable faults escalate when the budget is spent. Memory across attempts is what makes recovery terminate."
estimated_time: "50 min"
difficulty: "Intermediate"
prerequisites:
  - "7.2 — The recovery policy (retryable vs deterministic)"
  - "7.1 — The orchestrator"
learning_objectives:
  - "Explain why a retry budget and cross-attempt state are required for termination."
  - "Trace how a transient fault recovers and a persistent fault escalates."
  - "Distinguish immediate escalation (deterministic) from budget-exhaustion escalation."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: a retry loop with a counter k → run forward path → success (exit: recovered) / deterministic fault (exit immediately: escalate skip) / retryable fault (k+1, loop) → at k=N exit (escalate: budget exhausted); two example traces overlaid — transient (recovers at k=1) and persistent (escalates at k=N).]`

**Diagram Specification**
- **Objective:** show the bounded retry loop with its three exits and the cross-attempt counter.
- **Scene:** loop with counter, three exit paths, and two traces (transient recovers, persistent escalates).
- **Labels:** "k (attempts so far)", "success → recovered", "deterministic → escalate now", "k=N → escalate", "budget N".
- **Form:** SVG.
