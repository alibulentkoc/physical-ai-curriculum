...
unit: 02
lesson: 2.2
title: "Target Selection: Which Fruit, Which Pose?"
core_idea: "A clean world state may hold many pickable fruit. Something must choose one — and decide the pose to grasp it at — before any planning can begin. That choice is a policy decision owned by the Understand stage: filter to ripe and reachable, rank by a cost (here, distance), commit to one, and hand a target pose forward. It is not perception, not IK, not planning; it is the decision that sits between them."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "2.1 — From perception output to world state"
learning_objectives:
  - "State the target-selection policy: filter (ripe, reachable), rank, commit."
  - "Explain why selection — not perception or IK — owns the 'which fruit' decision."
  - "Describe what a target pose is and what it hands to the planning stage."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: a world state of five fruit; two greyed out (unripe / unreachable) by the filter; the remaining three ranked by distance with the nearest highlighted as "committed target"; an arrow carrying its pose to the "Plan" box.]`

**Diagram Specification**
- **Objective:** show filter → rank → commit, and the pose handed forward.
- **Scene:** greenhouse fruit set, filter greying out the ineligible, ranking arrows, the committed target glowing, a pose arrow into Plan.
- **Labels:** "filter: ripe ∧ reachable", "rank: nearest", "committed target + pose", "→ Plan".
- **Form:** SVG.
