...
unit: 04
lesson: 4.3
title: "System Walkthrough — A Goal Pose's Journey to Joint Motion"
core_idea: "We follow one goal pose the whole way from decision to physical motion: target pose → goal configuration (IK) → timed reference (planner) → sampled, tracked, and driven into real joint angles (controller + plant) → forward kinematics confirming the gripper arrived. It is the Unit 1 trace habit applied to the motion half of the pipeline — read the value, owner, and check at every stage, now ending in actual movement."
...
learning_objectives:
  - "Trace one goal pose from decision through IK, planning, and execution to joint motion."
  - "Read the motion-half trace: value, owner, and check at each stage."
  - "Confirm the executed motion arrives at the target via forward kinematics."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: a vertical journey for one pose — five stamped rows: target pose, goal configuration (IK, FK✓), reference(t) (validated), executed q(t) (tracking error→0), arrived (FK = target) — each row showing the value handed forward and the owning module.]`

**Diagram Specification**
- **Objective:** show one pose's full journey to motion as an accumulating, checked trace.
- **Scene:** five stamped stages from pose to arrival, each with value, owner, and check.
- **Labels:** "target pose [M9]", "q_goal [M5, FK✓]", "reference(t) [M7, validated]", "q(t) [M8, rms→0]", "arrived [FK = target]".
- **Form:** SVG.
