...
unit: 04
lesson: 4.1
title: "The Motion Stack: Reference → Velocity → Tracking Controller"
core_idea: "A validated reference is still just a plan on paper. Execute turns it into real joint motion by sampling the reference and tracking it closed-loop with Module 8's controller, which drives a plant whose actual state feeds back. Underneath, Module 6's velocity layer relates joint motion to tool motion. The stack is reference (M7) → controller (M8) → plant, with velocity (M6) as the differential primitive — all wrapped, all reused."
estimated_time: "50 min"
difficulty: "Intermediate"
prerequisites:
  - "3.4 — Unit 3 recap (a validated reference)"
  - "M8 — the control layer (tracking_controller); M6 — the velocity layer"
learning_objectives:
  - "Describe the motion stack: sample the reference, track it with the controller, drive the plant."
  - "Explain how the actual joint state feeds back to close the loop."
  - "Place Module 6's velocity layer as the differential primitive underneath execution."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: a closed loop — "reference(t)" sampled into "tracking_controller (M8)" producing a command into "plant (joint)"; the plant's actual (q, q̇) feeding back to the controller's input; a small side-box "velocity layer (M6): ξ = J q̇" annotating the joint↔tool relationship.]`

**Diagram Specification**
- **Objective:** show the closed execution loop and where each module sits.
- **Scene:** reference → controller → plant → (feedback) → controller, with the M6 velocity-layer side note.
- **Labels:** "reference(t) [M7]", "tracking_controller [M8]", "plant", "measured (q, q̇)", "velocity layer ξ = J q̇ [M6]".
- **Form:** SVG.
