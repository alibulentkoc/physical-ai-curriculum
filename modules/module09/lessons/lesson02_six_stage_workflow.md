...
unit: 01
lesson: 1.2
title: "The Six-Stage Workflow: Data Flow, Interfaces, and Subsystem Ownership"
core_idea: "The whole module hangs on one spine: Perceive → Understand → Plan → Execute → Track → Recover. Each stage has an owner, a defined input, and a defined output. Learn the spine as a contract — what information enters each stage, what leaves, and which subsystem owns the decision — and every later lesson becomes a walk along it."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "1.1 — Why integration is hard (the seams)"
learning_objectives:
  - "Name the six stages of the workflow in order and the question each answers."
  - "For each stage, state its owning subsystem, its input, and its output."
  - "Read the data-flow spine as a chain of interface contracts."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: the six-stage spine as a horizontal flow of labelled boxes Perceive→Understand→Plan→Execute→Track→Recover, each box annotated with its owning module and its input/output; a curved "Recover" arrow loops back from the end to Understand.]`

**Diagram Specification**
- **Objective:** make the spine memorable and show ownership + I/O at a glance.
- **Scene:** six boxes left-to-right with the data label on each connecting arrow (`detections`, `current_target`, `reference`, `command`, `tracking_error`), plus a feedback arrow from Recover back to Understand.
- **Labels:** each stage name, owning module under each box, the data name on each arrow.
- **Form:** SVG (a Mermaid version reads well too and may be added).
