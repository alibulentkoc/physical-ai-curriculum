...
unit: 06
lesson: 6.4
title: "Unit 6 Recap and Installment C Milestone: The System Can See Its Own Failures"
core_idea: "Units 5 and 6 gave the system self-awareness: it judges its runs (Track), watches its health (telemetry), names its failures (taxonomy), detects them automatically (guards), and localises each to a what, a where, and an owner. The forward path now fails loudly and precisely instead of silently. This recap consolidates Execute → Track → Detect and marks the Installment C milestone — everything is in place for Unit 7 to finally answer 'how do we recover?'"
estimated_time: "35 min"
difficulty: "Intermediate"
prerequisites:
  - "6.1 — Failure taxonomy"
  - "6.2 — Detecting failure"
  - "6.3 — Failure analysis"
learning_objectives:
  - "Reproduce the detect-and-localise pipeline end to end."
  - "State the What / Where / Who localisation for each fault."
  - "Identify exactly what Unit 7 (Recover) will add and why it could not come sooner."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: the Execute → Track → Detect chain — Execute (motion) → Track (verdict) + Monitor (telemetry) → Guards (annunciator panel firing events) → Analysis (What/Where/Who + owner); a greyed "Unit 7: Recover (How?)" box downstream, with an arrow labelled "localised fault + owner".]`

**Diagram Specification**
- **Objective:** one image consolidating detect-and-localise and pointing at recovery as next.
- **Scene:** Execute → Track/Monitor → Guards → Analysis (triad + owner) → greyed Recover.
- **Labels:** "verdict + telemetry", "events (panel)", "What/Where/Who + owner", "Unit 7: Recover (How?)".
- **Form:** SVG.
