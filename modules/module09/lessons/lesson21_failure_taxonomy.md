...
unit: 06
lesson: 6.1
title: "The Failure Taxonomy: Integration Events"
core_idea: "Failures in an integrated system are not mysterious component breakdowns — they are integration events: a known signal crossing a known line at a known seam. No-target, unreachable, invalid-plan, tracking-failure, near-singular, excess-effort: each is a condition on an output the layers already emit. Naming them as a taxonomy turns 'something went wrong' into a finite, checkable list, the prerequisite for detecting and localising failure without inventing diagnostic theory."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "5.4 — Unit 5 recap (telemetry and the Track verdict)"
  - "Units 1–4 — the six-stage forward path"
learning_objectives:
  - "Define failures as integration events tied to existing signals."
  - "Enumerate the failure taxonomy and the stage each event belongs to."
  - "Distinguish a hard failure from a health warning."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: an annunciator panel of six named lights — NO_TARGET, UNREACHABLE, PLAN_INVALID, TRACKING_FAILURE (red = failure) and NEAR_SINGULAR, EXCESS_EFFORT (amber = warning) — each wired to its source signal (understand commit, IK None, validated, verdict, manipulability, effort) and tagged with its stage.]`

**Diagram Specification**
- **Objective:** present the taxonomy as a named panel, each event wired to its signal and stage.
- **Scene:** six event lights (4 red failures, 2 amber warnings) with source-signal wires and stage tags.
- **Labels:** the six event names, "signal", "stage", "failure (red) vs warning (amber)".
- **Form:** SVG.
