...
unit: 02
lesson: 2.1
title: "From Perception Output to World State"
core_idea: "Perception does not hand the system a tidy list of fruit — it hands raw detections: noisy positions, confidence scores, sometimes duplicates, all in the camera's terms. The first seam of the pipeline, Perceive → Understand, is the act of turning that raw output into a clean, trustworthy world state the rest of the system can build on. Owning that conversion is a Module 9 job, not a perception job."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "1.4 — Unit 1 recap (the spine, the trace)"
  - "M3 — perception (the detections we consume)"
learning_objectives:
  - "Distinguish a raw detection from a world-state entry."
  - "Describe the normalization the Perceive → Understand seam performs."
  - "Explain why owning detection-to-world-state conversion belongs to integration, not perception."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: left, a messy "Perceive" cloud of raw detections (one duplicated pair, one noisy outlier, confidences shown); a funnel labelled "Understand: dedupe · annotate · normalize"; right, a tidy "world state" list of clean entries with reachability/distance annotations.]`

**Diagram Specification**
- **Objective:** show raw detections becoming a clean world state through a single normalization step.
- **Scene:** greenhouse detections on the left, the conversion funnel in the middle, the tidy world-state list on the right.
- **Labels:** "detections (raw)", "dedupe / annotate / normalize", "world state (trusted)".
- **Form:** SVG.
