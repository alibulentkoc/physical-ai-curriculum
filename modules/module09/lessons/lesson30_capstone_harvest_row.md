...
unit: 08
lesson: 8.2
title: "Capstone: Harvesting a Row with Injected Failure"
core_idea: "The capstone runs the whole system: the robot harvests a row of fruit, one pick cycle after another, keeping a ledger so each fruit is attempted once. Mid-row, a failure is injected — and the system absorbs it. A transient fault recovers and the pick succeeds; a deterministic fault is skipped and the harvest continues. One stubborn fruit never stalls the row. This is graceful degradation: the integrated system completes its job around failures rather than halting on them."
estimated_time: "55 min"
difficulty: "Advanced"
prerequisites:
  - "8.1 — The full pick cycle"
  - "7.3 — Retry limits and escalation"
learning_objectives:
  - "Run the full system across a row with a harvest ledger."
  - "Observe a mid-row failure recovered or skipped without stalling the row."
  - "Define graceful degradation for the integrated harvester."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: a row of fruit with the robot moving along it; harvested fruit marked ✓ (green), one injected-failure fruit shown either recovering (amber → ✓ after a retry) or skipped (✗, marked with its fault), the rest ✓; a ledger panel listing harvested vs skipped; a banner "graceful degradation: finish the row, never stall".]`

**Diagram Specification**
- **Objective:** show the full row harvest with one injected failure recovered or skipped, the rest clean.
- **Scene:** a fruit row with harvested ✓, the injected-fault fruit (recover or skip), and a harvest ledger.
- **Labels:** "harvested ✓", "recovered (retry)", "skipped (fault)", "ledger: H / S", "finish the row, never stall".
- **Form:** SVG.
