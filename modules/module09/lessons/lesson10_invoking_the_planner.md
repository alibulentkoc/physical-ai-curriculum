...
unit: 03
lesson: 3.2
title: "Invoking the Planner: Calling the Reference Layer"
core_idea: "With a goal configuration in hand, the Plan stage asks Module 7 for a timed reference trajectory: when each joint should be where, with velocities and accelerations, validated against limits. Module 9 does not plan — it invokes the real reference layer through its interface, reads the validated flag, and hands the reference(t) forward. The planner is wrapped verbatim; integration owns only the call and the handoff."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "3.1 — From target pose to goal configuration"
  - "M7 — the reference trajectory layer (the planner we wrap)"
learning_objectives:
  - "Invoke Module 7's reference layer with a start and goal configuration."
  - "Read the reference(t) signal: q_d, qd_d, qdd_d, and the validated flag."
  - "State what Module 9 owns about planning (the call and handoff) vs. what it does not (the planning math)."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: a box "goal configuration q_goal" feeding into the Module 7 "reference layer" box, which emits a reference(t) plotted as three stacked curves q_d, q̇_d, q̈_d over time, with a green "validated ✓" stamp and a duration label; an arrow carries reference(t) to "Execute".]`

**Diagram Specification**
- **Objective:** show the planner as a wrapped layer that turns a goal configuration into a timed, validated reference.
- **Scene:** q_goal → M7 reference layer → reference(t) curves (position/velocity/acceleration) → Execute.
- **Labels:** "q_start, q_goal", "Module 7 (wrapped)", "reference(t): q_d, q̇_d, q̈_d", "validated ✓", "duration".
- **Form:** SVG.
