...
unit: 03
lesson: 3.1
title: "From Target Pose to Goal Configuration"
core_idea: "The committed target is a pose in the world — but the planner and controller think in joint angles, not Cartesian points. The Understand → Plan seam must convert the target pose into a goal configuration using Module 5's inverse kinematics, verify it with Module 4's forward kinematics, and decide what to do when IK reports the pose unreachable or returns more than one solution. The math is M5's; the decision is integration's."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "2.4 — Unit 2 recap (a committed target with a pose)"
  - "M5 — inverse kinematics (the layer we wrap)"
  - "M4 — forward kinematics (the verification)"
learning_objectives:
  - "Convert a target pose to a goal configuration by invoking Module 5 IK."
  - "Verify a configuration with Module 4 FK and interpret a round-trip error."
  - "Decide what the seam does when IK returns None or multiple solutions."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: a target point in the workspace with TWO arm configurations reaching it (elbow-up dashed, elbow-down solid); a second point outside the reach annulus marked "IK → None"; an FK arrow looping from a configuration back to the point labelled "verify".]`

**Diagram Specification**
- **Objective:** show the two-solution ambiguity, the unreachable case, and FK verification.
- **Scene:** the 2R arm reaching one target two ways; an unreachable target beyond the annulus; an FK check arrow.
- **Labels:** "elbow up", "elbow down", "IK → None (unreachable)", "FK verifies".
- **Form:** SVG.
