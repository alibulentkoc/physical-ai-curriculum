...
unit: 04
lesson: 4.2
title: "Handoff Contracts: ξ_d, q̇, and Real-Time Execution"
core_idea: "The motion stack only works if each handoff carries exactly what the next stage expects, in the right units and frame, at the right rate. This lesson makes the contracts explicit: the reference hands the controller (q_d, q̇_d, q̈_d); the Jacobian relates a desired tool twist ξ_d to a joint rate q̇; and 'real-time' here means sampling the loop at a fixed timestep. Get the contracts right and the stack composes; get a unit or rate wrong and it silently fails."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "4.1 — The motion stack"
learning_objectives:
  - "State the exact contract of each motion-stack handoff (types, units, frame)."
  - "Relate a desired tool twist ξ_d to a joint rate q̇ via the Jacobian (M6)."
  - "Explain what 'real-time' means here: fixed-timestep periodic execution."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: three labelled contract cards — (a) reference→controller carrying (q_d, q̇_d, q̈_d) with units; (b) the Jacobian relating ξ_d=[v;ω] and q̇ in the same frame; (c) a fixed-Δt clock driving the periodic loop — each card noting "must match: units / frame / rate".]`

**Diagram Specification**
- **Objective:** make the three handoff contracts explicit and memorable.
- **Scene:** three cards — reference sample contract, twist↔joint-rate contract, fixed-Δt timing contract.
- **Labels:** "(q_d, q̇_d, q̈_d) rad", "ξ = J q̇, same frame", "fixed Δt, periodic".
- **Form:** SVG.
