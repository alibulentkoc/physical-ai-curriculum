...
unit: 08
lesson: 8.3
title: "Reading the Integrated System: Guarantees, Degradation, and Boundaries"
core_idea: "A finished system deserves an honest account of what it does and does not promise. The integrated harvester guarantees a definite outcome per fruit and a completed row (graceful degradation); it does not guarantee every fruit is picked, nor does it diagnose root causes or optimise the harvest. Reading the harvest report — harvested, skipped-with-reason, attempts — is how an operator judges a run. Knowing the boundaries is what separates an honest engineer from an overclaiming one."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "8.2 — The capstone harvest"
  - "6.4 — Detection and localisation"
learning_objectives:
  - "State what the integrated system guarantees and what it does not."
  - "Read a harvest report to judge a run."
  - "Name the scope boundaries the integration deliberately holds."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: two columns — "Guarantees" (accounting, completion, graceful degradation, localised faults, each ✓) and "Boundaries / Not Claimed" (every fruit picked, root-cause diagnosis, optimal sequencing, new dynamics, each marked out-of-scope) — with a harvest-report panel in the middle showing harvested/skipped/attempts.]`

**Diagram Specification**
- **Objective:** present the system's guarantees beside its explicit boundaries, with a sample harvest report.
- **Scene:** guarantees column, boundaries column, central harvest-report panel.
- **Labels:** "guarantees ✓", "out-of-scope", "harvest report: H / S / attempts".
- **Form:** SVG.
