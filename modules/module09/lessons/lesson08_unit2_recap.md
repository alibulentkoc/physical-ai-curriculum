...
unit: 02
lesson: 2.4
title: "Unit 2 Recap: Perceive → Understand"
core_idea: "Unit 2 opened and closed the first seam. Detections become a trusted world state (dedupe, annotate, normalize); the world state becomes a single committed target (filter, rank, commit); and the whole conversion stays correct under occlusion, noise, and duplicates. This recap stitches those into one runnable Perceive → Understand slice and sets up the handoff to Plan."
estimated_time: "30 min"
difficulty: "Intermediate"
prerequisites:
  - "2.1 — Perception output to world state"
  - "2.2 — Target selection"
  - "2.3 — When perception lies"
learning_objectives:
  - "Reproduce the full Perceive → Understand seam from detections to committed target."
  - "State the conversion's duties and the selection policy from memory."
  - "Identify what the seam hands to the Plan stage next."
tags:
  - physical-ai
  - robotics
  - systems-integration
...
`[Visual: the Perceive → Understand seam as one pipeline — raw detections in, a "clean" funnel (dedupe/annotate/normalize), a "decide" funnel (filter/rank/commit), and a single committed target + pose out, with three small fault icons (leaf, jitter, double) shown being absorbed along the way.]`

**Diagram Specification**
- **Objective:** one image summarising the whole first seam, faults included.
- **Scene:** detections → clean → decide → committed target/pose, with occlusion/noise/duplicate icons absorbed in the clean step.
- **Labels:** "detections", "clean", "decide", "committed target → Plan", "faults absorbed".
- **Form:** SVG.
