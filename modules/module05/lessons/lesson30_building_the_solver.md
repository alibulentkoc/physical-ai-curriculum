...
unit: 08
lesson: 8.2
title: "Building the Solver (Analytical + Numerical)"
core_idea: "Build the capstone's solver core: try the analytical closed form first (fast, all solutions); fall back to the numerical solver (Newton / damped least squares) when closed form doesn't apply. One solve() that returns all candidate configurations."
estimated_time: 50
difficulty: Capstone
prerequisites: [8.1]
learning_objectives:
  - Implement a solver that prefers analytical and falls back to numerical.
  - Return all candidate configurations for the target.
  - Seed the numerical solver to recover multiple solutions.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: solve() flow — analytical branch (closed form → all solutions) preferred; if not applicable, numerical branch (seed × N → candidates); both emit a candidate list]`

**Diagram Specification**
- **Objective:** show the analytical-first, numerical-fallback solver core.
- **Scene:** an entry "solve(target, arm)"; a decision "closed form applies?"; yes → "ik_2link_closed → {elbow-up, elbow-down}"; no → "seed × N → ik_solve (Newton/DLS) → candidates"; both merge into "candidate list (unverified)" handed to the pipeline.
- **Labels:** "analytic: fast, all solutions," "numeric: seed to recover multiplicity," "candidates → verify/filter/select."
- **Form:** SVG.
