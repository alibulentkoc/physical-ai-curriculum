...
unit: 08
lesson: 8.3
title: "Verifying, Selecting, and Handling No-Solution Cases"
core_idea: "Finish the capstone: verify every candidate by FK, filter by joint limits, select the best, and handle the no-solution cases (unreachable, no feasible) gracefully — returning either an executable configuration or a clear reason."
estimated_time: 50
difficulty: Capstone
prerequisites: [8.2]
learning_objectives:
  - Complete the pipeline with verification, filtering, and selection.
  - Handle unreachable and no-feasible-solution cases with clear status.
  - Return a single executable configuration or an actionable failure.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: the pipeline tail — candidates → verify (drop misses) → filter limits (drop infeasible) → select (min cost) → θ⋆ ok; with status branches to unreachable / no_solution / no_feasible]`

**Diagram Specification**
- **Objective:** show verification/selection and the no-solution status branches.
- **Scene:** "candidates" → "verify FK (drop residual > tol)" → "filter limits (drop out-of-range)" → "select min cost" → "θ⋆ (ok)"; side branches with red tags "unreachable," "no_solution," "verification_failed," "no_feasible," each pointing to a "report reason" box.
- **Labels:** "verify," "filter," "select," "clean failure with a reason."
- **Form:** SVG.
