...
unit: 03
lesson: 3.3
title: "Decoupling Position and Orientation (Wrist-Partitioned Arms)"
core_idea: "Many real arms split into an arm that sets the wrist's position and a wrist that sets orientation. Decoupling lets you solve position first, then orientation — turning one hard 6-DOF problem into two smaller closed-form ones (concept level)."
estimated_time: 45
difficulty: Core
prerequisites: [3.2]
learning_objectives:
  - Explain the idea of decoupling position from orientation.
  - Describe a spherical wrist and why it enables decoupling.
  - State why decoupling makes closed-form solutions possible for 6-DOF arms (concept).
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: a 6-DOF arm with a spherical wrist; the wrist center pw highlighted; step 1 the arm places pw, step 2 the wrist sets orientation in place]`

**Diagram Specification**
- **Objective:** show the position/orientation split via the wrist center.
- **Scene:** a stylized 6-joint arm; the last three axes meeting at a point labeled "wrist center p_w"; an offset $d_6$ along the gripper approach axis from $p_w$ to the gripper; a two-step callout: "(1) arm places p_w (position)" and "(2) wrist sets R (orientation, in place)."
- **Labels:** "spherical wrist," "p_w = p_d − d₆ R_d ẑ," "R₃⁶ = (R₀³)⁻¹R_d," "two small problems."
- **Form:** SVG.
