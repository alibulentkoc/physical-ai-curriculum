...
unit: 06
lesson: 6.3
title: "Choosing Among Solutions (Nearest, Smooth, Limit-Safe)"
core_idea: "When several solutions are feasible, the robot must pick one. Good criteria are: nearest to the current pose (least motion), smooth (no sudden flips), and limit-safe (away from joint limits and singularities). A small cost function ranks them."
estimated_time: 45
difficulty: Core
prerequisites: [6.2]
learning_objectives:
  - State the common criteria for selecting among IK solutions.
  - Pick the nearest solution to the current configuration.
  - Combine criteria into a simple selection cost.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: two feasible solutions for a target; the one nearer the current arm pose is chosen (least joint motion), the far one (a big flip) is rejected]`

**Diagram Specification**
- **Objective:** show "nearest to current pose" selection.
- **Scene:** the current arm pose (faint); a target with its two feasible solutions; the solution close to the current pose highlighted "chosen (least motion)"; the other, a large reconfiguration, greyed "rejected (big flip)"; small tags for "limit margin" and "away from singularity."
- **Labels:** "current pose," "nearest ✓," "big flip ✗," "cost = motion + limit + singularity."
- **Form:** SVG.
