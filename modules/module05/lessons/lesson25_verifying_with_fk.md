...
unit: 07
lesson: 7.1
title: "Verifying a Solution with Forward Kinematics"
core_idea: "Never trust an IK solution unverified: push every candidate joint configuration back through the forward map and check the gripper actually lands on the target within tolerance. Solve → verify → accept or reject."
estimated_time: 40
difficulty: Core
prerequisites: [6.4]
learning_objectives:
  - State the solve-verify-accept/reject discipline for inverse kinematics.
  - Verify an IK solution by forward kinematics within a tolerance.
  - Explain why verification catches analytical, numerical, and selection errors.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: solve → verify pipeline — a candidate θ feeds the FK map, the reached point is compared to the target; within tol → accept (green), else reject (red) and re-solve]`

**Diagram Specification**
- **Objective:** show the solve→verify→accept/reject loop.
- **Scene:** a box "IK solver → candidate θ"; arrow into "forward kinematics f(θ) → p_reached"; a comparator "‖p_target − p_reached‖ < tol?"; two outgoing arrows — green "accept → command the arm" and red "reject → re-solve / re-seed / reposition."
- **Labels:** "candidate θ," "FK is exact," "residual," "accept ✓ / reject ✗."
- **Form:** SVG.
