...
unit: 08
lesson: 8.1
title: "The Project: From Target Pose to Joint Angles"
core_idea: "The capstone: given a perceived fruit's grasp pose in the base frame, produce a chosen, verified joint configuration the arm can execute — integrating perception, transforms, forward kinematics, and inverse kinematics into one workflow."
estimated_time: 50
difficulty: Capstone
prerequisites: [7.4]
learning_objectives:
  - State the Reach-the-Fruit problem and its full input-to-output flow.
  - Identify which module each stage of the capstone draws on.
  - Run the integrated perceive-to-reach pipeline on a target.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: the full Reach-the-Fruit flow as an integrated diagram, each stage tagged with its source module, from perceived fruit to verified joint angles]`

**Diagram Specification**
- **Objective:** present the capstone as one integrated, module-tagged pipeline.
- **Scene:** a greenhouse fruit at left; stages left→right "perceive (M3)" → "place in base frame (M2/M4)" → "reachable?" → "solve: analytic ∥ numeric (M5)" → "filter limits (M5)" → "verify FK (M4/M5)" → "select (M5)" → an arm reaching the fruit with "θ⋆ verified"; failure exits "unreachable / no feasible."
- **Labels:** "grasp pose," "analytic where possible, numeric where needed," "verify before move," "Reach the Fruit."
- **Form:** SVG. Plus the flagship interactive demo below.
