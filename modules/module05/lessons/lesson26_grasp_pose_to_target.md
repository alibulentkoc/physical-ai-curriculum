...
unit: 07
lesson: 7.2
title: "From a Fruit's Grasp Pose to a Target Configuration"
core_idea: "A perceived fruit gives a grasp pose in the arm's base frame (via the Module 3 → Module 4 bridge). Inverse kinematics turns that pose into joint angles. This lesson connects the perception output to the IK input."
estimated_time: 45
difficulty: Core
prerequisites: [7.1]
learning_objectives:
  - Trace a fruit's grasp pose from perception into the arm's base frame.
  - Feed a base-frame target pose into the inverse-kinematics solver.
  - Identify the tolerances and frames that must line up end to end.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: the hand-off chain — camera detects fruit (p_cam) → T_base^cam → base-frame grasp pose → IK → joint angles]`

**Diagram Specification**
- **Objective:** show perception output becoming IK input through the transform chain.
- **Scene:** left a camera viewing a fruit with "p_cam"; arrow through a box "T_base^cam (mounting / extrinsics)"; to a base-frame target "grasp pose T_target^base (position + approach R_g)"; arrow into "inverse kinematics"; out to "joint angles θ."
- **Labels:** "camera frame," "base frame," "grasp pose," "frames + units + tolerances must align."
- **Form:** SVG.
