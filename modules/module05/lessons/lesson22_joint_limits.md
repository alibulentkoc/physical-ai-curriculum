...
unit: 06
lesson: 6.2
title: "Joint Limits and Feasible Solutions"
core_idea: "Real joints cannot rotate freely. A mathematically valid IK solution is only useful if every joint angle lies within its limits — so the solver must filter solutions for feasibility, which can rule out elbow-up, elbow-down, or both."
estimated_time: 40
difficulty: Core
prerequisites: [6.1]
learning_objectives:
  - Explain why joint limits constrain which IK solutions are usable.
  - Filter the 2-link arm's solutions against joint limits.
  - Describe how joint limits shrink the effective workspace.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: a target with its two 2-link solutions; elbow-up violates a joint limit (greyed, ✗), elbow-down is within limits (✓); a note that limits carve the reachable annulus]`

**Diagram Specification**
- **Objective:** show feasibility filtering of the two solutions.
- **Scene:** a target inside the annulus with both arm solutions drawn; the elbow-up arm greyed with a "✗ joint limit" tag, the elbow-down arm solid with a "✓ feasible" tag; a small inset annulus with a wedge removed labeled "effective workspace (limits applied)."
- **Labels:** "θᵢ ∈ [min, max]," "elbow-up ✗," "elbow-down ✓," "limits shrink the workspace."
- **Form:** SVG.
