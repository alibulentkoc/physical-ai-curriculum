...
unit: 05
lesson: 5.2
title: "The Jacobian-Transpose and Damped Least Squares"
core_idea: "When the pseudoinverse misbehaves (near ill-conditioned configurations), two robust steps help: the Jacobian-transpose step (cheap, always stable, slower) and damped least squares (a tunable blend that stays bounded everywhere)."
estimated_time: 45
difficulty: Core
prerequisites: [5.1]
learning_objectives:
  - State the Jacobian-transpose update and why it is always stable.
  - State the damped-least-squares update and the role of the damping factor.
  - Explain when to prefer each over the bare pseudoinverse.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: three step rules compared near an ill-conditioned target — pseudoinverse (huge overshoot), transpose (small safe step), damped (bounded step toward target)]`

**Diagram Specification**
- **Objective:** contrast the three updates near a hard configuration.
- **Scene:** a target near full arm extension; three correction arrows from the current gripper — a wild long "pseudoinverse" arrow overshooting, a short cautious "transpose" arrow, a moderate "damped least squares" arrow aimed at the target; a small caption with each formula.
- **Labels:** "J⁺e: fast but unbounded," "Jᵀe: safe, slow," "Jᵀ(JJᵀ+λ²I)⁻¹e: bounded, tunable."
- **Form:** SVG.
