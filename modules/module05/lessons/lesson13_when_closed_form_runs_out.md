...
unit: 04
lesson: 4.1
title: When Closed Form Runs Out
core_idea: "Closed-form inverse kinematics exists only for arms whose structure separates into trig-solvable pieces. General geometry, redundancy, and orientation coupling break that, so we need a method that works for any arm — numerical iteration."
estimated_time: 40
difficulty: Core
prerequisites: [3.4]
learning_objectives:
  - Identify when an arm has no closed-form inverse kinematics.
  - Explain how redundancy produces infinitely many solutions.
  - Motivate numerical iteration as the general-purpose alternative.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: two arms side by side — a 2-link arm with two clean solutions (closed form OK), and a redundant multi-joint arm with several poses reaching one target (closed form fails → numerical)]`

**Diagram Specification**
- **Objective:** contrast a closed-form-solvable arm with one that needs numerical IK.
- **Scene:** left — a 2-link arm with elbow-up/down through a target, captioned "structure → closed form, 2 solutions"; right — a 4-or-more-joint arm drawn in three different configurations all reaching the same target, captioned "redundant / general → no closed form, ∞ solutions → numerical."
- **Labels:** "clean triangle," "solution manifold," "evaluate + differentiate f(θ)," "iterate to a solution."
- **Form:** SVG.
