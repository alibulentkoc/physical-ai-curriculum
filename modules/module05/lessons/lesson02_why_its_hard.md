...
unit: 01
lesson: 1.2
title: "Why It's Hard: Nonlinear, and 0/1/Many Solutions"
core_idea: "Inverse kinematics is hard because the forward map is nonlinear in the joint angles, so the inverse equation can have no solution, exactly one, or many — and which case you are in depends on the target."
estimated_time: 40
difficulty: Core
prerequisites: [1.1]
learning_objectives:
  - Explain why the forward map is nonlinear in the joint angles.
  - Describe the three solution cases (none, one, many) and when each occurs.
  - Name the elbow-up / elbow-down pair as the canonical "many" case.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: three planar targets — one outside the reach annulus (0 solutions), one on the outer boundary (1 solution, straight arm), one inside (2 solutions drawn as elbow-up and elbow-down)]`

**Diagram Specification**
- **Objective:** show how the solution count depends on where the target is.
- **Scene:** a 2-link arm based at the origin; the reachable annulus (outer radius $L_1+L_2$, inner radius $|L_1-L_2|$) drawn faintly; three target dots — outside (red, "0"), on the outer circle (amber, "1", arm straight), inside (green, "2") with two arm poses through it labeled elbow-up / elbow-down.
- **Labels:** "0 solutions (unreachable)," "1 solution (boundary)," "2 solutions (elbow-up / elbow-down)," "reachable annulus."
- **Form:** SVG.
