...
unit: 03
lesson: 3.1
title: Closed-Form Solution of the 2-Link Arm
core_idea: "The planar 2-link arm's inverse kinematics can be written as an explicit pair of formulas — both elbow solutions in closed form — so the joint angles are computed directly, with no search or iteration."
estimated_time: 45
difficulty: Core
prerequisites: [2.4]
learning_objectives:
  - Write the full closed-form inverse-kinematics formulas for the planar 2-link arm.
  - Produce both elbow-up and elbow-down solutions from the formulas.
  - Verify a closed-form solution by forward kinematics.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: a flow from target (x,y) through the two closed-form steps — compute θ2 from the reach (±), then θ1 from the direction — producing the two arm configurations]`

**Diagram Specification**
- **Objective:** present the closed form as a two-step pipeline yielding both solutions.
- **Scene:** a left "target (x, y)" node; arrow to a box "θ₂ = ±arccos((x²+y²−L₁²−L₂²)/(2L₁L₂))"; arrow to a box "θ₁ = atan2(y,x) − atan2(L₂sinθ₂, L₁+L₂cosθ₂)"; two output arms (elbow-up, elbow-down) on the right reaching the same target.
- **Labels:** "reach → elbow," "direction → shoulder," "± → two solutions," "unreachable if cosθ₂ ∉ [−1,1]."
- **Form:** SVG.
