...
unit: 07
lesson: 7.3
title: "Closing the Loop: Perceive → Place → Solve"
core_idea: "Assemble the full chain into one function: perceive the fruit, place its pose in the base frame, solve IK, verify with FK, and select a feasible configuration — a single trustworthy perceive-to-reach pipeline."
estimated_time: 45
difficulty: Core
prerequisites: [7.2]
learning_objectives:
  - Assemble perception, transforms, IK, and verification into one pipeline.
  - Handle the accept/reject and no-solution outcomes end to end.
  - Trace a fruit from pixels to a verified, executable configuration.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: the full pipeline as labeled stages — perceive → place (base frame) → reachability gate → solve → filter limits → verify FK → select → configuration, with failure exits at each gate]`

**Diagram Specification**
- **Objective:** show the end-to-end perceive-to-reach pipeline with failure exits.
- **Scene:** a left-to-right chain of boxes: "perceive (p_cam)" → "place (p_base, grasp pose)" → "reachable? " → "solve (analytic/numeric)" → "filter limits" → "verify FK" → "select" → "configuration ✓"; downward red exits labeled "no detection," "unreachable," "no feasible solution," "all rejected."
- **Labels:** "pixels → verified θ," "each gate can fail cleanly," "nothing unverified passes."
- **Form:** SVG.
