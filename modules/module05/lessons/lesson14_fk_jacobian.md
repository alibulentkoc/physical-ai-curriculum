...
unit: 04
lesson: 4.2
title: "The Local Linear Map: the FK Jacobian (for Solving)"
core_idea: "The Jacobian is the matrix of partial derivatives of the forward map — it says how a small change in joint angles moves the gripper. In this module it is used purely as the local linear map that powers the numerical solver; its velocity meaning and singularity theory are Module 6."
estimated_time: 45
difficulty: Core
prerequisites: [4.1]
learning_objectives:
  - Define the FK Jacobian as the matrix of partial derivatives of the forward map.
  - Interpret it as the local linear map from joint changes to gripper changes.
  - Compute the Jacobian for the planar 2-link arm.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: a 2-link arm with two small arrows at the gripper — one showing the gripper motion from a small Δθ1, one from a small Δθ2 — labeled as the two columns of J]`

**Diagram Specification**
- **Objective:** show the Jacobian columns as gripper motions from each joint twitch.
- **Scene:** a 2-link arm at a sample pose; at the gripper, a small arrow $\partial\mathbf p/\partial\theta_1$ (from twitching $\theta_1$) and another $\partial\mathbf p/\partial\theta_2$ (from twitching $\theta_2$); a caption "Δp ≈ J Δθ (local linear map)."
- **Labels:** "column 1 = ∂p/∂θ₁," "column 2 = ∂p/∂θ₂," "valid for small moves," "used by the solver."
- **Form:** SVG.
