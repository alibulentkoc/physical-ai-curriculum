...
unit: 05
lesson: 5.1
title: "Newton's Method for IK (Jacobian Pseudoinverse)"
core_idea: "Newton's method for inverse kinematics steps the joints by the Jacobian pseudoinverse times the gripper error. It converges fast near a solution and is the standard numerical IK step — the guess–measure–step loop made precise."
estimated_time: 45
difficulty: Core
prerequisites: [4.4]
learning_objectives:
  - State the Newton/pseudoinverse step for inverse kinematics.
  - Explain why the pseudoinverse handles non-square Jacobians.
  - Watch convergence as the error shrinks each iteration.
tags:
  - physical-ai
  - kinematics
  - inverse-kinematics
...
`[Visual: convergence plot — error norm vs iteration dropping steeply to tolerance — beside an arm walking onto the target in 3–4 steps]`

**Diagram Specification**
- **Objective:** show fast (quadratic-ish) convergence of the Newton step.
- **Scene:** left — a small log-scale-feel plot of $\|\mathbf e\|$ vs iteration, dropping steeply then flattening at `tol`; right — the gripper at iterations 0,1,2,3 closing onto the target, error arrows shrinking fast.
- **Labels:** "θ ← θ + α J⁺ e," "error ≈ squares each step," "stop at tol," "fast near a solution."
- **Form:** SVG. Plus the interactive convergence stepper below.
