...
unit: 06
lesson: 6.2
title: "Lookahead and What-If"
core_idea: "Run-ahead becomes a decision tool when you run it more than once. Lookahead means forecasting far enough to see a problem coming; what-if means forecasting several candidate scenarios and comparing their outcomes before committing to any. Want to know whether an obstacle will cost you a fruit, or which of two situations harvests more? Simulate each in the twin and compare. Because every run is safe and reproducible, you can explore many possible futures and pick the most informative — all by running the existing system, never a learned predictor."
estimated_time: "50 min"
difficulty: "Advanced"
prerequisites:
  - "6.1 — prediction as run-ahead simulation"
  - "3.2 — the sandbox (what-if injection)"
learning_objectives:
  - "Use lookahead to foresee an outcome before it happens."
  - "Compare candidate futures with what-if simulation."
  - "Explain why lookahead/what-if is run-ahead, not learned prediction."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: a current "now" state branching into several what-if run-aheads (nominal · obstacle · disturbance), each producing a forecast outcome, with a comparison panel ranking them (e.g. nominal harvests all; obstacle skips F3); a note "many run-aheads → compare futures → the most informative one".]`

**Diagram Specification**
- **Objective:** show multiple what-if run-aheads from one state, compared.
- **Scene:** "now" branching into nominal/obstacle/disturbance run-aheads → forecast outcomes → comparison panel.
- **Labels:** "now (synced)", "what-if scenarios", "forecast outcomes", "compare futures", "lookahead spots the problem".
- **Form:** SVG.
