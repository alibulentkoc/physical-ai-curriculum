...
unit: 08
lesson: 8.2
title: "Capstone — The Self-Improving Greenhouse Harvest"
core_idea: "The capstone runs the whole module at once. The real harvester and its twin run together down a row; the twin pre-validates each pick, predicts a failure before it happens, and the system adapts (skip or re-order) so the real harvest comes out better than the naive plan would. 'Self-improving' means the twin-in-the-loop improves the real outcome — not that anything is learned. It demonstrates the full spine Mirror → Simulate → Monitor → Predict → Adapt on one harvest, using only the existing system."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "8.1 — assembling the full system"
  - "7.3 — the twin-in-the-loop cycle"
learning_objectives:
  - "Run the full twin-in-the-loop spine on a complete harvest."
  - "Show how pre-validation + adaptation produce a better real outcome than the naive plan."
  - "Explain why 'self-improving' here means loop-improved, not learned."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: two harvests side by side down the same row: 'blind plan' wastes attempts on a blocked fruit and skips it late; 'twin-in-the-loop' pre-validates, foresees the block, adapts (skip/re-order), and finishes with more fruit and less waste; the spine Mirror/Simulate/Monitor/Predict/Adapt annotated along the loop run]`

**Diagram Specification**
- **Objective:** show the capstone: twin-in-the-loop harvest beating the blind plan via pre-validation + adaptation.
- **Scene:** split comparison of blind vs twin-in-the-loop harvest on one row; ledger of fruit harvested/skipped/effort; spine annotated.
- **Labels:** "blind plan", "twin-in-the-loop", "pre-validate", "predict failure", "adapt", "better real harvest".
- **Form:** SVG.
