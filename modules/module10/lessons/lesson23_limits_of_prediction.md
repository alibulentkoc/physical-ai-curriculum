...
core_idea: "Every forecast the twin makes is only as faithful as the twin itself. A prediction is a simulation, so it inherits the sim-to-real gap: where the twin doesn't model an effect, its forecast can be confidently wrong. The discipline is calibrated confidence — trust a forecast where the twin is faithful and the monitored gap is small, distrust it where the gap is large or the situation exercises unmodeled effects. Prediction is powerful precisely when paired with the humility the first half of the module taught."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "6.1–6.2 — prediction and what-if"
  - "4.1–4.3 — the sim-to-real gap, measured and calibrated"
learning_objectives:
  - "Explain why predictions inherit the sim-to-real gap."
  - "Apply calibrated confidence: trust a forecast in proportion to twin fidelity."
  - "Use monitoring to bound how much a forecast should be trusted."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: a forecast outcome with a confidence band tied to the monitored gap — small gap → tight band ("trust"), large gap → wide band ("doubt"); an optimistic forecast flagged with extra caution; a note "a forecast is only as faithful as the twin — weight it by the measured gap".]`

**Diagram Specification**
- **Objective:** show forecast trust scaled by the monitored sim-to-real gap, with optimistic forecasts flagged.
- **Scene:** forecast with a confidence band that widens as the monitored gap grows; an "optimistic — extra caution" flag.
- **Labels:** "forecast", "small gap → trust", "large gap → doubt", "optimistic forecast = riskier", "calibrated confidence".
- **Form:** SVG.
