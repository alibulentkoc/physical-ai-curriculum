...
unit: 07
lesson: 7.1
title: "Pre-Validating Actions in the Twin"
core_idea: "Adaptation begins with a habit: before committing an action in reality, run it forward in the twin and look at the predicted outcome. Pre-validation is rehearsing the action in the twin first — if the twin predicts the action fails, you do not commit it to the real robot. This is not learning and not optimization; it is running the existing system on a candidate action and reading the result."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "6.1 — prediction as run-ahead simulation"
  - "6.2 — lookahead and what-if"
learning_objectives:
  - "Define pre-validation as running a candidate action forward in the twin before committing it."
  - "Use a simple, explained acceptance test to accept or reject the predicted outcome."
  - "Explain why pre-validation is not machine learning, RL, or optimization."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: a candidate action entering a 'twin rehearsal' box (run-ahead), producing a predicted outcome, which passes through a stated accept/reject gate; an accepted action flows on to the real robot, a rejected one is stopped before reality]`

**Diagram Specification**
- **Objective:** show pre-validation as rehearse-in-twin then accept/reject before committing to reality.
- **Scene:** candidate action → twin rehearsal (run-ahead) → predicted outcome → accept/reject gate → (accept) real robot / (reject) stop.
- **Labels:** "candidate action", "rehearse in twin", "predicted outcome", "accept / reject", "commit to reality".
- **Form:** SVG.
