...
unit: 02
lesson: 2.2
title: "Synchronizing Twin ↔ Real"
core_idea: "A twin becomes live by synchronization: each cycle, it copies the real system's reported state into its own, driving the gap between them to zero. Sync is the heartbeat that makes a static replica a twin. It reuses the Module 9 interface — read the report, write the twin's state — and adds no new theory. After a sync the twin mirrors the reported state exactly; between syncs it can drift, which is why the mirror must be kept live."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "2.1 — The twin's state frame"
learning_objectives:
  - "Define synchronization as copying the reported state into the twin to drive the gap to zero."
  - "Run a live sync loop over a sequence of real states."
  - "Explain why sync is what makes a replica a twin."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: a loop — real robot reports state → sync writes it into the twin → divergence meter reads ~0 → real robot acts again → divergence grows → next sync resets it to ~0; a sawtooth "gap vs time" inset (climbs between syncs, drops to 0 at each sync); a note "sync = the twin's heartbeat".]`

**Diagram Specification**
- **Objective:** show the live sync loop driving the gap to zero each cycle, with a sawtooth gap-vs-time.
- **Scene:** report → sync → divergence ~0 → act → gap grows → sync; a sawtooth inset.
- **Labels:** "report", "sync (copy in)", "gap → 0", "robot acts → gap grows", "sync = heartbeat".
- **Form:** SVG.
