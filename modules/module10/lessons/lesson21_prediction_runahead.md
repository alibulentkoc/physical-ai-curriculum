...
unit: 06
lesson: 6.1
title: "Prediction as Run-Ahead Simulation"
core_idea: "A predictor answers 'what is likely to happen next?' — and the twin answers it by running ahead. Sync the twin to the current real state, then simulate the system forward to see how the harvest is likely to unfold. Prediction is not a learned model, a statistical forecast, or adaptive control; it is run-ahead simulation: executing the existing Module 9 system inside the twin, from now into the near future. The same model that monitoring compares against, prediction runs forward."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "3.1 — simulation (running the system forward in the twin)"
  - "5.4 — monitoring answers 'now'; prediction answers 'next'"
learning_objectives:
  - "Define prediction as run-ahead simulation from the current synced state."
  - "Distinguish prediction (run-ahead) from learned/statistical forecasting."
  - "Explain how prediction reuses the same model monitoring compares against."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: reality's current state being synced into the twin (a "now" marker), then an arrow running the twin forward through the pick cycle to a forecast outcome (a "next" marker) — harvested/skipped likely; reality shown untouched; a banner "predict: what is likely to happen next?".]`

**Diagram Specification**
- **Objective:** show prediction as sync-to-now then run-ahead simulation to a forecast.
- **Scene:** reality "now" → sync into twin → run forward → forecast "next"; reality untouched.
- **Labels:** "now (sync to reality)", "run the system ahead", "forecast (next)", "what is likely to happen next?".
- **Form:** SVG.
