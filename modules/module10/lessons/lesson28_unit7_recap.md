...
unit: 07
lesson: 7.4
title: "Unit 7 Recap: Adaptation and the Twin-in-the-Loop"
core_idea: "Unit 7 added the final verb of the spine — Adapt — and closed the loop. Adaptation is pre-validation (rehearse an action in the twin) plus action-selection (rank candidates' forecasts and choose the better one), assembled into the twin-in-the-loop cycle. None of it is machine learning, RL, adaptive control, or optimization; it is the existing system, run inside the twin, used to decide. With monitoring, prediction, and adaptation in place, every stage of Mirror → Simulate → Monitor → Predict → Adapt now exists."
...
learning_objectives:
  - "Consolidate adaptation as pre-validation plus action-selection in the twin-in-the-loop cycle."
  - "Restate the scope boundary: no ML, RL, adaptive control, or optimization."
  - "Confirm the full spine Mirror → Simulate → Monitor → Predict → Adapt is now complete."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: the full spine Mirror → Simulate → Monitor → Predict → Adapt drawn as a labeled ribbon, with Unit 7's Adapt stage expanded into 'pre-validate' + 'choose' feeding the closed loop; a scope banner 'no ML / RL / adaptive control / optimization']`

**Diagram Specification**
- **Objective:** summarize Unit 7: adaptation = pre-validate + choose, closing the loop, completing the spine.
- **Scene:** spine ribbon with Adapt expanded into pre-validate + choose; closed loop ring; scope-boundary banner.
- **Labels:** "Mirror", "Simulate", "Monitor", "Predict", "Adapt = pre-validate + choose", "no ML/RL/optimization".
- **Form:** SVG.
