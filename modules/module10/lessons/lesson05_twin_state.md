...
unit: 02
lesson: 2.1
title: "The Twin's State: Mirroring the World-State Model"
core_idea: "For a twin to mirror the real robot, it must hold the same state in the same representation. The Module 9 system reports a world-state — the arm configuration, the tool position, the fruit and their statuses, the health signals, the pipeline stage — and the twin keeps a copyable frame of exactly that. The mirror is only as good as the state it copies: choosing the right state to represent (and nothing the robot doesn't expose) is the first step to building a faithful twin."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "1.4 — Unit 1 recap (the twin wraps the M9 world-state)"
  - "Module 9 — the WorldState / reported snapshot"
learning_objectives:
  - "Identify the world-state the real system reports and the twin must mirror."
  - "Capture a copyable state frame (snapshot) of the reported state."
  - "Explain why the twin mirrors only what the system actually reports."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: the real robot emitting a structured "snapshot" frame — boxes for q, tool_xy, fruit{ripe,picked}, health, stage — flowing into the twin's matching state frame; a note "same representation · copyable · reported-only".]`

**Diagram Specification**
- **Objective:** show the reported world-state captured as a copyable frame the twin holds.
- **Scene:** real robot → snapshot frame (q, tool, fruit, health, stage) → twin's matching state.
- **Labels:** "q", "tool_xy", "fruit {ripe, picked}", "health", "stage", "same representation · reported-only".
- **Form:** SVG.
