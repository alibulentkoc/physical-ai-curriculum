...
unit: 08
lesson: 8.1
title: "Assembling the Full Twin-in-the-Loop System"
core_idea: "Before the capstone, assemble every Module 10 capability into one running system: a real harvester (the asset), its synced twin (the mirror), and the cycle that monitors, predicts, and adapts. This lesson is the integration step — it wires sync, simulate, monitor, predict, and adapt together over a full row, so the capstone can simply press play. Nothing new is introduced; the parts are connected."
...
prerequisites:
  ...
  - "6.1 — prediction"
learning_objectives:
  - "Wire sync, simulate, monitor, predict, and adapt into one running system over a row."
  - "Trace data flow between reality (the asset) and the twin (the advisor) each cycle."
  - "Confirm the assembled system reuses only existing, verified parts."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: an architecture diagram: a 'reality' block (GroundTruth, the asset) and a 'twin' block (DigitalTwin, the advisor) separated by a dashed boundary; two arrows cross it (report in, action out); inside the twin side, the cycle stages monitor/predict/adapt; a row of fruit being processed one per cycle]`

**Diagram Specification**
- **Objective:** show the assembled twin-in-the-loop harvester with an explicit reality/twin boundary.
- **Scene:** reality block + twin block split by a dashed line; report-in and action-out arrows; cycle stages on the twin side; a row of fruit processed per cycle.
- **Labels:** "reality (asset)", "twin (advisor)", "report in", "action out", "monitor/predict/adapt", "row of fruit".
- **Form:** SVG.
