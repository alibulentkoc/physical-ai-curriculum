...
unit: 06
lesson: 6.4
title: "Unit 6 Recap: Prediction with the Twin"
core_idea: "Unit 6 gave the twin foresight. Prediction is run-ahead simulation from the current synced state — answering 'what is likely to happen next?' with the same model monitoring compares against; lookahead and what-if run it forward many times to foresee events and compare candidate futures; and the limits of prediction keep it honest — a forecast is only as faithful as the twin, so trust it with calibrated confidence. Together with Unit 5, the twin now answers both 'what is happening now?' and 'what is likely to happen next?' from one model. The last step, Unit 7, closes the loop: act on the answers."
estimated_time: "30 min"
difficulty: "Advanced"
prerequisites:
  - "6.1–6.3 — run-ahead, what-if, limits"
learning_objectives:
  - "Consolidate prediction as run-ahead, what-if comparison, and calibrated confidence."
  - "State the now/next symmetry between monitoring and prediction."
  - "Set up adaptation: acting on what the twin foresees."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: the spine with Model · Mirror · Simulate · Monitor · Predict lit (Predict newly added) and Adapt greyed; a two-question panel — "now? (monitor: compare)" and "next? (predict: run forward)" — both pointing to one twin model; an arrow toward a greyed "act on it → Unit 7".]`

**Diagram Specification**
- **Objective:** consolidate Unit 6 on the spine and show the now/next symmetry from one model.
- **Scene:** spine (Predict newly lit) + two-question panel (now: compare / next: run-forward) → one twin model; arrow to Unit 7.
- **Labels:** "Predict ✓", "now? (monitor)", "next? (predict)", "same model", "act on it → Unit 7".
- **Form:** SVG.
