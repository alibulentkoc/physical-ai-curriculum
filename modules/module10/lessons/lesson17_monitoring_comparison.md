...
unit: 05
lesson: 5.1
title: "Monitoring as Reality ↔ Twin Comparison"
core_idea: "A monitor answers one question: what is happening now? With a twin, you answer it by comparison — hold the twin's mirrored state beside reality's live report and look at the difference. When they agree, the system is behaving as expected; when they diverge, something is happening that the twin didn't account for. Monitoring is not a new sensor or estimator — it is the Reality ↔ Twin comparison, with divergence as the signal."
estimated_time: "45 min"
difficulty: "Advanced"
prerequisites:
  - "2.2 — synchronizing twin ↔ real"
  - "2.3 — sync error (the state divergence that monitoring reads)"
learning_objectives:
  - "Frame monitoring as a Reality ↔ Twin comparison answering 'what is happening now?'"
  - "Use state divergence between twin and reality as the monitoring signal."
  - "Explain why monitoring needs no new estimation theory."
tags:
  - physical-ai
  - robotics
  - digital-twin
...
`[Visual: reality's live report and the twin's mirrored state side by side feeding a "compare" node that outputs a divergence reading; a quiet/green state ("in sync — nothing notable") vs an alert/amber state ("diverged — something is happening"); a banner "monitor: what is happening now?".]`

**Diagram Specification**
- **Objective:** show monitoring as a Reality ↔ Twin comparison producing a divergence signal.
- **Scene:** reality report + twin state → compare → divergence; two states (in-sync quiet vs diverged alert).
- **Labels:** "reality (live report)", "twin (mirrored state)", "compare", "divergence = signal", "what is happening now?".
- **Form:** SVG.
