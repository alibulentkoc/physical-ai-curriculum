...
unit: 02
lesson: 2.5
title: Homogeneous Coordinates in Physical AI — Unit 2 Recap
core_idea: "One extra coordinate turns translation into a matrix, so rotation and translation combine into a single homogeneous transform — the representation robots use."
estimated_time: 20
difficulty: Review
prerequisites: [2.1, 2.2, 2.3, 2.4]
learning_objectives:
  - Consolidate homogeneous coordinates: points (w=1), directions (w=0), translation and rotation as one matrix.
  - Explain why this representation solves the robot's constant problem.
  - Bridge to SE(2)/SE(3) as the named families of these transforms.
tags:
  - physical-ai
  - transformations
  - homogeneous
...
`[Visual: the four pieces of Unit 2 — (x,y,1), w=1 vs 0, the translation matrix, and the combined block form — leading to one homogeneous transform and SE(2)/SE(3)]`

**Diagram Specification**
- **Objective:** consolidate Unit 2 — lift to (x,y,1), points vs directions, translation as a matrix, combined block form.
- **Scene:** four small panels ((x,y,1); w=1 vs w=0; T(tx,ty); [[R,t],[0,1]]) feeding into "one homogeneous transform → SE(2), SE(3)."
