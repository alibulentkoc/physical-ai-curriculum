...
unit: 04
lesson: 4.2
title: "Position Interpolation and the IK-per-Sample Loop"
core_idea: "A Cartesian position trajectory is realized by sampling the straight-line tool path densely in time and solving inverse kinematics at each sample. The IK branch must be chosen consistently so the joints don't flip mid-path, and every sample must be reachable for the motion to be feasible."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L4.1 — Why Cartesian space; the interpolate-then-IK recipe"
  - "M5 — Inverse kinematics and solution branches (elbow up/down)"
learning_objectives:
  - "Implement a Cartesian straight-line position trajectory via the interpolate-then-IK loop."
  - "Explain why IK branch consistency matters and how a branch flip corrupts the joint motion."
  - "Identify reachability of every sampled point as the feasibility condition for the path."
---
...
`[Visual: a straight tool line sampled in time; per sample the arm is drawn — top row branch-consistent (smooth joints), bottom row a branch flip (the arm convulses mid-path)]`

**Diagram Specification**

- **Objective:** show that the same straight tool path can give smooth or convulsing joint motion depending on IK branch consistency.
- **Scene:** two rows over the same straight tool line with sampled tool dots. **Top:** arm poses along the line, elbow staying up — joints sweep smoothly (green "consistent branch"). **Bottom:** elbow flips at the midpoint — two arm poses straddling the flip drawn in red, with a "branch flip → joint jump" marker; a small $q$-vs-time inset shows the discontinuity.
- **Labels:** "consistent branch (smooth)" vs "branch flip (joint jump)"; reachable annulus boundary faint.
- **Form:** SVG, 2 rows. Smooth emerald `#10b981`; flip error `#b91c1c`; arm ink; tool line teal.
