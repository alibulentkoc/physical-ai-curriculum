...
unit: 07
lesson: 7.1
title: "What Makes a Trajectory Good? Quality Metrics"
core_idea: "Among the many feasible trajectories for a task, some are better than others. Quality is measured by concrete, geometric-temporal metrics — duration, peak speed and acceleration, peak jerk (smoothness), and tool path length — that let you compare and choose. These are proxies for 'good motion,' never energy or dynamics."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L2.4 — Jerk and the smooth-vs-fast trade"
  - "M7 L5.3 — The fastest feasible timing"
learning_objectives:
  - "Name the quality metrics that distinguish good trajectories: duration, peak speed/acceleration, jerk, path length."
  - "Compare two feasible trajectories using these metrics and explain the trade-offs."
  - "Recognize these as geometric-temporal proxies, not energy or dynamics measures."
---
...
`[Visual: two feasible trajectories to the same goal compared on a small metrics panel — duration, peak speed, peak acceleration, peak jerk, path length — one smoother/slower, one faster/jerkier]`

**Diagram Specification**

- **Objective:** show quality as a comparison across metrics, exposing the trade-offs.
- **Scene:** two trajectories (A = smooth/quintic, B = fast/trapezoidal) to the same goal, each with a small bar row for the five metrics (duration, peak speed, peak accel, peak jerk, path length), drawn so A wins on jerk/smoothness and B wins on duration. A "priority" caption notes the choice depends on weighting.
- **Labels:** "A: smooth (low jerk, longer)", "B: fast (short, high jerk)"; metric names; "best depends on priority".
- **Form:** SVG, two metric bar-rows. A in teal `#0d9488`, B in violet `#8b5cf6`, neutral axes.
