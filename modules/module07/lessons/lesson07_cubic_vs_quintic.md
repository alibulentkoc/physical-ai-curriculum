...
unit: 02
lesson: 2.3
title: "Polynomial Time Scaling: Cubic vs Quintic Profile Shaper"
core_idea: "Polynomials solve point-to-point timing by matching boundary conditions: a cubic matches position+velocity at both ends (C1, zero endpoint velocity) but leaves an acceleration jump; a quintic adds zero endpoint acceleration (C2), erasing the jump. Each pair of boundary conditions buys one more rung of continuity."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L2.1 — s(t) and its derivatives"
  - "M7 L2.2 — Continuity classes and jerk"
learning_objectives:
  - "Derive cubic and quintic time scalings by matching boundary conditions."
  - "Show a cubic gives C1 with an endpoint acceleration jump and a quintic gives C2."
  - "Compute polynomial coefficients for given endpoint conditions and duration."
  - "Use the interactive Profile Shaper to see the quintic remove the acceleration discontinuity."
tags:
  - physical-ai
  - robotics
...
`[Visual: cubic vs quintic overlaid — position nearly identical, but the acceleration panel shows the cubic's nonzero endpoint values (jump) and the quintic's zero endpoints]`

**Diagram Specification**

- **Objective:** show that cubic and quintic look alike in position but differ decisively in acceleration at the endpoints.
- **Scene:** three stacked panels over $[0,T]$, cubic and quintic overlaid in each. Position: two nearly-coincident S-curves 0→1. Velocity: two bumps (quintic's peak slightly higher), both 0 at ends. Acceleration: **cubic** starts/ends at $\pm 6/T^2$ (mark the nonzero endpoints with red dots and the word "jump"); **quintic** starts/ends at 0 (green dots, "smooth").
- **Labels:** legend "cubic (C¹)" vs "quintic (C²)"; annotate the acceleration endpoints.
- **Form:** SVG. Cubic in error `#b91c1c`, quintic in act/emerald `#10b981`; ink axes; this is the same comparison the interactive animates.

**Interactive demo (this lesson's flagship):** *Polynomial Profile Shaper* — drag the duration $T$, toggle **cubic ↔ quintic**, and watch position/velocity/acceleration/**jerk** update live. The acceleration panel shows the cubic's endpoint jump snapping to zero when you switch to the quintic. (Embedded from `modules/module07/demos/lesson07_polynomial_profile_shaper.html`.)
