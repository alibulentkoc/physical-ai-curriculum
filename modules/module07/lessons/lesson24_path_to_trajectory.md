...
unit: 06
lesson: 6.4
title: "From Safe Path to Feasible Trajectory: Smoothing and Time Scaling"
core_idea: "An RRT path is collision-free but jagged and untimed. Two finishing steps turn it into an executable trajectory: shortcut-smooth it to remove the random zig-zags (keeping it collision-free), then time-scale it to respect velocity and acceleration limits. This completes the chain Obstacle → Constraint → Safe Path → Feasible Trajectory."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L6.3 — RRT produces a jagged collision-free path"
  - "M7 L5.2 — Time scaling for feasibility; M7 L3.4 — splines/blending"
learning_objectives:
  - "Smooth a jagged planned path by shortcutting while preserving collision-freedom."
  - "Time-scale a smoothed path into a trajectory that respects velocity and acceleration limits."
  - "Assemble the full pipeline Obstacle → Constraint → Safe Path → Feasible Trajectory."
---
...
`[Visual: the pipeline — jagged RRT path → shortcut-smoothed path (shorter, still collision-free) → time-scaled trajectory (velocity/accel under limits) — with the Obstacle→Constraint→Safe Path→Feasible Trajectory chain labeled]`

**Diagram Specification**

- **Objective:** show the jagged path becoming smooth then timed, and label the four-stage chain.
- **Scene:** three small C-space panels left to right: (1) jagged RRT path around the C-obstacle; (2) the same after shortcut smoothing — fewer, straighter segments, still avoiding the obstacle (a removed zig-zag faintly shown with a "shortcut" arrow); (3) a speed-vs-time inset showing the timed trajectory's peaks under the limit lines. Below, the labeled chain Obstacle → Constraint → Safe Path → Feasible Trajectory aligned under the stages.
- **Labels:** "RRT (jagged)" → "smoothed (shorter, collision-free)" → "time-scaled (within limits)"; the four-stage chain.
- **Form:** SVG, 3 panels + chain. Jagged path muted; smoothed emerald `#10b981`; obstacle error-tint; limit lines dashed.
