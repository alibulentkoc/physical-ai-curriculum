...
unit: 01
lesson: 1.4
title: "The Motion Pipeline: Plan → Parameterize → Execute → Track"
core_idea: "Robot motion is produced by a pipeline: plan a geometric path, time-parameterize it into a trajectory, execute it open-loop through the Module 6 velocity layer (to prove feasibility), and hand the validated reference to Module 8 to track with feedback. Module 7 owns planning and parameterization; it never closes a feedback loop."
estimated_time: "30 min"
difficulty: "Introductory"
prerequisites:
  - "M7 L1.1–1.3 — Motion quality, path vs trajectory, the four criteria"
  - "M6 — The velocity layer (D-060)"
learning_objectives:
  - "Draw the motion pipeline: plan → parameterize → velocity layer (M6) → track (M8)."
  - "State exactly which stages Module 7 owns and which it must not implement."
  - "Explain why Module 7 executes open-loop through M6 only to demonstrate feasibility, not to correct error."
  - "Describe the reference trajectory Module 7 hands to Module 8."
tags:
  - physical-ai
  - robotics
  - pipeline
...
`[Visual: the four-stage pipeline as a left-to-right flow, with a coloured box around the two-and-a-half stages Module 7 owns and a clearly separated Module 8 box for tracking]`

**Diagram Specification**

- **Objective:** make the pipeline and the module ownership unmistakable, especially the M7/M8 fence.
- **Scene:** four stage boxes left→right: **Plan** (path), **Parameterize** (trajectory), **Execute open-loop via M6 velocity layer**, **Track with feedback (M8)**. A solid arrow runs through all four (primary flow). Draw a labelled bracket "Module 7" enclosing Plan, Parameterize, and the open-loop Execute box; a separate bracket "Module 8" around Track. Show a *dashed* feedback arrow looping back **only inside the Module 8 box** (the loop M7 never closes).
- **Labels:** stage names; "geometry," "geometry + timing," "open-loop, no correction," "closes the loop"; the two brackets; "reference trajectory q_ref, q̇_ref, q̈_ref" on the arrow leaving Module 7.
- **Form:** Mermaid flow is acceptable here (the structure reads as a graph), plus the SVG version. Stage colours: plan/reason violet `#8b5cf6`, parameterize teal `#0d9488`, execute act/emerald `#10b981`, track (M8) muted/slate to mark it out-of-scope; dashed feedback arrow in error `#b91c1c` to signal "not M7."
