...
unit: 03
lesson: 3.3
title: "Via-Points and Multi-Segment Joint Trajectories"
core_idea: "A via-point is an intermediate configuration the trajectory must pass through on the way to the goal. Stringing point-to-point segments through via-points lets a path route around obstacles, but stopping at every via-point is wasteful — the real design choice is whether to stop at or flow through each via-point."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L3.1 — Per-joint polynomial moves"
  - "M7 L3.2 — Synchronizing multiple joints"
learning_objectives:
  - "Define a via-point and explain why multi-segment trajectories are needed."
  - "Build a multi-segment joint trajectory that passes through one or more via-points."
  - "Contrast stop-at-each-via-point with flow-through, and identify the continuity cost of stopping."
---
...
`[Visual: a joint-space path through two via-points — stop-at-each (speed dips to zero at each via, stutter) vs flow-through (speed stays positive, smooth glide)]`

**Diagram Specification**

- **Objective:** contrast stopping at via-points with flowing through them.
- **Scene:** top = the $(q_1,q_2)$ joint plane with start, two via-points, goal, and the routed path. Bottom = a speed-vs-time plot for the route: **stop-at-each** curve dips to zero at each via-point (marked "stop"); **flow-through** curve stays positive through the vias (marked "pass through").
- **Labels:** via-points numbered; "stops (slow, stutter)" vs "flows through (fast, smooth)".
- **Form:** SVG, 2 panels. Path ink; stop-at-each error `#b91c1c`; flow-through emerald `#10b981`; via-points violet dots.
