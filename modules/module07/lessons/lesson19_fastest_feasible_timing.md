...
unit: 05
lesson: 5.3
title: "The Fastest Feasible Timing: Respecting Velocity and Acceleration Limits"
core_idea: "The fastest a move can run is set by its binding limit: shrink the duration until either the velocity or the acceleration limit is hit, and stop there. The trapezoidal velocity profile realizes this 'as fast as the hardware allows' timing — accelerate at the limit, cruise at the speed limit, decelerate at the limit."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L5.2 — Uniform time scaling and the binding limit"
  - "M7 L2.4 — Trapezoidal vs S-curve velocity profiles"
learning_objectives:
  - "Find the minimum feasible duration of a move from its velocity and acceleration limits."
  - "Explain the trapezoidal profile as the fastest limit-respecting timing, and the triangular short-move case."
  - "Distinguish 'fastest feasible' (limit-respecting) from formal time-optimal trajectory optimization."
---
...
`[Visual: trapezoidal velocity profile (ramp up at a_max, cruise at v_max, ramp down) with the triangular short-move case inset; both saturate a limit at all times]`

**Diagram Specification**

- **Objective:** show the trapezoid as the fastest limit-saturating timing, and the triangular short-move variant.
- **Scene:** main panel = a trapezoidal speed-vs-time curve: ramp up (slope $a_{\max}$, labeled), flat top at $v_{\max}$ (labeled, dashed limit line), ramp down. Shade "accel-limited" on the ramps and "velocity-limited" on the cruise. Inset = a triangular profile (short move) that peaks below $v_{\max}$ at $v_p=\sqrt{a_{\max}D}$.
- **Labels:** "$a_{\max}$ ramps", "$v_{\max}$ cruise", "$T_{\min}$"; inset "short move: never reaches $v_{\max}$".
- **Form:** SVG. Profile teal `#0d9488`; limit lines muted dashed; saturated regions lightly shaded.
