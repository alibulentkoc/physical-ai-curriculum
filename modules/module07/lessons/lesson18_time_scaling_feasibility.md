...
unit: 05
lesson: 5.2
title: "Slowing Down to Restore Feasibility: Uniform Time Scaling"
core_idea: "Uniformly stretching a trajectory's duration shrinks every velocity by 1/T and every acceleration by 1/T², without changing the path. So slowing down can always make a velocity/acceleration-limited trajectory feasible — and the geometry is untouched, only the clock is rescaled."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L5.1 — Velocity and acceleration limits; peaks scale as 1/T and 1/T²"
  - "M7 L1.2 — Path vs trajectory (geometry vs timing)"
learning_objectives:
  - "Explain how uniform time scaling reduces velocity and acceleration peaks without changing the path."
  - "Compute the stretch factor needed to bring a trajectory within limits."
  - "Recognize slowing down as the universal fix for velocity/acceleration infeasibility."
---
...
`[Visual: same path, two timings — fast (peaks over the limit lines) and stretched (peaks under), with the velocity bar shrinking by 1/k and the acceleration bar by 1/k²]`

**Diagram Specification**

- **Objective:** show that stretching the duration slides the peaks under the limits without changing the path.
- **Scene:** left = the path/geometry once (a curve or arm sweep) labeled "path unchanged". Right = two bar-pairs: **fast** (peak speed and accel both over the limit lines, red) and **stretched ×k** (both under, green), with arrows showing speed shrinking by $1/k$ and acceleration by $1/k^2$.
- **Labels:** "path unchanged (same geometry)"; "× k slower → v÷k, a÷k²"; limit lines $\dot q_{\lim}$, $\ddot q_{\lim}$.
- **Form:** SVG. Over-limit error `#b91c1c`, under-limit emerald `#10b981`, path teal; limit lines muted dashed.
