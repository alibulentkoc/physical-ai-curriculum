...
unit: 07
lesson: 7.4
title: "Sampling and Representing the Reference: Discretization for Execution"
core_idea: "A continuous reference must be turned into something a digital controller can step through: a time-indexed series of (q_d, q̇_d, q̈_d) sampled at the control rate, or a function the controller evaluates at each tick. The sampling rate must be fine enough to capture the motion; too coarse a rate loses fidelity exactly as coarse collision sampling tunnels."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L7.3 — What the reference must provide"
  - "M7 L6.2 — Sampling density (tunneling)"
learning_objectives:
  - "Discretize a continuous reference into a time-indexed series at a control rate."
  - "Choose a sampling rate fine enough to represent the motion faithfully."
  - "Describe the reference data structure handed to execution, and recap Unit 7."
---
...
`[Visual: a smooth continuous reference curve overlaid with its discrete samples at the control rate — fine rate (faithful) vs coarse rate (slideshow, misses the peak) — plus the reference data-structure table (t, q_d, q̇_d, q̈_d)]`

**Diagram Specification**

- **Objective:** show discretization fidelity vs rate, and the resulting data structure.
- **Scene:** **Top:** a smooth reference curve (q vs t) with dense sample dots (fine rate) tracing it faithfully; a faint inset shows coarse samples that skip the peak ("slideshow — misses the motion"). **Bottom:** a small table with columns $t_k,\ \mathbf q_d,\ \dot{\mathbf q}_d,\ \ddot{\mathbf q}_d$ and a few rows, labeled "reference data structure (timestamped, validated, rate f)".
- **Labels:** "fine rate: faithful", "coarse rate: misses peaks (like tunneling)"; column headers; "duration T, rate f, validated".
- **Form:** SVG, curve+samples on top, table below. Curve teal `#0d9488`, fine samples emerald, coarse samples error `#b91c1c`.
