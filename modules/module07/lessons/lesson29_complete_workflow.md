...
unit: 08
lesson: 8.1
title: "The Complete Workflow: Plan → Parameterize → Validate → Execute"
core_idea: "The whole module assembles into one complete robotics workflow — Goal → Path → Trajectory → Validation → Execution — driven by four actions: PLAN a collision-free path, PARAMETERIZE it into a timed trajectory within limits, VALIDATE it against every check, and EXECUTE it as an open-loop reference. Each stage is a unit you've already built; the capstone is wiring them into a single, repeatable pipeline from goal to motion."
estimated_time: "50 min"
difficulty: "Advanced"
prerequisites:
  - "M7 Units 1-6 (planning, parameterization) and Unit 7 (validation, reference)"
learning_objectives:
  - "State the complete robotics workflow as the artifact chain Goal → Path → Trajectory → Validation → Execution, and the four actions (Plan, Parameterize, Validate, Execute) that produce it."
  - "State the four-stage workflow Plan → Parameterize → Validate → Execute and what each stage produces."
  - "Trace how every earlier unit slots into exactly one stage of the workflow."
  - "Use the Trajectory Studio to run the whole pipeline end to end and see each stage's output."
---
...
`[Visual: the complete robotics workflow as the artifact chain Goal → Path → Trajectory → Validation → Execution, with the four actions Plan / Parameterize / Validate / Execute on the arrows, each stage showing its output and source unit]`

**Diagram Specification**

- **Objective:** show the five-artifact chain (Goal → Path → Trajectory → Validation → Execution) with the four actions on the connecting arrows, each with its output and source unit.
- **Scene:** five labeled nodes left to right — **Goal** (the task: start/goal poses) · **Path** (a route around an obstacle in C-space; action "PLAN", "Unit 6") · **Trajectory** (the path with a velocity profile / timed curve; action "PARAMETERIZE", "Units 2–5") · **Validation** (a green checklist; action "VALIDATE", "Unit 7") · **Execution** (a reference signal crossing a handoff line to a greyed "Module 8 tracker"; action "EXECUTE", "open-loop reference"). Arrows carry the action verbs.
- **Labels:** the five artifact names; the four action verbs on arrows; per-stage output; source unit; "Execute = open-loop reference; tracking is Module 8 (dashed)".
- **Form:** SVG, five nodes + action-labeled arrows + dashed M8 box. Stage accents teal→sky→emerald→violet; M8 muted/dashed.

**Interactive demo (this lesson's flagship):** *Trajectory Studio* — set a start and goal for the greenhouse arm and place an obstacle, then run the pipeline stage by stage: **Plan** draws the RRT path, **Parameterize** times it (showing the velocity profile against the limit lines), **Validate** runs the checklist (green/red per check), and **Execute** plays the validated reference (the arm sweeps the motion) and displays the reference signal that would hand off to Module 8. Each stage's output is shown before the next runs. (Embedded from `modules/module07/demos/lesson29_trajectory_studio.html`.)
