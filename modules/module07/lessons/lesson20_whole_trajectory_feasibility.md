...
unit: 05
lesson: 5.4
title: "Feasibility Across the Whole Trajectory: Reachability, Limits, and the Workspace"
core_idea: "A trajectory is feasible only if it passes every check at every point: every configuration within joint limits, every velocity/acceleration within limits, and (for Cartesian moves) every tool point reachable. Slowing down fixes speed/acceleration violations, but a reachability or joint-limit violation is geometric and needs a different path."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L5.2 — Time scaling fixes velocity/acceleration violations"
  - "M7 L4.2 — Reachability along a Cartesian path"
learning_objectives:
  - "List the feasibility checks a trajectory must pass along its whole length."
  - "Distinguish timing-fixable violations (velocity/acceleration) from geometric ones (reachability, joint limits)."
  - "Apply a whole-trajectory feasibility check and choose the right remedy for each failure type."
---
...
`[Visual: a trajectory passing through four feasibility "gates" — joint limits, velocity, acceleration, reachability — with one failing gate flagged, and the remedy (slow down vs change path) labeled per gate type]`

**Diagram Specification**

- **Objective:** show feasibility as passing all checks at all points, and sort failures into timing-fixable vs geometric.
- **Scene:** a horizontal trajectory passing through four labeled gates: "joint limits", "velocity", "acceleration", "reachability". Two gates green (passed); one velocity gate flagged amber with "→ slow down (timing)"; one reachability gate flagged red with "→ change path (geometric)". A side legend maps gate type → remedy.
- **Labels:** gate names; "timing-fixable: slow down" (velocity/accel) vs "geometric: new path" (joint limits/reachability).
- **Form:** SVG, gate sequence + legend. Passed emerald `#10b981`, timing-fix amber/teal, geometric-fail error `#b91c1c`.
