...
unit: 03
lesson: 3.2
title: "Synchronizing Multiple Joints"
core_idea: "When several joints move together, they must share one clock so they start and finish as a single coordinated motion. The duration is set by the slowest-limited joint: find the minimum time each joint needs under its limits, take the maximum, and time-scale every joint to that common duration."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L3.1 — Per-joint polynomial moves"
  - "M7 L2.4 — Velocity profiles and limits"
learning_objectives:
  - "Explain why multi-joint moves use a single common time scaling."
  - "Find the synchronizing duration as the max over joints of each joint's minimum feasible time."
  - "Distinguish coordinated (synchronized) motion from independent per-joint motion."
---
...
`[Visual: two joints' angle-vs-time curves — top: independent (finish at different times); bottom: synchronized to a common T (finish together), with the bottleneck joint at its speed limit]`

**Diagram Specification**

- **Objective:** contrast independent finishing times with synchronized motion, and show the bottleneck joint sets $T^\star$.
- **Scene:** two stacked time plots. **Top (independent):** joint 1 (large $\Delta q$) finishes at $T_1$, joint 2 (small $\Delta q$) finishes earlier at $T_2<T_1$, then sits flat — a "waiting" plateau marked. **Bottom (synchronized):** both finish at $T^\star=T_1$; joint 2 is gentler (stretched). A dashed $\dot q_{\max}$ line touches joint 1's peak speed.
- **Labels:** "joint 2 waits" (top); "common $T^\star$ set by the slowest joint" and "bottleneck at limit" (bottom).
- **Form:** SVG, 2 stacked panels. Joint 1 teal `#0d9488`, joint 2 violet `#8b5cf6`, limit line muted dashed.
