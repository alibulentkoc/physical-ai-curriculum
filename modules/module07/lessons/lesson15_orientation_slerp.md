...
unit: 04
lesson: 4.3
title: "Orientation Interpolation: SLERP"
core_idea: "A tool has an orientation as well as a position, and orientation can't be interpolated by averaging matrix entries. SLERP (spherical linear interpolation) moves between two orientations along the shortest rotation at a constant angular rate, giving a smooth, well-defined orientation path."
estimated_time: "45 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L4.2 — Cartesian position interpolation"
  - "M2 — Rotations and orientation representations"
learning_objectives:
  - "Explain why naive interpolation of rotation matrices or Euler angles fails."
  - "Describe SLERP as constant-rate interpolation along the shortest rotation arc."
  - "Use SLERP (and its planar shortest-arc form) to interpolate a tool's orientation smoothly."
---
...
`[Visual: two tool orientations and the SLERP arc between them on the sphere of rotations — shortest great-circle arc, constant-rate tick marks — vs a naive path that shrinks/detours]`

**Diagram Specification**

- **Objective:** show SLERP as a constant-rate shortest arc, contrasted with naive interpolation.
- **Scene:** left = a small gripper drawn at orientation A and orientation B with several SLERP-interpolated gripper orientations between them, turning directly and evenly (constant-rate tick marks). Right = a stylized "sphere of rotations" with the great-circle arc A→B (SLERP, emerald) and a chord/naive path (red, marked "shrinks — not a valid rotation").
- **Labels:** "SLERP: shortest arc, constant rate, always valid" vs "naive average: invalid / detour"; shortest-arc note.
- **Form:** SVG, 2 panels. SLERP emerald `#10b981`; naive error `#b91c1c`; grippers ink.
