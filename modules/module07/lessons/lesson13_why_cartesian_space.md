...
unit: 04
lesson: 4.1
title: "Why Cartesian Space? Straight-Line Tool Motion"
core_idea: "Joint-space moves are simple but make the tool trace a curve; many tasks need the tool to follow a controlled path in space — most often a straight line. Cartesian-space trajectories plan the tool's path directly, then solve inverse kinematics to find the joint motion that realizes it."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L3.1 — Joint-space moves trace a curved tool path"
  - "M5 — Inverse kinematics (tool pose → joint angles)"
learning_objectives:
  - "Explain why some tasks require planning in task (Cartesian) space rather than joint space."
  - "Contrast the curved tool path of a joint move with the straight tool path of a Cartesian move."
  - "State the basic Cartesian-trajectory recipe: interpolate the tool path, then solve IK along it."
---
...
`[Visual: same start and goal tool positions — a joint move's curved tool path vs a Cartesian move's straight tool path, with the arm poses overlaid]`

**Diagram Specification**

- **Objective:** show, for identical start/goal tool positions, the joint move bowing and the Cartesian move going straight.
- **Scene:** the $(x,y)$ workspace with the arm at start (solid) and goal (ghosted). Two tool paths overlaid: the **joint move** path (curved, violet, "joints simple → tool curved") and the **Cartesian** path (straight, emerald, "tool straight → joints complex"). A greenhouse stem drawn near the straight line to motivate "approach along the line."
- **Labels:** "joint-space move" (curve) vs "Cartesian move" (line); start/goal; stem/obstacle.
- **Form:** SVG. Joint path violet `#8b5cf6`; Cartesian path emerald `#10b981`; arm ink; stem/fruit rose `#e11d48`.
