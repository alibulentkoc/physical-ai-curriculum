...
unit: 08
lesson: 8.4
title: "Capstone Integration: The Greenhouse Harvest Cycle"
core_idea: "The whole module, on a real task: a harvest cycle (reach → grasp pose → retreat) is produced by running Plan → Parameterize → Validate → Execute for each motion, chaining validated reference layers into a complete sequence. This is Module 7's capstone — the motion-planning workflow delivering the reference layer Module 8 will track."
estimated_time: "50 min"
difficulty: "Advanced"
prerequisites:
  - "M7 L8.1-8.3 — the workflow and the reference trajectory layer"
  - "M7 Units 1-7 (the whole module)"
learning_objectives:
  - "Produce a multi-motion task (a harvest cycle) by chaining the four-stage workflow."
  - "Assemble validated reference layers into a complete motion sequence handed to Module 8."
  - "Recap Module 7 and state precisely the bridge to Module 8."
---
...
`[Visual: the harvest cycle — home → pre-grasp → grasp → retreat — each leg a Plan→Parameterize→Validate→Execute block producing a validated reference layer, chained into one sequence handed to Module 8]`

**Diagram Specification**

- **Objective:** show the multi-leg cycle, each leg the full workflow, chained into one validated sequence.
- **Scene:** a horizontal sequence of three legs (reach, grasp, retreat) between pose nodes (home, pre-grasp, grasp, retreat-clear). Each leg is a compact "Plan→Param→Validate→✓ layer" block. The legs chain at rest-poses (small "at rest" markers at the joins). A final arrow hands the chained reference across the M7/M8 line to a greyed "Module 8 tracker". A small greenhouse arm icon traces the cycle.
- **Labels:** pose names; per-leg workflow; "rest joins → continuous"; "validated cycle reference → Module 8".
- **Form:** SVG, leg sequence + handoff. Legs teal/emerald, rest-joins marked, M8 dashed/muted.
