...
unit: 03
lesson: 3.1
title: "Point-to-Point Joint Moves: Per-Joint Polynomials"
core_idea: "A joint-space point-to-point move applies a time scaling to each joint independently: every joint runs its own cubic or quintic from its start angle to its goal angle over a shared duration T. Joint-space motion is simple and always feasible in the joints, but the tool traces a curved path in space."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L2.3 — Cubic vs quintic time scaling"
  - "M4 — Forward kinematics (joint angles → tool pose)"
learning_objectives:
  - "Construct a joint-space point-to-point trajectory by giving each joint its own polynomial time scaling."
  - "Explain why joint-space moves are simple and joint-feasible but trace a curved tool path."
  - "Use the engine to build and sample a per-joint quintic move and read off the tool path it produces."
---
...
`[Visual: two-link arm doing a per-joint quintic move — straight line in the (q1,q2) joint plane on the left, curved tool path in (x,y) space on the right]`

**Diagram Specification**

- **Objective:** show the same motion as a straight line in joint space and a curve in task space.
- **Scene:** left panel = the $(q_1,q_2)$ joint plane with a straight arrow from $\mathbf q_0$ to $\mathbf q_f$ and evenly-spaced dots (eased timing). Right panel = the $(x,y)$ workspace with the arm at start (solid) and goal (ghosted), and the **curved** tool path the move produces, with dots at the same time samples.
- **Labels:** "straight in joint space" (left) vs "curved in task space" (right); start/goal arm poses; greenhouse fruit at the goal.
- **Form:** SVG, 2 panels. Joint path teal `#0d9488`; tool path violet `#8b5cf6`; arm ink; fruit rose `#e11d48`.
