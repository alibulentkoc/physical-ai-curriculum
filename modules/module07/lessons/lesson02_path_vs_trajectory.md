...
unit: 01
lesson: 1.2
title: "Path vs Trajectory: Geometry versus Timing"
core_idea: "A path is pure geometry — the ordered set of poses the tool passes through, parameterized by a unitless s∈[0,1]. A trajectory adds a clock: it says when the tool is at each point, q(t). Planning chooses the path; time parameterization turns it into a trajectory."
estimated_time: "35 min"
difficulty: "Introductory"
prerequisites:
  - "M7 L1.1 — Motion quality: smooth, safe, efficient"
  - "M5 — A pose corresponds to joint values via IK"
learning_objectives:
  - "Define a path as a geometric curve q(s) (or X(s)) with s a unitless parameter in [0,1]."
  - "Define a trajectory as a time-parameterized motion q(t), adding velocity and acceleration."
  - "Explain that many trajectories share one path: same route, different timing."
  - "Identify which Module 7 stage produces the path and which produces the trajectory."
tags:
  - physical-ai
  - robotics
  - path
...
`[Visual: one fixed S-shaped path drawn once, with two different time schedules shown as dots spaced along it — evenly for one trajectory, bunched-then-sparse for another — making "same path, different trajectory" literal]`

**Diagram Specification**

- **Objective:** make "one path → many trajectories" visible: identical curve, different *spacing in time* of sample points.
- **Scene:** a single smooth curve from a "start" marker to a tomato (rose `#e11d48`). Below/over it, two timelines of dots placed at equal *time* steps: trajectory A spaces them evenly (constant speed); trajectory B clusters them at the ends and spreads them in the middle (ease-in/ease-out → fast in the middle). Same dots' *positions lie on the same curve*.
- **Labels:** "path: q(s), geometry only," "trajectory A: even timing," "trajectory B: ease-in/ease-out," "same route, different clock."
- **Form:** SVG. Ink `#0f172a` curve; teal `#0d9488` for A dots, violet `#8b5cf6` for B dots; legend pairs colour with a text label.
