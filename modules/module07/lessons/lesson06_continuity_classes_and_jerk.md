...
unit: 02
lesson: 2.2
title: "Continuity Classes C⁰/C¹/C² and Why Jerk Matters"
core_idea: "Continuity class measures how many derivatives of a motion are continuous: C0 position, C1 velocity, C2 acceleration. Because force tracks acceleration, C2 motion has no force jumps; bounding jerk on top of C2 removes the residual onset shock that wears mechanisms and bruises payloads."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L2.1 — s(t) and its derivatives"
  - "M7 L1.3 — Smoothness as a quality criterion"
learning_objectives:
  - "Define continuity classes C0, C1, C2 precisely in terms of which derivatives are continuous."
  - "Explain why C2 continuity means continuous force and therefore no shock loads."
  - "Distinguish bounded jerk from C2 continuity and say what each buys."
  - "Diagnose the continuity class of a motion from its position/velocity/acceleration plots."
tags:
  - physical-ai
  - robotics
  - continuity
...
`[Visual: three columns (C0, C1, C2) each showing position, velocity, acceleration stacked — highlighting where the jump lives in each class]`

**Diagram Specification**

- **Objective:** show, per class, which derivative first shows a discontinuity, and that $C^2$ has none through acceleration.
- **Scene:** a 3-column grid. Column $C^0$: position with a kink, velocity with a jump (red marker on the jump), acceleration spiking. Column $C^1$: position smooth, velocity continuous, acceleration with a step jump (red marker). Column $C^2$: position, velocity, acceleration all continuous (all green); a fourth mini-row shows jerk, bounded.
- **Labels:** "first discontinuity at:" velocity ($C^0$), acceleration ($C^1$), none through accel ($C^2$). Pair every red jump with the word "jump."
- **Form:** SVG, 3×(3–4) grid. Continuous curves emerald `#10b981`, discontinuities error `#b91c1c`, ink axes.
