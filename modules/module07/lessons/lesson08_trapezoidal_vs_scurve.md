...
unit: 02
lesson: 2.4
title: "Trapezoidal vs S-Curve Velocity Profiles"
core_idea: "A trapezoidal velocity profile (accelerate at a_max, coast at v_max, decelerate at a_max) is time-optimal under velocity/acceleration limits but is only C1 — its acceleration steps cause jerk spikes. An S-curve rounds those corners by bounding jerk, giving C2 motion for a small time penalty."
estimated_time: "40 min"
difficulty: "Intermediate"
prerequisites:
  - "M7 L2.2 — Continuity classes and jerk"
  - "M7 L2.3 — Polynomial time scaling"
learning_objectives:
  - "Construct a trapezoidal velocity profile from distance, v_max, and a_max."
  - "Explain why the trapezoidal profile is time-optimal but only C1 (acceleration discontinuous)."
  - "Describe the S-curve as a jerk-limited profile that rounds the trapezoid's corners to reach C2."
  - "Choose between polynomial, trapezoidal, and S-curve methods for a given motion goal."
---
...
`[Visual: trapezoidal vs S-curve overlaid — velocity (trapezoid with sharp corners vs rounded S), and acceleration (steps vs continuous trapezoids), with jerk spikes marked on the trapezoid]`

**Diagram Specification**

- **Objective:** show the corner-rounding: trapezoid's velocity has sharp corners and stepped acceleration; the S-curve rounds both and keeps acceleration continuous.
- **Scene:** two stacked panels over time. *Velocity:* trapezoid (straight ramp-flat-ramp, sharp corners, error `#b91c1c`) vs S-curve (same envelope but rounded corners, emerald `#10b981`). *Acceleration:* trapezoid as a $+a_{\max}/0/-a_{\max}$ step function with red "jump" markers and small jerk-spike glyphs at the steps; S-curve acceleration as continuous trapezoids.
- **Labels:** phase markers ($t_a$, coast, $t_a$); "C¹: acceleration steps → jerk spikes" vs "C²: acceleration continuous, jerk bounded"; "time-optimal" vs "slightly slower, smooth".
- **Form:** SVG, 2 panels. Dashed $v_{\max}$/$a_{\max}$ limit lines; colour always paired with text.
