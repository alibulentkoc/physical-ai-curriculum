...
unit: 06
lesson: 6.2
title: Adding Depth Recovers a Point
core_idea: "Given a back-projected ray and a depth measurement, the 3D camera-frame point is fixed: P_c = Z·(x_n, y_n, 1). Depth resolves the ambiguity."
estimated_time: 45
difficulty: Core
prerequisites: [6.1]
learning_objectives:
  - Combine a ray with depth to get a 3D camera-frame point.
  - Identify where depth comes from (depth camera, stereo, geometry).
  - Compute the point from a pixel and a depth value.
tags:
  - physical-ai
  - perception
  - back-projection
...
`[Visual: the ray from Lesson 6.1 with a depth value Z marked, selecting the single 3D point P_c = Z·(x_n,y_n,1) on the ray]`

**Diagram Specification**
- **Objective:** show depth selecting the point on the ray.
- **Scene:** camera center, the ray through pixel (u,v); a depth marker at Z along the optical axis; the chosen point P_c highlighted on the ray; formula P_c = Z·(x_n,y_n,1).
- **Labels:** "ray direction (x_n,y_n,1)," "depth Z (from sensor)," "P_c = (Z x_n, Z y_n, Z)," "ambiguity resolved."
- **Form:** SVG (faux-3D).
