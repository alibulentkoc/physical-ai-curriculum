...
unit: 03
lesson: 3.3
title: Principal Point and Focal Length in Pixels
core_idea: "The principal point is where the optical axis meets the image; focal length in pixels sets the scale — together they are the camera's intrinsic parameters, found by calibration."
estimated_time: 40
difficulty: Core
prerequisites: [3.2]
learning_objectives:
  - Define the principal point and its typical location.
  - Interpret focal length in pixels as image scale.
  - Explain at a high level what calibration produces.
tags:
  - physical-ai
  - perception
  - intrinsics
...
`[Visual: an image with the principal point marked slightly off-center (where the optical axis hits), and an illustration of how larger f_x spreads a point farther from the principal point]`

**Diagram Specification**
- **Objective:** show the principal point and the scaling role of focal length in pixels.
- **Scene:** an image rectangle ($640\times480$) with the principal point marked near but not exactly at center; a point straight-ahead landing on it; two cases (small $f_x$, large $f_x$) showing the same direction landing nearer/farther from the principal point.
- **Labels:** "principal point (c_x, c_y) ≈ center," "optical axis hits here," "large f_x → farther from center," "calibration finds these."
- **Form:** SVG.
