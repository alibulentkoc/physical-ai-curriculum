...
unit: 03
lesson: 3.4
title: Camera Intrinsics — Unit 3 Recap
core_idea: "Intrinsics turn the pinhole rule into pixels: K holds focal length and principal point, and projection is K·(X,Y,Z) then divide-by-Z."
estimated_time: 20
difficulty: Review
prerequisites: [3.1, 3.2, 3.3]
learning_objectives:
  - Consolidate metric-to-pixel conversion, K, and its parameters.
  - State pixel projection compactly.
  - Bridge to projecting in practice and OpenCV (Unit 4).
tags:
  - physical-ai
  - perception
  - intrinsics
...
`[Visual: the intrinsics summary — (X,Y,Z) → K → ÷Z → pixel (u,v), with K's four parameters labeled and a point on the optical axis landing at the principal point]`

**Diagram Specification**
- **Objective:** consolidate pixel projection via $K$.
- **Scene:** a flow: camera-frame point $(X,Y,Z)$ → box "× K" → box "÷ Z" → pixel $(u,v)$ on an image grid; $K$'s entries labeled; a dashed optical-axis point landing at the principal point.
