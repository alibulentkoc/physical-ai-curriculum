...
core_idea: "Trust the world position by verifying it: re-projection round-trip, distance preservation, and a visualization of the camera, ray, fruit, and world frame."
estimated_time: 50
difficulty: Project
prerequisites: [8.2]
learning_objectives:
  - Implement the three acceptance checks.
  - Visualize the perception-to-world result.
  - Interpret check failures to localize faults.
tags:
  - physical-ai
  - perception
  - project
...
`[Visual: a verification panel — the re-projected pixel landing back on the detection, plus a faux-3D plot of camera, ray, fruit, and world frame]`

**Diagram Specification**
- **Objective:** show numeric + visual verification together.
- **Scene:** left, an image with the original detection and the re-projected pixel coinciding (≤1px); right, faux-3D: camera center, ray, fruit point P_w, world axes; a small table of the three checks with ✓.
- **Labels:** "re-projection ≤1px," "distance preserved," "in workspace," "fruit on the ray."
- **Form:** SVG (faux-3D).
