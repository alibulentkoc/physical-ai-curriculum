...
lesson: 5.3
title: Undistortion
core_idea: "Undistortion reverses the distortion model so a measured pixel maps back to its ideal pinhole location — the step that lets back-projection use clean geometry."
estimated_time: 45
difficulty: Core
prerequisites: [5.2]
learning_objectives:
  - Explain what undistortion produces and why it is needed.
  - Describe how undistortion is computed (iteratively / via OpenCV).
  - Use undistorted points with the ideal pinhole model.
tags:
  - physical-ai
  - perception
  - distortion
...
`[Visual: a distorted edge point being iteratively nudged back to its ideal pinhole location, with the center unchanged and the correction larger at the edge]`

**Diagram Specification**
- **Objective:** show undistortion as recovering the ideal point from the measured one.
- **Scene:** a distorted grid (bowed) with a measured edge point; arrows showing an iterative nudge back onto the straight ideal grid; center point unchanged; label "a few iterations converge."
- **Labels:** "measured (distorted) pixel," "K⁻¹ → normalized," "iterate to invert distortion," "ideal pinhole point," "center: no change."
- **Form:** SVG.
