...
unit: 05
lesson: 5.2
title: Radial and Tangential Distortion
core_idea: "Distortion is modeled by a few coefficients: radial terms (k1, k2, k3) that grow with radius and tangential terms (p1, p2) from lens misalignment."
estimated_time: 45
difficulty: Core
prerequisites: [5.1]
learning_objectives:
  - Write the radial and tangential distortion model.
  - Interpret the distortion coefficients.
  - Apply the model to distort a normalized point.
tags:
  - physical-ai
  - perception
  - distortion
...
`[Visual: the distortion pipeline on a normalized point — ideal (x_n,y_n) → radial scaling (grows with r) → tangential nudge → distorted (x_d,y_d) → K → pixel — with the coefficient vector labeled]`

**Diagram Specification**
- **Objective:** show the two distortion components and the coefficient vector.
- **Scene:** a normalized point near the edge; arrow "radial × (1+k1 r²+k2 r⁴+k3 r⁶)" pushing it outward; small arrow "tangential (p1,p2)" nudging sideways; then "× K → pixel"; a box listing distCoeffs = (k1,k2,p1,p2,k3).
- **Labels:** "radial: grows with r," "tangential: slight skew," "k1 dominant," "all zero = ideal pinhole."
- **Form:** SVG.
