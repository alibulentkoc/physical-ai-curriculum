...
## D-066 — Module 8 launch package APPROVED (Feedback Control and Real-Time Execution (ROS 2))

**Architect approved the Module 8 launch package** — 8 units · 32 lessons · 32 notebooks · 32 SVGs · 4 demos (L07/L17/L21/L29) · 32 quizzes · 33 answer keys · 1 midpoint (after Unit 4), with all §9 rulings:

- **§9.1 Dynamics depth:** use disturbances, loads, friction, saturation, and model mismatch as **intuition only** — **no formal manipulator dynamics** (no mass matrix, Coriolis, computed-torque, Lagrangian).
- **§9.2 Control-theory formalism:** **no Laplace transforms, transfer functions, root locus, Bode, or Nyquist** in the core arc. Teach **error → correction → stability → implementation**.
- **§9.3 ROS 2:** conceptual architecture + lightweight code only; **not** a full ROS 2 course.
- **§9.4 Plant fidelity:** **integrator + disturbance + saturation** model.
- **§9.5 Breadth:** maintain intuition-first depth across all domains (control, actuation, communication, embedded, ROS 2).
- **§9.6 Title:** **"Feedback Control and Real-Time Execution (ROS 2)."**
- **§9.7 M7 feedforward:** explicitly consume **q_d, q̇_d, q̈_d** from Module 7; **feedforward + feedback is a major continuity theme.**
- **Additional guidance:** **begin by repeatedly contrasting open-loop vs closed-loop** before introducing PID — students must first experience why feedback is necessary.

**Unit structure:** U1 The Tracking Problem and the Feedback Loop · U2 PID Control · U3 Stability, Response, and Tuning · U4 Tracking the Whole Arm: Feedforward + Feedback **[MIDPOINT after U4]** · U5 Actuator Control · U6 Communication · U7 Embedded/Real-Time Execution · U8 ROS 2 Integration and the Control Stack (Capstone). **Capstone artifact:** `tracking_controller(reference, measured_state) → actuator_command` packaged as a ROS 2 node — the **Module 9 handoff**, completing the chain **velocity_layer (M6) → reference layer (M7) → control layer (M8) → integrated system (M9)**. Running example: planar 2-link greenhouse arm (L1=0.4, L2=0.3) driven against a simulated plant.

**Directed:** proceed with **Installment A (Units 1–2)**; pause at the Installment A milestone.

## D-067 — Module 8 Installment A (Units 1–2) delivered

**Installment A delivered: Units 1 (The Tracking Problem and the Feedback Loop) and 2 (Proportional, Integral, and Derivative Control), L01–L08.**

**Lessons.** **Unit 1** opens on the architect's directive — repeatedly contrasting **open-loop vs closed-loop** so students first feel why feedback is necessary: 1.1 *Why Open-Loop Isn't Enough: the reference-vs-reality gap* → 1.2 *Tracking Error* (e = q_d − q, the quantity control fights) → 1.3 *The Feedback Loop* (sense → compare → correct → actuate) → 1.4 *Anatomy of a Control System* (reference, plant, controller, feedback). **Unit 2** builds PID from error intuition: 2.1 *Proportional Control* (correction ∝ error; the load/Kp steady-state offset) → 2.2 *Integral Control* (erasing the offset; windup & anti-windup) → 2.3 *Derivative Control* (anticipate & damp; the noise caveat; **L07 demo**) → 2.4 *The PID Controller* (the complete single-joint tracker). Each lesson uses the 12-section template + the two companion blocks.

**Engine.** Shared `m8_engine.py` (verified): a simulated **plant** `Joint` (integrator + injectable disturbance/load/friction + actuator saturation), `PIDController` (e = q_d − q, derivative-on-error, integral with optional anti-windup clamp), `p_command`, `feedforward_command`, `simulate_open_loop`/`simulate_closed_loop`, `step_response_metrics` (overshoot/settling/rise/steady-state), `classify_stability` (settles/oscillates/diverges, envelope-based), `tracking_rms`, plus `quintic_reference`/`setpoint_reference`. Verified facts: open-loop drifts under load while closed-loop tracks; P leaves ss-offset ≈ load/Kp; I removes it; D cuts overshoot; feedforward + feedback beats feedback-only.

**Artifacts.** 8 lessons · 8 SVGs (m08-l1..l8, XML-valid) · 8 notebooks (**all "All checks passed." under Restart-and-Run-All**) · **1 flagship demo** (L07 PID Playground — Kp/Ki/Kd sliders, live overshoot/settling/steady-state, noise toggle; no storage, accessible) · 8 quizzes (5 MC + 3 short) · 8 answer keys.

**Generator + site.** Generator registered (`"08"` + module/unit titles); auto-discovered L01–L08 and injected each SVG (after §4), the L07 demo iframe (after §7), the notebook tip (after §8), and the quiz (after §9); validator clean (no `[Visual:]` leak; Diagram Specification stripped). Nav added to `mkdocs.yml` (Units 1–2, "· demo" on 2.3/L07). `mkdocs build --strict` **passes** (exit 0) at **237 lesson pages** (229 + 8).

**Boundaries held (§9):** disturbance/load/friction/saturation/model-mismatch intuition only (no formal manipulator dynamics); no Laplace/transfer-function/Bode/Nyquist; plant = integrator + disturbance + saturation. **M7 continuity:** the controller consumes M7's `q_d`, and feedforward `q̇_d, q̈_d` is introduced as the lead term — feedforward + feedback set up to be fully developed in Unit 4.

**Paused at the Installment A milestone for architect review.** Remaining: B (U3–U4 + midpoint), C (U5–U6), D (U7–U8 + capstone).

## D-068 — Module 8 Installment B (Units 3–4 + midpoint) delivered

**Installment B delivered: Units 3 (Stability, Response, and Tuning) and 4 (Tracking the Whole Arm: Feedforward + Feedback), L09–L16, plus the Module 8 midpoint assessment (Units 1–4).**

**Unit 3 — taught experience-first, per the architect's directive (behaviour before terminology).** L09 (3.1) opens on the driving question *"what happens when correction goes wrong?"* and walks the **correction spectrum** — too weak → good → too strong → oscillation → runaway — so students *recognise* each behaviour by eye before any vocabulary. Only then: L10 (3.2) names the **response shape** (rise time, overshoot, settling time, steady-state error) and the speed-vs-overshoot trade; L11 (3.3) names the stability regimes **stable / marginal / unstable** by envelope and the three destabilisers — **too much gain, too little damping, too much delay** — making the point that **latency is as dangerous as excess gain** (demonstrated by a delay sweep at fixed gains that crosses from stable to unstable); L12 (3.4) gives a practical **P → D → I tuning workflow** (raise Kp to the oscillation edge and back off, add D to damp and buy headroom, add I to erase the offset, verify the four metrics and leave margin).

**Unit 4 — the payoff (feedforward + feedback).** L13 (4.1) extends single-joint PID to **per-joint independent control** of the whole arm tracking a Module-7 trajectory, and exposes the speed-dependent **following error** of feedback-only tracking (coupling handled as a disturbance — no formal dynamics). L14 (4.2) is the conceptual core: **feedback reacts, feedforward anticipates** — the inverse-model feedforward `u_ff = m·q̈_d + b·q̇_d + ℓ` is presented as anticipation, visibly **consuming Module 7's q̇_d and q̈_d**, and shown (good model) to track a fast move with almost no lag. L15 (4.3) runs the **required comparison** on the *identical* trajectory: feedback-only (RMS ≈ 0.075) vs feedforward+feedback (RMS ≈ 0.017) — about **4.5× tighter**, with *smaller* feedback effort — and shows the combination beats feedforward-only under model mismatch. L16 (4.4) proves feedback is non-negotiable: a mid-trajectory **disturbance** (which enters the plant, not the reference) is **unrejectable by feedforward** (final |err| ≈ 1.4) but **rejected by feedback** (final |err| ≈ 0.003); it assembles the **complete joint tracker** `u = m·q̈_d + b·q̇_d + ℓ + PID(q_d − q)` and names the interface `tracking_controller(reference, measured_state) → actuator_command` as the **Module 9 handoff**, tying the module's last lesson back to Unit 1's open-loop drift.

**Engine.** Extended additively (superset of Installment A; backup at `engine/m8_engine_A_backup.py`): `feedforward_full` (m·q̈_d + b·q̇_d + ℓ — consumes both q̇_d and q̈_d), `track_reference` (single-joint trajectory tracking with selectable feed-forward mode and a **sensor-latency** option for the Unit-3 delay demos), `JointTracker` (per-joint PID + feed-forward — the complete tracker), `multi_quintic` and `track_arm` (whole-arm tracking + overall RMS). Verified behaviours: gain sweep reproduces the spectrum; latency at fixed gains marches stable→unstable; ff+fb ≈ 4.5× tighter than feedback-only; feedforward-only cannot reject a disturbance while ff+fb does.

**Artifacts.** 8 lessons (L09–L16, 12-section template + companion blocks) · 8 SVGs (m08-l9..l16, XML-valid; L9 a five-panel behaviour spectrum, L15 the ff-vs-ff+fb comparison with RMS bars) · 8 notebooks (**all "All checks passed."**) · 8 quizzes (5 MC + 3 short) · 8 answer keys · **1 midpoint assessment + coaches' key** (Units 1–4, Sections A–E). **No new demo in Installment B** (the next demos are L17/L21 in Installment C).

**Generator + site.** Generator unit titles added for ("08","03") and ("08","04"); nav extended with Units 3–4. `mkdocs build --strict` **passes** (exit 0) at **245 lesson pages** (237 + 8); validator clean (no `[Visual:]` leak; Diagram Specification stripped; SVGs inlined).

**Boundaries held (§9):** no formal manipulator dynamics (disturbance/load/friction/saturation/model-mismatch intuition only; inter-joint coupling treated as disturbance), no control-theory formalism (Laplace/transfer-functions/Bode/Nyquist/poles named **only** as out-of-scope), plant = integrator + disturbance + saturation + latency. **M7 continuity** is the spine of Unit 4 throughout.

**Paused at the Installment B milestone for architect review.** **Next:** Installment C — Units 5 (Actuator Control) and 6 (Communication), with demos at L17 and L21.

## D-069 — Module 8 Installment C (Units 5–6) delivered

**Installment C delivered: Units 5 (Actuator Control) and 6 (Communication), L17–L24. Architect approved all eight lesson drafts and resolved the three boundary questions; this entry records the delivered build.**

**Boundary rulings (as approved).** (1) **L23 control-rate framing — qualitative; no discrete-time formalism.** Control rate is taught as "the controller acts N times per second on slightly stale information, holding its command between updates," and stability stays the envelope-based settle/oscillate/diverge classification — no sampling theory, no Laplace/Nyquist/phase-margin. (2) **L22 illustrative broker — a pattern, not a framework.** Publish/subscribe is taught as nodes/topics/messages with decoupled publish and subscribe; ROS 2 is named **only** as the Unit 8 implementation. (3) **L21 nervous-system analogy — retained with explicit limitations** (messages are discrete packets, the controller is a fixed-rate program; the analogy conveys "distributed and delayed," not a model of computation).

**Unit 5 — Actuator Control.** L17 (5.1) opens the actuator as a **request→delivery converter**: a requested command becomes a delivered effort distorted by **deadband** (tiny requests deliver nothing), **saturation** (effort ceiling ±u_max, surplus discarded), and a **rate limit** (delivered effort slews, doesn't jump) — same request 5 delivered as 5 / 0 / 3 through ideal / deadband(8) / saturated(3). L18 (5.2) shows **saturation degrades tracking** (RMS ≈ 0.0002 generous vs ≈ 0.06 saturated on a fast move) and feeds **integral windup**, cured by the Unit-2 anti-windup clamp (≈69% overshoot plain vs ≈18% clamped, same gains). L19 (5.3) shows **deadband + stiction** stall P/PD short of target (|e| ≈ 0.44) while **integral wins the last millimetre** (|e| ≈ 0.003). L20 (5.4) assembles the **command pipeline** (reference → controller → requested command → actuator → delivered effort → motion) and the **feasibility envelope** (deliverable effort/rate), tying back to **Module 7 feasibility**: a within-envelope move tracks cleanly (no clip, RMS ≈ 0) while an over-aggressive one clips (~60%) and is missed (RMS ≈ 0.29) — fix by re-planning, not re-tuning.

**Unit 6 — Communication.** L21 (6.1) reframes the loop as **messages** between separate sensor/controller/actuator subsystems (the nervous-system analogy, with limits), giving latency a physical source: **loop latency = Σ per-hop times** (demo **L21 Message Bus**). L22 (6.2) teaches **publish/subscribe** — nodes, topics, messages, and **decoupling** (publishers/subscribers never reference each other; the topic mediates) — mapped onto the loop, with ROS 2 placed as one implementation. L23 (6.3) shows **latency and a finite control rate** both add loop delay that, **at fixed gains**, marches a stable loop through oscillation to instability (the Unit-3 lesson, communication-sourced; verified by a latency sweep and a control-rate sweep) — motivating real-time execution. L24 (6.4) organises the robot's data flow into an **architecture layered by timing**: a fast, **latency-critical inner loop** vs slower, **latency-tolerant outer layers** (verified asymmetry: heavy latency in the loop → unstable; the same slowness in the outer reference update → stable, RMS ≈ 0.086) — motivating real-time (Unit 7) and framing the ROS 2 stack (Unit 8).

**Engine.** Extended **additively** (every Installment-A/B function unchanged; B-state backup at `engine/m8_engine_B_backup.py`). Unit 5: `Actuator(u_max, deadband, rate_max)` with `.deliver`/`.characteristic`/`.reset` and `clipped`/`rate_limited` flags; `apply_stiction`; `step_plant`; `track_reference_actuated` (full pipeline with `sensor_delay_steps` latency and `control_period_steps` control-rate levers, reporting requested vs delivered effort and clip/rate fractions); `feasibility_envelope`. Unit 6: `Bus` (subscribe/publish/latest/topics) pub/sub broker; `loop_latency`; `latency_to_steps`; `zoh_reference`; `run_pubsub_loop` (loop run as messages over the Bus). All asserted behaviours verified before notebook build.

**Artifacts.** 8 lessons (L17–L24, 12-section template + AI Learning Companion + Global Learning Support in 4 languages) · 8 SVGs (m08-l17..l24, XML-valid) · 8 notebooks (**all "All checks passed." under Restart-and-Run-All**) · **2 flagship demos** (L17 Actuator Bench — dial requested command, toggle deadband/saturation/rate limit, live transfer curve + step ramp; L21 Message Bus — a message traverses each hop, loop latency accumulates as Σ hops; both accessible, no browser storage) · 8 quizzes (5 MC + 3 short) · 8 answer keys.

**Generator + site.** Generator unit titles added for ("08","05")="Actuator Control" and ("08","06")="Communication"; nav extended with Units 5–6 ("· demo" on 5.1/L17 and 6.1/L21). `mkdocs build --strict` **passes** (exit 0) at **253 lesson pages** (245 + 8); validator clean (no `[Visual:]` leak; Diagram Specification stripped; SVGs inlined; demos/quizzes copied to served dirs).

**Boundaries held (§9 + the three rulings):** Unit 5 actuators **plant-level only** (no motor electrodynamics/current loops/actuator dynamics); Unit 6 communication **conceptual** (pub/sub a pattern, ROS 2 named only as the Unit 8 implementation); L23 **qualitative** (no discrete-time/sampling formalism); L21 analogy **with explicit limits**. No control-theory formalism anywhere; plant = integrator + disturbance + saturation + latency, now driven through the actuator. M6→M7→M8→M9 chain intact; the Unit-6 timing split and the Unit-5 envelope set up Units 7–8.

**Paused at the Installment C milestone for architect review.** **Next:** Installment D — Units 7 (Embedded and Real-Time Execution) and 8 (ROS 2 Integration and the Control Stack, capstone), with the L29 demo and the `tracking_controller` ROS 2 node (the Module 9 handoff).

## D-070 — Module 8 Installment D (Units 7–8, capstone) delivered — **MODULE 8 COMPLETE**

**Final installment delivered: Units 7 (Embedded Execution and Real-Time Control) and 8 (ROS 2 Integration and the Control Stack), L25–L32, including the L29 Closed-Loop Tracking Studio, the Module 8 capstone, and the control-layer handoff to Module 9. Built straight through to module completion per the architect directive — no further installment reviews required within this module.**

**Directive (as relayed).** Proceed with Installment D: Unit 7 *Embedded Execution and Real-Time Control* and Unit 8 *ROS 2 Integration and the Control Stack*, including the L29 Closed-Loop Tracking Studio, the Module 8 capstone, and the control-layer handoff to Module 9. Maintain boundaries: **no formal dynamics, no advanced control theory, no system integration beyond the control layer.** The capstone produces the control layer consumed by Module 9. **Pause at Module 8 completion; no further installment reviews within this module.**

**Unit 7 — Embedded Execution and Real-Time Control.** L25 (7.1) defines **real-time = correct AND on time**, with the figure of merit the **worst case, not the average**: a bounded-interval loop is stable, while a loop far faster on average but with rare long stalls is unstable (mean interval ≈ 14.7 steps yet worst ≈ 182 → unstable). Hard vs soft real-time; the inner loop is hard. L26 (7.2) frames the inner loop as a **periodic task** — sense → compute → actuate every control period $T_c$ (rate $1/T_c$), with a per-period **timing budget** — and shows a fixed-rate periodic loop tracks a moving reference (RMS ≈ 0.0007, 101 updates); the period is the **execution of the continuous control law** (held between samples), framed qualitatively (no discrete-time/$z$-transform formalism). L27 (7.3) names the three timing hazards — **jitter** (varying period), **overruns** (work exceeds the period), **missed deadlines** (skipped update, command held) — and shows all three **add loop delay** (the Unit-6 destabiliser): deterministic stable, jitter unstable, overruns unstable at identical gains. L28 (7.4) concludes with the **real-time target**: the inner loop runs on a protected, deterministic target (stable); the same loop on a best-effort target jitters and goes unstable; the slow outer layer tolerates loose timing (RMS ≈ 0.086) — the **placement rule** latency-critical → real-time, latency-tolerant → best-effort (the 6.4 timing split realised).

**Unit 8 — ROS 2 Integration and the Control Stack (capstone).** L29 (8.1) assembles the **closed-loop control stack as a ROS 2 node/topic graph** — sensor node → `joint/state` → controller node (tracker) → `joint/command` → actuator node → loop closed — the 6.2 publish/subscribe pattern now carrying real control, with the controller node on the Unit-7 real-time target (tracks via the two topics, RMS ≈ 0.0015); demo **L29 Closed-Loop Tracking Studio**. L30 (8.2) zooms to the **lightweight tracker node**: two subscriptions, one publication, one control-law evaluation per cycle — **intelligence (the control law) vs plumbing (the node)**, with one command published per state (1001 → 1001). L31 (8.3) packages the **control layer** — `tracking_controller(reference, measured_state) → actuator_command` — as the **Module 9 handoff**, completing **velocity layer (M6) → reference layer (M7) → control layer (M8) → integrated system (M9)**; inside the layer is feedforward + feedback + actuator, outside (Module 9) is planning/coordination/supervision — **no integration beyond the control layer**. L32 (8.4) is the **capstone**: the planar 2-link greenhouse arm tracks a Module 7 reference through the closed-loop control layer (overall RMS ≈ 0.0000), and **feedback rejects a disturbance feedforward alone cannot** (final joint error ≈ 0.012 with feedback vs ≈ 4.9 feedforward-only) — the module's thesis, on the arm — and produces the control layer for Module 9.

**Engine.** Extended **additively** (every A/B/C function unchanged; C-state backup at `engine/m8_engine_C_backup.py`). Unit 7: `track_reference_variable_delay` (per-step measurement-delay schedule, for the worst-case-vs-average demonstration); `track_reference_rt` (periodic loop with `period_steps`, `jitter_steps`, `overrun_prob`, and rare-`spike_prob`/`spike_steps` heavy-tail stalls; reports updates, missed deadlines, mean/max interval). Unit 8: `control_layer(gains, ff, m_nominal, b_nominal, load_comp, actuator)` returning a stateful `tracking_controller(reference_sample, measured_state, dt) → (actuator_command, info)` with `.reset()` — **the Module 9 handoff**; `run_control_stack` (ROS-2-style pub/sub closed loop over the `Bus`, topics `joint/state` + `joint/command`); `run_arm_control_stack` (N-joint closed loop for the capstone). All asserted behaviours verified before notebook build.

**Artifacts.** 8 lessons (L25–L32, 12-section template + AI Learning Companion + Global Learning Support in 4 languages) · 8 SVGs (m08-l25..l32, XML-valid; L29/L32 spot-rendered clean) · 8 notebooks (**all "All checks passed." under Restart-and-Run-All**) · **1 flagship demo** (L29 Closed-Loop Tracking Studio — the stack closing the loop: scrub the reference, toggle feedback on/off, inject a disturbance, watch the measured trajectory converge and the tracking error shrink; accessible, ARIA, no browser storage) · 8 quizzes (5 MC + 3 short) · 8 answer keys.

**Generator + site.** Generator unit titles added for ("08","07")="Embedded Execution and Real-Time Control" and ("08","08")="ROS 2 Integration and the Control Stack"; nav extended with Units 7–8 ("· demo" on 8.1/L29). `mkdocs build --strict` **passes** (exit 0) at **261 lesson pages** (253 + 8); SVGs inlined, demo/quizzes served, no `[Visual:]` leak.

**Boundaries held:** **no formal dynamics** (the plant stays integrator + disturbance + load + saturation + latency through the actuator; joints independent, coupling treated as disturbance); **no advanced control theory** (real-time scheduling formalism — rate-monotonic / WCET — named only as out-of-scope; stability stays qualitative settle/oscillate/diverge; no discrete-time/sampling/$z$-transform); **ROS 2 conceptual + lightweight** (the pub/sub `Bus` stands in for the middleware; no `rclpy`/DDS/executors/QoS); **no system integration beyond the control layer** (planning, coordination, supervision are Module 9). The M6→M7→M8→M9 chain is complete through M8.

**Module 8 totals:** 8 units · 32 lessons · 32 notebooks · 32 SVGs · **4 demos** (L07, L17, L21, L29) · 32 quizzes · 32 answer keys · 1 midpoint assessment. **Module 8 is COMPLETE — 8 of 10 modules signed off.** Paused at module completion per directive. **Next:** Module 9 — the integrated autonomous system, which consumes the Module 8 control layer `tracking_controller(reference, measured_state) → actuator_command`.

## D-071 — Module 9 Installment A (Units 1–2) delivered — Perceive → Understand

**Module 9 — System Integration launches.** The integration course that makes the real M3–M8 layers cooperate as one greenhouse harvesting robot. Workflow spine **Perceive → Understand → Plan → Execute → Track → Recover** (8 units, 32 lessons, 4 installments A–D, 4 lessons/unit, midpoint after Unit 4). Governing rule: **Wrap. Do not redefine.** — every stage calls a real existing layer; Module 9 adds no estimation, planning, or control theory. Running example: planar 2-link arm **L1=0.4, L2=0.3** (canonical, reconciled at re-base).

**Directive (as relayed).** Build the integration course; reuse the real layers via adapters; keep the focus on the seams, not new theory. Deliver per installment with architect review; pause at each milestone.

**Installment A — Units 1–2 (L01–L08).** U1 (Why integration is hard / the six-stage workflow / tracing one tomato): the seams, not the parts, are the work; the data-flow contract from detection to harvested fruit. U2 (Perception → Understand): `model_perception` (M3 boundary, fault-injectable: noise/occlude/duplicate), `understand(detections, world) → (targets, current_target)` (dedupe → ripe∧reachable → nearest), the world state. **Flagship A — Pipeline Data-Flow Explorer** (L1.2): trace one detection through the stages. Integration package `modules/module09/integration/` vendors the real M5/M6/M7/M8 layers verbatim + `world.py` + `layers.py` adapter.

**Boundaries.** Wrap don't redefine; no new perception theory; the canonical 0.4/0.3 arm locked. **APPROVED.**

## D-072 — Module 9 Installment B (Units 3–4) delivered — Understand → Plan → Execute (midpoint)

**Directive (as relayed).** Proceed with Installment B: Unit 3 *Understand → Plan* and Unit 4 *Plan → Execute*, with the Motion-Stack Visualizer flagship and the Unit-4 midpoint. Keep wrapping the real layers; hold the integration focus.

**Installment B — Units 3–4 (L09–L16).** U3 (Understand → Plan): `to_configuration` (M5 IK seam, FK-verified), `plan_reference` (M7 planner wired). U4 (Plan → Execute): the motion stack (M6 velocity + M8 control), `execute_reference` (per-joint control_layer + Joint plant loop), the handoff contracts (ξ_d / q̇ / real-time), the **midpoint** at L16 — the forward path assembled, runs once (rms ≈ 0.0001, reached), no failure handling yet; feedback recovers from a disturbance. **Flagship B — Motion-Stack Visualizer** (L4.1): closed-loop tracking, ref-vs-actual + error, disturbance/ff/Kp toggles. The real M7 planner (~29 KB) and M8 control (~39 KB) libraries vendored verbatim.

**Boundaries.** Forward path only through the midpoint; no detection/recovery yet; no new planning/control theory. **APPROVED.**

## D-073 — Module 9 Installment C (Units 5–6) delivered — Execute → Track + Failure Detection

**Directive (as relayed).** Proceed with Installment C: Unit 5 *Execute → Track* and Unit 6 *Failure Detection*, with the Failure-Injection Sandbox flagship. Keep the emphasis on telemetry, health signals, ownership of failure, and fault localisation — **What failed? → Where did it fail? → Who owns the fix?** — *before* recovery strategies. Failure taxonomy integration-focused; reuse existing layer outputs and health signals; **no new control, estimation, or planning theory.** Pause at the Installment C milestone.

**Installment C — Units 5–6 (L17–L24).** U5 (Execute → Track): `track` (verdict against success criteria — final error, RMS, pose — with a localising reason), `system_monitor` (collect existing health signals M6 w/σ_min/κ, M8 effort, tracking error into one dashboard), telemetry option on `execute_reference`; *reached ≠ succeeded*, *health ≠ success*. U6 (Failure Detection): `FAILURE_TAXONOMY` (six integration events on existing signals, failure vs warning), health-signal **guards** that halt-or-flag at each seam (`run_pipeline`), `localize` (the What/Where/Who triad with an owner; where-it-surfaced may differ from who-owns-it). **Flagship C — Failure-Injection Sandbox** (L6.2): inject occlusion/unreachable/block/disturbance/near-singular and watch the guarded pipeline halt or flag at the right seam with the triad.

**Boundaries held.** Detection is **thresholded reading of existing outputs** — no estimation, fault-diagnosis, or control theory. Recovery deliberately deferred to Unit 7. **APPROVED.**

## D-074 — Module 9 Installment D (Units 7–8, capstone) delivered — **MODULE 9 COMPLETE**

**Directive (as relayed).** Proceed with Installment D: Unit 7 *Recover* and Unit 8 *Full System Integration*, with the flagship end-to-end recovery demo. Keep the sequence **What failed? → Where? → Who owns the fix? → Recover.** Recovery is an **integration behavior built on the existing layers and detection signals** — **no new estimation, planning, or control theory.** Pause at the Installment D milestone.

**Unit 7 — Recover.** L25 the orchestrator as **coordination, not a new layer** — the sixth stage, consuming detection's localised fault + owner and re-invoking an existing layer call (re-perceive / retry / re-select / skip), closing the loop. L26 the **recovery policy** (`RECOVERY_POLICY`): fault → owner → response, tagged **retryable** (NO_TARGET → re-perceive, TRACKING_FAILURE → retry-execute) vs **deterministic** (UNREACHABLE → re-select, PLAN_INVALID → skip). L27 **retry limits + state across attempts**: a transient fault recovers within budget, a persistent retryable fault escalates at the budget (`retry-budget-exhausted`), a deterministic fault escalates immediately (`skip-target`). L28 recap. Substrate: `recover(world, q_start, target=…, max_attempts, …)` with transient-vs-persistent injection via attempt-aware specs.

**Unit 8 — Full System Integration (capstone).** L29 the **full pick cycle** — the six stages composed into one self-healing loop (a clean and a recovered pick differ only in recovery bookkeeping). L30 the **capstone**: `harvest_row` runs the cycle across a whole row under a ledger (every fruit attempted once); a mid-row fault **recovers** (transient) or is **skipped-with-reason** (deterministic) — **graceful degradation**, one stubborn fruit never stalls the row. L31 **guarantees vs boundaries** (accounting / completion / graceful degradation / localised faults; *not* completeness, diagnosis, optimal sequencing, or new dynamics). L32 **Module 9 close + Module 10 handoff** (`harvest_row` exposes the whole system in one call for the Digital Twin Capstone to mirror). **Flagship D — End-to-End Pick-Cycle Player** (L8.2): animate the full row harvest, inject a fault, watch recover/skip and the live ledger.

**Boundaries held.** **No new estimation, planning, or control theory** anywhere in Module 9 — every stage wraps a real M3–M8 layer; detection is reading, recovery is coordination + bookkeeping. The M6→M7→M8→M9 chain is complete; the integrated, self-healing harvester is the deliverable.

**Module 9 totals:** 8 units · 32 lessons · 32 notebooks · 32 SVGs · **4 flagship demos** (L1.2 Pipeline Data-Flow Explorer, L4.1 Motion-Stack Visualizer, L6.2 Failure-Injection Sandbox, L8.2 End-to-End Pick-Cycle Player) · 32 quizzes · 32 answer keys · integration package `modules/module09/integration/`. **All 32 notebooks pass under Restart-and-Run-All; `mkdocs build --strict` green at 293 lesson pages (261 + 32).** **Module 9 is COMPLETE — 9 of 10 modules signed off.** Paused at the Installment D milestone. **Next:** Module 10 — the Digital Twin Capstone (single robot), which consumes `harvest_row` / the integrated system.

## D-075 — Module 10 Installment A (Units 1–2) delivered — Model → Mirror

**Directive (as relayed).** Begin Module 10, the Digital Twin Capstone (single robot, not a fleet): wrap the Module 9 integrated harvester in a lightweight digital twin along the spine **Model → Mirror → Simulate → Monitor → Predict → Adapt**. Installment A: Unit 1 *What Is a Digital Twin?* and Unit 2 *Building the Mirror: State Synchronization*. Keep the twin lightweight (no Gazebo/Isaac); represent reality and the twin separately; wrap Module 9, do not redefine it; no new theory. Pause at the Installment A milestone.

**Installment A — Units 1–2 (L01–L08).** U1 (the twin concept): a digital twin as a *living mirror* of the robot, distinguished from a model and from a simulation; what is twinned — the Module 9 system as the physical asset (`harvest_row`). U2 (building the mirror): the twin's mirrored state; `DigitalTwin.sync` (mirror reality's reported q / tool / fruit / health); sync error when the mirror drifts; the Installment-A milestone. Substrate `modules/module10/twin/`: `GroundTruth` (reality) and `DigitalTwin` (the twin) as **separate** objects with their own worlds; `snapshot`, `clone_layout`, `DigitalTwin.divergence`. **Flagship A — Twin vs Model vs Simulation** (L1.2).

**Boundaries.** Lightweight Python twin; reality/twin separated; wrap Module 9; no new theory. **APPROVED.**

## D-076 — Module 10 Installment B (Units 3–4) delivered — Simulate + Sim-to-Real Gap (midpoint)

**Directive (as relayed).** Proceed with Installment B: Unit 3 *Simulating the System in the Twin* and Unit 4 *The Sim-to-Real Gap*, with the Sim-to-Real Gap Explorer flagship and the module midpoint. Simulation is `harvest_row` run forward on a copy; the gap is inevitable and informative; calibration is *modeling a known effect, not learning*. Pause at the Installment B milestone.

**Installment B — Units 3–4 (L09–L16).** U3 (simulate): `DigitalTwin.simulate` runs the M9 harvester forward on the twin's own copy (run-ahead), the twin as a risk-free sandbox for what-ifs, determinism for replay. U4 (the gap): why twin and reality diverge (unmodeled effects on reality's world), measuring the gap (`outcome_gap`), calibrating by modeling a known effect (`DigitalTwin.calibrate`) — **modeling, not learning**, with a residual that always remains. Midpoint at L16 + `assessments/module10_midpoint_assessment.md` + coaches' key: **faithful but intentionally imperfect.** **Flagship B — Sim-to-Real Gap Explorer** (L4.1).

**Boundaries.** Reuses M9 verbatim; reality/twin separate; calibration ≠ learning; residual preserved; no new theory. **APPROVED.**

## D-077 — Module 10 Installment C (Units 5–6) delivered — Monitor + Predict

**Directive (as relayed).** Proceed with Installment C: Unit 5 *Monitoring with the Twin* and Unit 6 *Prediction with the Twin*. Monitoring is the Reality ↔ Twin comparison (*what is happening now?*) with divergence as the signal, reusing the outcome/residual gaps — no new estimation theory. Prediction is run-ahead / lookahead / what-if (*what is likely to happen next?*) executing the existing system inside the twin — no ML, RL, predictive models, or adaptive control. Pause at the Installment C milestone.

**Installment C — Units 5–6 (L17–L24).** U5 (monitor): `monitor` (Reality ↔ Twin comparison, divergence as signal), divergence-as-signal, from divergence to diagnosis; *the twin is useful because it can disagree with reality*. U6 (predict): `predict` (run-ahead from the synced state), `compare_futures` (lookahead / what-if over candidate scenarios), the honest limits of prediction (a forecast is only as faithful as the twin). The refrain: a monitor asks *what is happening now?*, a predictor asks *what is likely to happen next?* — same model, two uses. **Flagship C — Lookahead & What-If Explorer** (L6.2).

**Boundaries held.** Monitoring reads existing divergence; prediction runs the existing system forward — no estimation, ML, RL, predictive models, or adaptive control. **APPROVED.**

## D-078 — Module 10 Installment D (Units 7–8, capstone) delivered — **MODULE 10 COMPLETE · CURRICULUM COMPLETE**

**Directive (as relayed).** Proceed with Installment D: Unit 7 *Adaptation: Closing the Twin-in-the-Loop* and Unit 8 *Digital Twin Capstone & Curriculum Close*, with the Twin-in-the-Loop Harvest flagship, the one-tomato-through-ten-modules synthesis (first-class objective), and the final curriculum close. Adaptation stays pre-validation + action-selection using the existing twin — no ML, RL, adaptive control, or optimization. The capstone demonstrates **Mirror → Simulate → Monitor → Predict → Adapt** on the existing system. L31–L32 explicitly close the curriculum. Pause at Module 10 completion.

**Unit 7 — Adaptation.** L25 **pre-validation** (`prevalidate` — rehearse a candidate action in the twin before committing it; accept/reject by a stated rule). L26 **action-selection** (`select_action` — pre-validate several candidates, rank their forecasts by a stated score, choose the better one). L27 **the twin-in-the-loop cycle** (`twin_in_the_loop` — monitor → re-sync if drifted → predict → adapt, composed in order). L28 recap. Substrate appended additively to `modules/module10/twin/twin.py`; verified to discriminate (rejects a doomed pick, ranks the better action, re-syncs on drift).

**Unit 8 — Capstone & Curriculum Close.** L29 **assembling the full system** (sync/simulate/monitor/predict/adapt wired over a row; explicit reality/twin boundary, two crossing channels). L30 **capstone — the Self-Improving Greenhouse Harvest**: the twin pre-validates each pick, foresees a doomed pick, and adapts (de-target/skip), beating the blind plan with equal harvest and less wasted effort — *self-improving = loop-improved, not learned*. L31 **one tomato through ten modules** (M1 vector → M2 frames → M3 perception → M4 FK → M5 IK → M6 Jacobian → M7 trajectory → M8 control → M9 integration → M10 twin), a first-class synthesis lesson. L32 **course close** (the through-line *represent → perceive → reach → move → integrate → twin*; scope boundaries as the edges of a foundation). **Flagship D — Twin-in-the-Loop Harvest** (L8.2): real harvest + twin together, pre-validate/predict-failure/adapt, spine lighting stage by stage, live blind-vs-loop ledger.

**Boundaries held.** Adaptation is pre-validation + action-selection over a given candidate set with stated rules — **no machine learning, RL, adaptive control, or optimization**; the capstone composes verified operators with no new theory; reality and twin stay separate.

**Module 10 totals:** 8 units · 32 lessons · 32 notebooks · 32 SVGs · **4 flagship demos** (L02 Twin vs Model vs Simulation, L13 Sim-to-Real Gap Explorer, L22 Lookahead & What-If Explorer, L30 Twin-in-the-Loop Harvest) · 32 quizzes · 32 answer keys · midpoint assessment + key · twin substrate `modules/module10/twin/`. **All 32 notebooks pass under Restart-and-Run-All; `mkdocs build --strict` green at 325 lesson pages (293 + 32).**

**Module 10 sign-off: APPROVED. Module 10 is COMPLETE — 10 of 10 modules signed off.** With it, the **ten-module Physical AI curriculum is COMPLETE.** Final deliverables produced: Module 10 completion report, curriculum completion report, and the PROJECT_STATE / master_progress / ARCHITECT_DECISIONS updates. **Paused at full curriculum completion; no new module production remains.**
