...
unit: 07
lesson: 7.3
title: Estimating the Fruit's World Position
core_idea: "The full perception-to-world estimate: detect a pixel, undistort, back-project with depth, then transform through the extrinsics chain to a world-frame position the arm can use."
estimated_time: 50
difficulty: Core
prerequisites: [7.2]
learning_objectives:
  - Assemble the end-to-end pixel-to-world estimate.
  - Apply undistortion, back-projection, and the extrinsics chain in order.
  - Sanity-check the world-frame result.
tags:
  - physical-ai
  - perception
  - pipeline
...
`[Visual: the full pixel-to-world pipeline as a five-stage flow, with the fruit ending at a world-frame position near the robot]`

**Diagram Specification**
- **Objective:** show the assembled estimate end to end.
- **Scene:** flow: detected pixel → "undistort (U5)" → "K⁻¹ ray + depth (U6)" → P_c (camera) → "T(world←cam) (U7)" → P_w (world) shown at a tomato near the robot base.
- **Labels:** each stage with its unit; final "fruit world position → arm target."
- **Form:** SVG.
