...
unit: 01
lesson: 1.2
title: "World → Pixels → World: The Perception Problem"
core_idea: "Perception is a round trip: the camera maps the 3D world to pixels (forward), and the robot must recover a 3D position from pixels (inverse) to act."
estimated_time: 40
difficulty: Introductory
prerequisites: [1.1]
learning_objectives:
  - Distinguish the forward problem (world → pixels) from the inverse problem (pixels → world).
  - Explain why the inverse problem needs extra information.
  - Place Module 3 (forward + back to camera frame) and Module 2 (camera frame → world) in one pipeline.
tags:
  - physical-ai
  - perception
  - projection
...
`[Visual: the round trip — a 3D world point projecting to a pixel (forward, many points on one ray → one pixel), and a pixel back-projecting to a ray that needs depth to pick the point (inverse)]`

**Diagram Specification**
- **Objective:** contrast forward (world→pixel, many-to-one) with inverse (pixel→ray, needs depth).
- **Scene:** left: several 3D points along one ray all hitting the same pixel (forward, depth lost). Right: one pixel expanding into a ray into the scene, with a marker "depth picks the point" (inverse).
- **Labels:** "forward: world → pixels (easy, loses depth)," "inverse: pixels → world (needs depth)," "pixel = a ray."
- **Form:** SVG.
