...
unit: 07
lesson: 7.1
title: Reading the End-Effector Pose
core_idea: "The end-effector pose T_0^n is the answer to 'where is the gripper, and how is it turned?' Reading it well — position, approach axis, and what each tells the planner — is the point of all the machinery."
estimated_time: 45
difficulty: Core
prerequisites: [6.4]
learning_objectives:
  - Interpret the full end-effector pose for a real arm.
  - Identify the approach axis and what it means for grasping.
  - Connect pose to a grasp decision.
tags:
  - physical-ai
  - kinematics
  - pose
...
`[Visual: an end-effector pose at a fruit, showing the position p, the approach axis ẑ_g toward the fruit, and the finger-facing axes, with the matrix beside it]`

**Diagram Specification**
- **Objective:** show reading a pose for grasping.
- **Scene:** a gripper near a fruit; its origin marked $\mathbf{p}$; the approach axis $\hat z_g$ as an arrow pointing into the fruit; $\hat x_g,\hat y_g$ showing finger orientation; the $T_0^n$ matrix beside it with position column and rotation block boxed.
- **Labels:** "position p = where," "ẑ_g = approach direction," "x̂_g, ŷ_g = finger facing," "grasp when T_0^n ≈ T_grasp."
- **Form:** SVG.
