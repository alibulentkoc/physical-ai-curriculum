...
unit: 04
lesson: 4.3
title: Forward Kinematics in Code
core_idea: "A clean FK implementation builds each joint's SE(3) factor from its parameters and multiplies them in order. The same function handles any serial arm; SymPy can keep it symbolic."
estimated_time: 50
difficulty: Core
prerequisites: [4.2]
learning_objectives:
  - Implement general forward kinematics as a matrix product.
  - Verify it against the planar closed form.
  - Use SymPy to get a symbolic forward-kinematics expression.
tags:
  - physical-ai
  - kinematics
  - implementation
...
`[Visual: a code-flow diagram — list of joint factors → multiply in order → T_0^n → extract position and orientation — with a SymPy branch giving a symbolic formula]`

**Diagram Specification**
- **Objective:** show the FK code pipeline.
- **Scene:** boxes left→right: "joint factors [T_0^1, …]" → "multiply in order (∏)" → "T_0^n" → split into "position p" and "orientation R"; a lower branch "SymPy → symbolic p(θ), R(θ)."
- **Labels:** "one factor per joint," "numeric (NumPy) and symbolic (SymPy)," "verify vs planar closed form."
- **Form:** SVG.
