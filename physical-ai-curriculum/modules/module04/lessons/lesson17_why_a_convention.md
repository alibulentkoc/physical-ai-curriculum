...
unit: 05
lesson: 5.1
title: Why a Convention
core_idea: "Hand-built joint transforms are ad hoc and error-prone. A shared convention for placing frames lets any serial robot be described the same way — so kinematics becomes a table, not a bespoke derivation."
estimated_time: 40
difficulty: Core
prerequisites: [4.4]
learning_objectives:
  - Explain why an arbitrary frame placement makes transforms inconsistent.
  - State the goal of the Denavit–Hartenberg convention.
  - Recognize a convention as a shared standard, not new physics.
tags:
  - physical-ai
  - kinematics
  - denavit-hartenberg
...
`[Visual: two engineers placing frames differently on the same arm (inconsistent matrices) vs the DH convention giving one agreed frame placement and a compact 4-number description]`

**Diagram Specification**
- **Objective:** contrast arbitrary frame placement with a shared convention.
- **Scene:** left — the same 2-link arm with two different sets of frames drawn (labeled "engineer A," "engineer B," "different matrices"); right — the same arm with the DH frames (z along joint axes, x along common normals) and a small "(θ, d, a, α) per joint" table.
- **Labels:** "arbitrary frames → inconsistent," "DH convention → one standard," "6 free choices → 4 parameters."
- **Form:** SVG.
