...
unit: 04
lesson: 4.5
title: Applying SE(3); Inverses in 3D
core_idea: "Applying SE(3) moves a 3D point or frame rigidly; its inverse — rotate back, then move back — undoes it and is itself SE(3)."
estimated_time: 45
difficulty: Introductory
prerequisites: [4.4]
learning_objectives:
  - Apply an SE(3) matrix to a 3D point and to a frame.
  - Compute and use the inverse of an SE(3) transform.
  - Confirm SE(3) motion is rigid (distances preserved).
tags:
  - physical-ai
  - transformations
  - se3
...
`[Visual: faux-3D isometric — an object moved by SE(3) to a new pose, and T-inverse returning it exactly; notes "rotate back, then move back" in 3D]`

**Diagram Specification**
- **Objective:** show applying SE(3) and its inverse (return trip) in 3D.
- **Scene:** faux-3D isometric: a small frame/object at start (ghost), an arrow labeled T to its moved pose, and a return arrow labeled T⁻¹ back onto the start; a tracked edge labeled equal length in both to show rigidity.
- **Labels:** "T (apply)," "T⁻¹ (rotate back, then move back)," "T⁻¹T = identity," "same size (rigid)."
