...
core_idea: "Implement the perception-to-pose pipeline as three SE(3) poses composed into the tomato's world pose."
estimated_time: 50
difficulty: Capstone
prerequisites: [8.1]
learning_objectives:
  - Implement the three given poses as SE(3) matrices.
  - Compose them into the tomato's world pose.
  - Extract the world position and orientation for the planner.
tags:
  - physical-ai
  - transformations
  - capstone
...
`[Visual: three SE(3) matrices (detection, extrinsics, arm pose) multiplied left-to-right into one world-pose matrix, with position and orientation highlighted in the result]`

**Diagram Specification**
- **Objective:** show the three matrices composing into the result, with the readout highlighted.
- **Scene:** three labeled 4×4 blocks (T(world←arm), T(arm←cam), T(cam←tomato)) with multiplication dots, equals one 4×4 result; the result's translation column ("world position") and rotation block ("world orientation") highlighted.
- **Labels:** "arm pose," "extrinsics," "detection," "= tomato world pose," "read position + orientation."
- **Form:** SVG.
