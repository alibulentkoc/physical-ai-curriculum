...
core_idea: "The inverse of a rigid motion is the motion that undoes it — go back the way you came; for SE(2) it is itself an SE(2) transform."
estimated_time: 45
difficulty: Introductory
prerequisites: [3.3]
learning_objectives:
  - Describe the inverse of a transformation as the motion that undoes it.
  - Recognize that the inverse of an SE(2) transform is itself SE(2).
  - Use an inverse to convert a point back to the original frame.
tags:
  - physical-ai
  - transformations
  - se2
...
`[Visual: a point moved by T to a new pose, and T-inverse bringing it exactly back; arrows showing "rotate back, then move back"]`

**Diagram Specification**
- **Objective:** show the inverse as the return trip that lands exactly on the start.
- **Scene:** start pose (ghost) → arrow labeled T → moved pose; a second arrow labeled T⁻¹ curving back onto the start pose exactly; small notes "undo the rotation," "undo the translation."
- **Labels:** "T (go)," "T⁻¹ (come back)," "T⁻¹T = identity," "back to start."
- **Form:** SVG.
